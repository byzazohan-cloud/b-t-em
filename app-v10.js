...
function expenseForm(x={}){
  return`<form class="form" id="expenseForm">
    ...
    ${select('category','Kategori',C,x.category||'Market')}
    ${input('date','Harcama Tarihi',x.date||iso(),'date')}
    <div class="notice">NORMAL HARCAMALARDA SON ÖDEME TARİHİ YOKTUR. SON ÖDEME SADECE SABİT GİDERLER/FATURALAR VE KREDİ KARTLARINDA KULLANILIR.</div>
...
function cardForm(c={}){
  ...
  return`<form class="form" id="cardForm">
    ...
    ${select('style','Kart Stili',['blackgold','green','blue'],c.style||'blackgold')}
...
async function saveExpense(d,i){
  ...
  const x={id:i||id(),title:upper((d.title||'GİDER').trim()||'GİDER'),amount,category:d.category,date:d.date||iso(),dueDate:null,recurring:false,paid:true};
