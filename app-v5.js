...
async function unlock(p){const m=meta();if(!m)return false;try{const k=await derive(p,ub64(m.salt)),st=await decrypt(JSON.parse(localStorage.getItem(DATA)),k);key=k;state=st;applyTheme();return true}catch{return false}}
function def(){return{version:1,selectedMonth:ym(new Date()),profile:{name:'',photo:'',motto:'Disiplin, özgürlüğün kapısını açar.'},settings:{lockMinutes:15,leadDays:3,notifications:false},theme:{bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'},incomes:[],expenses:[],cards:[]}}
...
function profile(){return`<div class="profile card"><div class="ava">${state.profile.photo?`<img src="${state.profile.photo}">`:esc((state.profile.name||'H')[0])}</div><h2>${esc(state.profile.name||'HANE')}</h2><small>${esc(state.profile.motto||'')}</small></div><div class="card" style="margin-top:12px"><div class="setting" data-action="editProfile"><span>●</span><span>Profil Resmi ve İsim</span><b>›</b></div><div class="setting" data-action="changePin"><span>🔒</span><span>PIN Değiştir</span><b>›</b></div><div class="setting"><span>⏱</span><span>Otomatik Kilit</span><b>${state.settings.lockMinutes} Dakika</b></div><div class="setting" data-tab="theme"><span>🎨</span><span>Tema Stüdyosu</span><b>›</b></div><div class="setting" data-tab="backup"><span>☁</span><span>Yedekleme</span><b>›</b></div><div class="setting" data-tab="settings"><span>⚙</span><span>Uygulama Ayarları</span><b>›</b></div></div>`}
...
function colorForm(k,label){return`<div class="field"><label>${label}</label><input id="nativeColor" type="color" value="${state.theme[k]}" style="height:54px"></div><button class="btn gold" style="width:100%;margin-top:12px" data-action="saveColor" data-kind="${k}">Tamam</button>`}
...
function bind(){
  $$('[data-tab]').forEach(x=>x.onclick=()=>{current=x.dataset.tab;modal=null;render()});
...
async function act(a,el){if(a==='back'){current='home';render()}else if(a==='addIncome')open('Gelir Ekle',incomeForm());else if(a==='editIncome'){const x=state.incomes.find(z=>z.id===el.dataset.id);open('Geliri Düzenle',incomeForm(x),{id:x.id})}else if(a==='delIncome'){state.incomes=state.incomes.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='addExpense')open('Gider Ekle',expenseForm());else if(a==='editExpense'){const x=state.expenses.find(z=>z.id===el.dataset.id);open('Gideri Düzenle',expenseForm(x),{id:x.id})}else if(a==='delExpense'){state.expenses=state.expenses.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='addCard')open('Kart Ekle',cardForm());else if(a==='editCard'){const c=state.cards.find(z=>z.id===el.dataset.id);open('Kartı Düzenle',cardForm(c),{id:c.id})}else if(a==='delCard'){state.cards=state.cards.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='editProfile')open('Profili Düzenle',profileForm());else if(a==='pickProfile')$('#profileInput').click();else if(a==='receipt')$('#receiptInput').click();else if(a==='close'){modal=null;render()}else if(a==='pickBg')open('Arka Plan Rengi',colorForm('bg','Arka Plan Rengi'));else if(a==='pickAccent')open('Detay Rengi',colorForm('accent','Detay Rengi'));else if(a==='pickIncome')open('Grafik Gelir',colorForm('income','Gelir Rengi'));else if(a==='pickExpense')open('Grafik Gider',colorForm('expense','Gider Rengi'));else if(a==='pickRemain')open('Grafik Kalan',colorForm('remain','Kalan Rengi'));else if(a==='saveColor'){state.theme[el.dataset.kind]=$('#nativeColor').value;await save();modal=null;render()}else if(a==='preset'){state.theme.bg=el.dataset.bg;state.theme.accent=el.dataset.accent;await save();render()}else if(a==='backupNow')backupNow();else if(a==='restoreNow')$('#restoreInput').click();else if(a==='changePin')changePin();else if(a==='clearAll'){if(confirm('Tüm HANE verileri silinsin mi?')){localStorage.removeItem(META);localStorage.removeItem(DATA);location.reload()}}}
