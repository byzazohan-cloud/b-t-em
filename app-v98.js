...
function buildTopBar(){
  if(current==='home')return`<div class="top homeTop v97HomeTop"><button class="ib premiumTopIcon menuBtn" data-action="openMenu">☰</button><div class="brand brandLogo officialBrand">${haneLogo(44,'brandMark')}</div><div class="topRight"><button class="ib premiumTopIcon" data-tab="alerts">${premiumIcon('bell',28)}</button><button class="v97IdentityButton" data-action="openIdentity" aria-label="Kimlik Kartım">${state.profile.photo?`<img src="${state.profile.photo}" alt="Profil">`:`<span>${esc((state.profile.name||'H')[0])}</span>`}<i></i></button></div></div>`;
  const t={transactions:'HAREKETLER',fixed:'GİDERLER',cards:'FİNANS',calendar:'TAKVİM',reports:'RAPORLAR',profile:'PROFİL',backup:'YEDEKLEME',settings:'AYARLAR',members:'HANE ÜYELERİ',homeEdit:'ANA SAYFAYI DÜZENLE',categories:'KATEGORİLER',theme:'TEMA STÜDYOSU',alerts:'HATIRLATMALAR',about:'HAKKINDA',monthSpent:'BU AY HARCANAN',monthPaid:'AYLIK HESAP'};
...
function identityCardBody(){
  const members=Math.max(1,(state.members||[]).length),cards=(state.cards||[]).length,accounts=(state.accounts||[]).length+(state.flexAccounts||[]).length,total=(state.incomes||[]).length+(state.expenses||[]).length+(state.cardPayments||[]).length+(state.flexTransactions||[]).length;
  const avatar=state.profile.photo?`<img src="${state.profile.photo}" alt="Profil">`:`<span>${esc((state.profile.name||'H')[0])}</span>`;
  return `<div class="v97IdentityCard"><div class="v97IdBrand">${haneLogo(38,'v97IdLogo')}<small>KİMLİK KARTIM</small></div><div class="v97IdPerson"><div class="v97IdAvatar">${avatar}</div><div><h2>${esc(state.profile.name||'HANE')}</h2><b>HANE YÖNETİCİSİ</b><p>${esc(state.profile.motto||'Planlarım, yarınlarım için.')}</p></div></div><div class="v97IdStats"><div><span>Hane Üyesi</span><b>${members}</b></div><div><span>Aktif Kart</span><b>${cards}</b></div><div><span>Kayıtlı Hesap</span><b>${accounts}</b></div><div><span>Toplam Kayıt</span><b>${total.toLocaleString('tr-TR')}</b></div></div><button class="btn v97IdPrimary" data-action="editProfile">PROFİLİMİ DÜZENLE ›</button><div class="v97IdLinks"><button data-tab="members">HANE ÜYELERİ</button><button data-action="changePin">GÜVENLİK</button><button data-action="editProfile">KİŞİSEL BİLGİLER</button><button data-action="openTheme">TEMA AYARLARI</button></div></div>`;
}
function home(){const T=totals();return`<section class="v97HomeIntro" data-ui-build="V98"><div><small>İYİ ${new Date().getHours()<12?'SABAHLAR':new Date().getHours()<18?'GÜNLER':'AKŞAMLAR'},</small><h2>${esc(state.profile.name||'HANE')}</h2><p>Bugün de kontrol sende.</p></div><time>${new Date().toLocaleDateString('tr-TR',{day:'2-digit',month:'long',year:'numeric'})}</time></section>
<div class="v97MonthLine"><button data-action="monthPrev">‹</button><b>${monthLabel(state.selectedMonth)}</b><button data-action="monthNext">›</button></div>
<section class="v97Summary"><button data-action="summaryDetail" data-kind="income"><i>${premiumIcon('income',25)}</i><span>GELİR</span><strong>${money(T.i)}</strong></button><button data-action="summaryDetail" data-kind="expense"><i>${premiumIcon('expense',25)}</i><span>GİDER</span><strong>${money(T.e)}</strong></button><button data-action="summaryDetail" data-kind="remain"><i>=</i><span>KALAN</span><strong>${money(T.r)}</strong></button><p>Planlı bir bugün, daha güçlü bir yarın.</p></section>
<div class="v97MainActions"><button data-action="addIncome"><i>＋</i><span><b>Gelir Ekle</b><small>Kazancını kaydet</small></span></button><button data-action="addExpense"><i>−</i><span><b>Gider Ekle</b><small>Harcamanı kaydet</small></span></button></div>
<div class="v97Shortcuts"><button data-tab="cards">${premiumIcon('cards',25)}<span>Kartlar</span></button><button data-tab="monthPaid">${premiumIcon('fixed',25)}<span>Aylık Hesap</span></button><button data-tab="fixed">${premiumIcon('home',25)}<span>Sabit Giderler</span></button><button data-tab="reports">${premiumIcon('report',25)}<span>Raporlar</span></button><button data-action="openMenu">•••<span>Diğer</span></button></div>
<div class="card monthlyPreview v97MonthlyPreview">${monthlyAccountPreview(3)}</div>
<div class="v97Recent">${recentMovementsHome(3)}</div>`}
function transactions(){
  ...
  return `${monthNavigator()}<div class="seg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Ödeme')}</div><div class="liveTxSearch"><span>⌕</span><input id="txSearch" autocomplete="off" placeholder="Hareket ara..." value="${esc(txSearch)}">${q?'<button type="button" data-action="clearTxSearch">×</button>':''}</div><div class="section"><b>${q?'ARAMA SONUÇLARI':state.selectedMonth}</b><span>${q?a.length:visible} SATIR · ${a.length} KAYIT</span></div><div class="list">${a.length?rows:'<div class="notice">Eşleşen hareket bulunamadı.</div>'}</div>`
}
...
function fixed(){
  ...
  return`${monthNavigator()}<div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ SABİT GİDER</button></div><div class="notice fixedSortNote">BU EKRAN SABİT GİDER YÖNETİMİ İÇİNDİR. NORMAL HARCAMALAR HAREKETLER'DE, ANALİZLER RAPORLAR'DA GÖRÜNÜR.</div><div class="list fixedList">${fixedHtml}</div><div class="section"><b>YÖNETİM</b><span></span></div><div class="cleanManageLinks"><button type="button" data-tab="categories"><span>KATEGORİLER</span><b>YÖNET ›</b></button><button type="button" data-tab="monthPaid"><span>AYLIK HESAP</span><b>AÇ ›</b></button></div>`
}
