...
function cardPaymentInfo(c,base=new Date()){
  ...
  const statementDay=Math.max(1,Math.min(31,Number(c.statementDay)||1));
  const dueDay=Math.max(1,Math.min(31,Number(c.dueDay)||1));
  const statementMonth=t.getMonth()+(t.getDate()>statementDay?1:0);
  const statementDate=new Date(t.getFullYear(),statementMonth,statementDay);
  const dueMonth=statementDate.getMonth()+(dueDay>statementDay?0:1);
  const dueDate=new Date(statementDate.getFullYear(),dueMonth,dueDay);
  return{statementDate,dueDate,days:Math.max(0,Math.ceil((dueDate-t)/86400000))}
}
...
function reports(){const T=periodTotals(),B=(k,l)=>`<button data-action="reportPeriod" data-period="${k}" class="${reportPeriod===k?'active':''}">${l}</button>`;return`<div class="seg">${B('day','Gün')}${B('week','Hafta')}${B('month','Ay')}${B('year','Yıl')}</div><div class="section"><b>${T.range.label}</b><span></span></div><div class="summary"><div class="sum"><label>Toplam Gelir</label><strong style="color:var(--green)">${money(T.i)}</strong></div><div class="sum"><label>Toplam Gider</label><strong style="color:var(--red)">${money(T.e)}</strong></div><div class="sum"><label>Kalan</label><strong>${money(T.r)}</strong></div></div><div class="card" style="padding:14px;margin-top:10px"><canvas id="chart" style="width:100%;height:220px"></canvas></div>`}
...
async function saveExpense(d,i){const amount=Number(String(d.amount||'0').replace(',','.'));if(!Number.isFinite(amount)||amount<0)throw new Error('Tutar geçersiz');const x={id:i||id(),title:upper((d.title||'GİDER').trim()||'GİDER'),amount,category:d.category,date:d.date||iso(),dueDate:d.dueDate||d.date||iso(),recurring:d.recurring==='true',paid:d.paid==='true'};if(i){const n=state.expenses.findIndex(z=>z.id===i);state.expenses[n]={...state.expenses[n],...x}}else state.expenses.push(x);await save();modal=null;render()}
async function saveCard(d,i){const x={id:i||id(),bank:upper(d.bank||'BANKA'),name:upper(d.name||'KART'),last4:(d.last4||'0000').replace(/\D/g,'').slice(-4).padStart(4,'0'),limit:+d.limit||0,balance:+d.balance||0,statementDay:+d.statementDay||12,dueDay:+d.dueDay||27,style:d.style||'blackgold',network:'VISA'};if(i){const n=state.cards.findIndex(z=>z.id===i);state.cards[n]={...state.cards[n],...x}}else state.cards.push(x);await save();modal=null;render()}
