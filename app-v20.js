...
let navHistory=['home'],calendarDate=new Date(),customStart=iso(new Date(new Date().getFullYear(),new Date().getMonth(),1)),customEnd=iso(),txSearch='',txCategory='all',txSource='all',txMember='all',txMin='',txMax='',txStart='',txEnd='';
...
function normalizeV19(st){
  ...
  st.accounts=Array.isArray(st.accounts)?st.accounts:[];st.flexAccounts=Array.isArray(st.flexAccounts)?st.flexAccounts:[];st.categoryMeta=st.categoryMeta||{};st.customCategories=Array.isArray(st.customCategories)?st.customCategories:[];st.members=Array.isArray(st.members)?st.members:[];st.installments=Array.isArray(st.installments)?st.installments:[];st.settings=st.settings||{};if(typeof st.settings.privacy!=='boolean')st.settings.privacy=false;st.homeLayout=Array.isArray(st.homeLayout)?st.homeLayout:['summary','quick','month','recommended'];st.homeHidden=Array.isArray(st.homeHidden)?st.homeHidden:[];st.fixedPayments=Array.isArray(st.fixedPayments)?st.fixedPayments:[];st.flexTransactions=Array.isArray(st.flexTransactions)?st.flexTransactions:[];st.version=20;
...
function def(){return{version:20,selectedMonth:ym(new Date()),profile:{name:'',photo:'',motto:'Disiplin, özgürlüğün kapısını açar.'},settings:{lockMinutes:15,leadDays:3,notifications:false,darkMode:true},theme:{bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'},incomes:[],expenses:[],cards:[],accounts:[],flexAccounts:[],customCategories:[],categoryMeta:{},cardTransactions:[],cardPayments:[],members:[],installments:[],homeLayout:['summary','quick','month','recommended']}}
...
function buildTopBar(){
  ...
  const t={transactions:'HAREKETLER',fixed:'GİDERLER',cards:'FİNANS',reports:'RAPORLAR',profile:'PROFİL',backup:'YEDEKLEME',settings:'AYARLAR',categories:'KATEGORİLER',theme:'TEMA STÜDYOSU',alerts:'HATIRLATMALAR',about:'HAKKINDA',monthSpent:'BU AY HARCANAN',monthPaid:'BU AY ÖDENEN',calendar:'TAKVİM',members:'HANE ÜYELERİ'};
...
function menuBody(){const a=[['home','🏠','ANA EKRAN'],['transactions','📋','HAREKETLER'],['fixed','🧾','GİDERLER'],['cards','💳','KARTLAR / ESNEK HESAP'],['reports','📊','RAPORLAR'],['calendar','▦','TAKVİM'],['alerts','🔔','HATIRLATMALAR'],['members','♟','HANE ÜYELERİ'],['settings','⚙️','AYARLAR'],['profile','👤','PROFİL']];return `<div class="menuList">${a.map(x=>`<button data-action="menuGo" data-go="${x[0]}"><i>${x[1]}</i><b>${x[2]}</b><span>›</span></button>`).join('')}</div>`}
...
function fixed(){
  ...
  return`<div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ GİDER EKLE</button></div><div class="notice sortHint">⋮⋮ TUTUP SÜRÜKLEYEREK SIRALAYABİLİRSİN.</div><div class="list fixedList">${a.length?a.map(x=>{const paid=fixedPaidForMonth(x);return`<div class="item fixedItem ${paid?'isPaid':''}" draggable="true" data-fixed-id="${x.id}"><button class="dragHandle" type="button">⋮⋮</button><div class="ico premiumIco" data-action="editFixedExpense" data-id="${x.id}">${I[x.category]||'●'}</div><div data-action="editFixedExpense" data-id="${x.id}"><b>${esc(x.title)}</b><small>${x.dueDate||x.date} · SABİT</small></div><div class="fixedRight"><b style="color:${paid?'var(--green)':'var(--red)'}">${paid?'✓ ':'-'}${money(x.amount)}</b><button type="button" class="paidToggle ${paid?'on':''}" data-action="toggleFixedPaid" data-id="${x.id}">${paid?'✓ ÖDENDİ':'ÖDEDİM'}</button></div></div>`}).join(''):'<div class="notice">SABİT GİDER YOK.</div>'}</div>`
}
...
function flexPanel(){return `<div class="section"><b>ESNEK HESAPLAR</b><button class="miniAddBtn" data-action="addFlex">+ ESNEK HESAP</button></div><div class="financeCarousel">${state.flexAccounts.map(a=>`<div class="flexAccount luxuryCard"><div class="cardTop"><b>${esc(a.bank)}</b><span>ESNEK</span></div><small>${esc(a.name)}</small><div class="grid"><div><small>LİMİT</small><b>${money(a.limit)}</b></div><div><small>KULLANILAN</small><b>${money(a.balance)}</b></div><div><small>HESAP KESİM</small><b>${esc(a.statementDate||'-')}</b></div><div><small>SON ÖDEME</small><b>${esc(a.dueDate||'-')}</b></div></div><div class="cardActions"><button class="btn" data-action="flexSpend" data-id="${a.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="flexPay" data-id="${a.id}">₺ ÖDEME YAPTIM</button><button class="iconEdit" data-action="editFlex" data-id="${a.id}">✎</button></div></div>`).join('')||'<div class="notice">HENÜZ ESNEK HESAP EKLENMEDİ.</div>'}</div>`}
...
function reports(){const T=periodTotals(),inside=x=>x.date>=T.range.start&&x.date<=T.range.end,B=(k,l)=>`<button data-action="reportPeriod" data-period="${k}" class="${reportPeriod===k?'active':''}">${l}</button>`,items=[...state.incomes.filter(inside).map(x=>({...x,_kind:'income'})),...state.expenses.filter(inside).map(x=>({...x,_kind:'expense'}))].sort((a,b)=>String(b.date).localeCompare(String(a.date)));return`<div class="seg">${B('day','Gün')}${B('week','Hafta')}${B('month','Ay')}${B('year','Yıl')}</div><div class="section"><b>${T.range.label}</b><span></span></div><div class="summary"><div class="sum"><label>Toplam Gelir</label><strong style="color:var(--green)">${money(T.i)}</strong></div><div class="sum"><label>Toplam Gider</label><strong style="color:var(--red)">${money(T.e)}</strong></div><div class="sum"><label>Kalan</label><strong>${money(T.r)}</strong></div></div><div class="card" style="padding:14px;margin-top:10px"><canvas id="chart" style="width:100%;height:220px"></canvas><div class="chartLegend"><span><i style="background:var(--gi)"></i>GELİR</span><span><i style="background:var(--ge)"></i>GİDER</span><span><i style="background:var(--gr)"></i>KALAN</span></div></div><div class="section"><b>GRAFİĞE AİT HAREKETLER</b><span>${items.length} KAYIT</span></div><div class="list reportMoves">${items.length?items.map(x=>`<div class="item"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x._kind==='income'?'GELİR':esc(x.category||'DİĞER')}${x._kind==='expense'?' · '+paymentLabel(x):''}</small></div><div class="reportMoveRight"><b style="color:${x._kind==='income'?'var(--green)':'var(--red)'}">${x._kind==='income'?'+':'-'}${money(x.amount)}</b><div><button type="button" data-action="${x._kind==='income'?'editIncome':'editExpense'}" data-id="${x.id}">DÜZENLE</button><button type="button" class="danger" data-action="${x._kind==='income'?'delIncome':'delExpense'}" data-id="${x.id}">SİL</button></div></div></div>`).join(''):'<div class="notice">BU GRAFİK ARALIĞINDA HAREKET YOK.</div>'}</div>`}
...
function settings(){return`<div class="card"><div class="setting" data-tab="categories"><span class="settingIco">🎨</span><span>KATEGORİLERİ DÜZENLE / EKLE</span><b>›</b></div><div class="setting" data-action="toggleDarkMode"><span class="settingIco">${premiumIcon("palette",30)}</span><span>KARANLIK MOD</span><b>${state.settings.darkMode!==false?'Açık':'Kapalı'}</b></div><div class="setting"><span class="settingIco">${premiumIcon("bell",30)}</span><span>BİLDİRİMLER</span><b>${state.settings.notifications?'Açık':'Kapalı'}</b></div><div class="setting" data-action="togglePrivacy"><span class="settingIco">◉</span><span>GİZLİLİK MODU</span><b>${state.settings.privacy?'Açık':'Kapalı'}</b></div><div class="setting" data-action="homeCustomize"><span class="settingIco">▦</span><span>ANA SAYFAYI DÜZENLE</span><b>›</b></div><div class="setting" data-tab="members"><span class="settingIco">♟</span><span>HANE ÜYELERİ</span><b>›</b></div><div class="setting" data-tab="about"><span class="settingIco">${premiumIcon("info",30)}</span><span>HAKKINDA</span><b>›</b></div><div class="setting" data-action="clearAll"><span class="settingIco dangerIco">${premiumIcon("trash",30)}</span><span style="color:var(--red)">TÜM VERİLERİ SIFIRLA</span><b></b></div></div>`}
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 20.0</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
function view(){return({home,transactions,fixed,cards,reports,profile,backup,settings,categories,theme,alerts,about,monthSpent,monthPaid,calendar,members}[current]||home)()}
...
function incomeForm(x={}){const mem=['HANE',...(state.members||[]).map(m=>m.name)];return`<form class="form" id="incomeForm">${input('title','Gelir Adı',x.title||'')}${input('amount','Tutar',x.amount||0,'number')}${input('date','Tarih',x.date||iso(),'date')}${select('member','Hane Üyesi',mem,x.member||'HANE')}<div class="field"><label>Sabit Gelir</label><select name="recurring"><option value="false" ${!x.recurring?'selected':''}>Hayır</option><option value="true" ${x.recurring?'selected':''}>Evet</option></select></div><button class="btn gold" type="submit">Kaydet</button>${x.id?`<button type="button" class="btn" data-action="delIncome" data-id="${x.id}">Sil</button>`:''}</form>`}
...
function cardSpendForm(c){const mem=['HANE',...(state.members||[]).map(x=>x.name)];return`<form class="form" id="cardSpendForm">${input('amount','Tutar',0,'number','step="0.01" min="0"')}${input('title','Açıklama','')}${select('category','Kategori',C,'Market')}${input('date','Tarih',iso(),'date')}${select('member','Kim yaptı?',mem,'HANE')}<div class="row2"><div class="field"><label>Ödeme</label><select name="payType"><option>Tek Çekim</option><option>Taksitli</option></select></div>${input('installmentCount','Taksit Sayısı',1,'number','min="1" max="36"')}</div><div class="notice">Taksitli seçersen HANE taksit planını aylara otomatik dağıtır.</div><button type="button" class="btn" data-action="receipt">📷 Fiş / Kamera</button><button class="btn gold" type="submit">Harcamayı Kaydet</button></form>`}
...
function bind(){
  bindFixedSorting();
  $$('[data-tab]').forEach(x=>x.onclick=()=>{const next=x.dataset.tab;if(next==='theme')themeDraft={...(state.theme||{})};else if(current==='theme')themeDraft=null;if(next!==current)navigateTo(next);else render()});
  $$('[data-action]').forEach(x=>x.onclick=e=>{if(x.closest('form')&&x.type==='submit')return;e.stopPropagation();act(x.dataset.action,x)});

  const homeCustomizeFormEl=$('#homeCustomizeForm');if(homeCustomizeFormEl)homeCustomizeFormEl.onsubmit=async e=>{e.preventDefault();const d=new FormData(e.currentTarget);state.homeHidden=state.homeLayout.filter(k=>!d.get('show_'+k));await save();modal=null;render();};
  const memberFormEl=$('#memberForm');if(memberFormEl)memberFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),x={id:modal?.id||id(),name:upper(d.name||'ÜYE'),icon:d.icon||'●'};if(modal?.id){const i=state.members.findIndex(z=>z.id===modal.id);state.members[i]=x}else state.members.push(x);await save();modal=null;render()};
  ...
  const flexFormEl=$('#flexForm');if(flexFormEl)flexFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),x={id:modal?.id||id(),bank:upper(d.bank||'BANKA'),name:upper(d.name||'ESNEK HESAP'),limit:+d.limit||0,balance:+d.balance||0,statementDate:d.statementDate,dueDate:d.dueDate};const n=state.flexAccounts.findIndex(a=>a.id===x.id);n>=0?state.flexAccounts[n]=x:state.flexAccounts.push(x);await save();modal=null;render()};
  const simpleAmountFormEl=$('#simpleAmountForm');if(simpleAmountFormEl)simpleAmountFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),kind=e.currentTarget.dataset.kind,objId=e.currentTarget.dataset.id,amt=+d.amount||0;if(amt<=0)return alert('Tutar girin');if(kind.startsWith('account')){const a=state.accounts.find(x=>x.id===objId);if(kind==='accountSpend'){a.balance-=amt;state.expenses.push({id:id(),title:upper(d.title||a.bank+' HARCAMA'),amount:amt,category:'Diğer',date:d.date,recurring:false,paid:true,source:'cash'})}else{a.balance+=amt;state.incomes.push({id:id(),title:upper(d.title||a.bank+' GELİR'),amount:amt,date:d.date,recurring:false})}}else{const a=state.flexAccounts.find(x=>x.id===objId);if(kind==='flexSpend'){a.balance+=amt;state.expenses.push({id:id(),title:upper(d.title||a.bank+' ESNEK HESAP'),amount:amt,category:'Diğer',date:d.date,recurring:false,paid:true,source:'cash'})}else{a.balance=Math.max(0,a.balance-amt)}state.flexTransactions=state.flexTransactions||[];state.flexTransactions.push({id:id(),flexId:a.id,type:kind==='flexSpend'?'spend':'payment',amount:amt,title:upper(d.title||a.bank+' ESNEK HESAP'),date:d.date})}await save();modal=null;render()};
...
async function act(a,el){if(a==='togglePrivacy'){state.settings.privacy=!state.settings.privacy;await save();render();return}else if(a==='calPrev'){calendarDate=new Date(calendarDate.getFullYear(),calendarDate.getMonth()-1,1);render();return}else if(a==='calNext'){calendarDate=new Date(calendarDate.getFullYear(),calendarDate.getMonth()+1,1);render();return}else if(a==='calendarDay'){open(el.dataset.date,calendarDayDetail(el.dataset.date));return}else if(a==='addMember'){open('HANE ÜYESİ EKLE',memberForm());return}else if(a==='editMember'){const x=state.members.find(z=>z.id===el.dataset.id);open('HANE ÜYESİNİ DÜZENLE',memberForm(x),{id:x.id});return}else if(a==='delMember'){const mm=state.members.find(x=>x.id===el.dataset.id);state.members=state.members.filter(x=>x.id!==el.dataset.id);if(mm){state.incomes.forEach(x=>{if(x.member===mm.name)x.member='HANE'});state.expenses.forEach(x=>{if(x.member===mm.name)x.member='HANE'});state.cardTransactions.forEach(x=>{if(x.member===mm.name)x.member='HANE'})}await save();modal=null;render();return}else if(a==='txSearch'){const f=$('#txFilterForm');if(f){const d=Object.fromEntries(new FormData(f).entries());txSearch=(d.q||'').trim().toLocaleUpperCase('tr-TR');txCategory=d.category||'all';txSource=d.source||'all';txMember=d.member||'all';txMin=d.min||'';txMax=d.max||'';txStart=d.start||'';txEnd=d.end||'';}render();return}else if(a==='homeCustomize'){open('ANA SAYFAYI DÜZENLE',homeCustomizeForm());return}else if(a==='homeMove'){const k=el.dataset.key,dir=+el.dataset.dir||0,i=state.homeLayout.indexOf(k),j=i+dir;if(i>=0&&j>=0&&j<state.homeLayout.length){[state.homeLayout[i],state.homeLayout[j]]=[state.homeLayout[j],state.homeLayout[i]];open('ANA SAYFAYI DÜZENLE',homeCustomizeForm())}return}else if(a==='openMenu')open('MENÜ',menuBody());else if(a==='menuGo'){navigateTo(el.dataset.go)}else if(a==='summaryDetail'){txFilter=el.dataset.kind==='income'?'income':'all';navigateTo(el.dataset.kind==='expense'?'monthSpent':el.dataset.kind==='income'?'transactions':'reports')}else if(a==='financeTab'){financeTab=el.dataset.finance;render()}else if(a==='scrollCard'){document.querySelectorAll('.financeCarousel .luxuryCard')[+el.dataset.index]?.scrollIntoView({behavior:'smooth',inline:'center',block:'nearest'})}else if(a==='addCategory')open('KATEGORİ EKLE',categoryForm(),{oldCat:''});else if(a==='editCategory'){open('KATEGORİYİ DÜZENLE',categoryForm(el.dataset.cat),{oldCat:el.dataset.cat})}else if(a==='delCategory'){const c=el.dataset.cat;if(confirm(c+' kategorisi silinsin mi?')){state.customCategories=state.customCategories.filter(x=>x!==c);delete state.categoryMeta[c];state.expenses.forEach(x=>{if(x.category===c)x.category='Diğer'});normalizeV19(state);await save();modal=null;render()}}else if(a==='addAccount')open('HESAP EKLE',accountForm());else if(a==='editAccount'){const x=state.accounts.find(z=>z.id===el.dataset.id);open('HESABI DÜZENLE',accountForm(x),{id:x.id})}else if(a==='delAccount'){state.accounts=state.accounts.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='accountSpend'||a==='accountIncome'){const x=state.accounts.find(z=>z.id===el.dataset.id);open(a==='accountSpend'?'HARCAMA EKLE':'GELİR EKLE',simpleAmountForm(a,x))}else if(a==='addFlex')open('ESNEK HESAP EKLE',flexForm());else if(a==='editFlex'){const x=state.flexAccounts.find(z=>z.id===el.dataset.id);open('ESNEK HESABI DÜZENLE',flexForm(x),{id:x.id})}else if(a==='delFlex'){state.flexAccounts=state.flexAccounts.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='flexSpend'||a==='flexPay'){const x=state.flexAccounts.find(z=>z.id===el.dataset.id);open(a==='flexSpend'?'HARCAMA EKLE':'ÖDEME YAPTIM',simpleAmountForm(a,x))}else if(a==='editCardPayment'){const x=state.cardPayments.find(z=>z.id===el.dataset.id);if(x)open('ÖDEMEYİ DÜZENLE',cardPaymentEditForm(x),{id:x.id})}else if(a==='delCardPayment'){state.cardPayments=state.cardPayments.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='back'){if(current==='theme')themeDraft=null;current=navHistory.pop()||'home';render()}else if(a==='txFilter'){txFilter=el.dataset.filter||'all';render()}else if(a==='goCards'){navigateTo('cards')}else if(a==='reportPeriod'){reportPeriod=el.dataset.period||'month';render()}else if(a==='openTheme'){themeDraft={...(state.theme||{bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'})};current='theme';modal=null;render()}else if(a==='addIncome')open('Gelir Ekle',incomeForm());else if(a==='editIncome'){const x=state.incomes.find(z=>z.id===el.dataset.id);open('Geliri Düzenle',incomeForm(x),{id:x.id})}else if(a==='delIncome'){state.incomes=state.incomes.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='addExpense')open('Gider Ekle',expenseForm());else if(a==='addFixedExpense')open('Gider Ekle',expenseForm({recurring:true,category:'Kira',dueDate:iso()}));else if(a==='editFixedExpense'){const x=state.expenses.find(z=>z.id===el.dataset.id);open('Gideri Düzenle',expenseForm(x),{id:x.id})}else if(a==='editExpense'){const x=state.expenses.find(z=>z.id===el.dataset.id);open('Gideri Düzenle',expenseForm(x),{id:x.id})}else if(a==='delExpense'){const ex=state.expenses.find(x=>x.id===el.dataset.id);if(ex?.cardTxId){state.cardTransactions=state.cardTransactions.filter(t=>t.id!==ex.cardTxId);const c=state.cards.find(c=>c.id===ex.cardId);if(c)c.balance=Math.max(0,(+c.balance||0)-(+ex.amount||0))}state.expenses=state.expenses.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='toggleFixedPaid'){const x=state.expenses.find(z=>z.id===el.dataset.id);if(x){x.paidMonths=Array.isArray(x.paidMonths)?x.paidMonths:[];const m=state.selectedMonth;x.paidMonths=x.paidMonths.includes(m)?x.paidMonths.filter(v=>v!==m):[...x.paidMonths,m];x.paidDates=x.paidDates||{};state.fixedPayments=state.fixedPayments||[];if(x.paidMonths.includes(m)){x.paidDates[m]=iso();if(!state.fixedPayments.some(p=>p.expenseId===x.id&&p.month===m))state.fixedPayments.push({id:id(),expenseId:x.id,month:m,date:iso(),amount:+x.amount||0,title:x.title,category:x.category})}else{delete x.paidDates[m];state.fixedPayments=state.fixedPayments.filter(p=>!(p.expenseId===x.id&&p.month===m))}await save();render()}}else if(a==='addCard')open('Kart Ekle',cardForm());else if(a==='editCard'){const c=state.cards.find(z=>z.id===el.dataset.id);open('Kartı Düzenle',cardForm(c),{id:c.id})}else if(a==='delCard'){state.cards=state.cards.filter(x=>x.id!==el.dataset.id);await save();modal=null;render()}else if(a==='cardSpend'){const c=state.cards.find(x=>x.id===el.dataset.id);open('Kart Harcaması Ekle',cardSpendForm(c),{cardId:c.id})}else if(a==='cardPay'){const c=state.cards.find(x=>x.id===el.dataset.id);open('Ödeme Yaptım',cardPayForm(c),{cardId:c.id})}else if(a==='cashDetails'){const F=cashFlow();open('Bu Ay · Harcanan / Ödenen',`<div class="cashDetail"><div class="notice"><b>Bu Ay Harcanan: ${money(F.spent)}</b><br>Bu ay yaptığın gerçek ev harcamalarıdır.</div><div class="notice" style="margin-top:8px"><b>Bu Ay Ödenen: ${money(F.paid)}</b><br>Kart ödemeleri ${money(F.cardPaid)} + nakit/ödenmiş giderler ${money(F.directPaid)}.</div><div class="notice" style="margin-top:8px">Kart ödemeleri yeniden gider sayılmaz. Önceki aylardan gelen kart borcu ödesen bile sadece “Bu Ay Ödenen” bölümünde görünür.</div></div>`)}else if(a==='editProfile')open('Profili Düzenle',profileForm());else if(a==='pickProfile')$('#profileInput').click();else if(a==='viewReceipt'){const x=state.expenses.find(z=>z.id===el.dataset.id);if(x?.attachment)open('FİŞ / FATURA',`<img src="${x.attachment}" style="width:100%;border-radius:18px"><button class="btn" data-action="removeReceiptById" data-id="${x.id}">FİŞİ SİL</button>`)}else if(a==='removeReceipt'){if(modal)modal.attachment='';render()}else if(a==='removeReceiptById'){const x=state.expenses.find(z=>z.id===el.dataset.id);if(x){x.attachment='';await save();modal=null;render()}}else if(a==='receipt')$('#receiptInput').click();else if(a==='close'){modal=null;render()}else if(a==='pickBg')open('Arka Plan Rengi',colorForm('bg','Arka Plan Rengi'));else if(a==='pickAccent')open('Detay Rengi',colorForm('accent','Detay Rengi'));else if(a==='pickIncome')open('Grafik Gelir',colorForm('income','Gelir Rengi'));else if(a==='pickExpense')open('Grafik Gider',colorForm('expense','Gider Rengi'));else if(a==='pickRemain')open('Grafik Kalan',colorForm('remain','Kalan Rengi'));else if(a==='saveColor'){if(!themeDraft)themeDraft={...(state.theme||{})};themeDraft[el.dataset.kind]=$('#nativeColor').value;modal=null;render()}else if(a==='preset'){if(!themeDraft)themeDraft={...(state.theme||{})};themeDraft.bg=el.dataset.bg;themeDraft.accent=el.dataset.accent;render()}else if(a==='resetTheme'){themeDraft={bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'};render()}else if(a==='saveTheme'){state.theme={...themeDraft};await save();themeDraft=null;applyTheme();current='profile';render();alert('Tema kaydedildi.')}else if(a==='cancelTheme'){themeDraft=null;current='profile';render()}else if(a==='backupNow')backupNow();else if(a==='restoreNow')$('#restoreInput').click();else if(a==='changePin')changePin();else if(a==='toggleDarkMode'){state.settings.darkMode=!(state.settings.darkMode!==false);await save();applyTheme();render();showToast(state.settings.darkMode?'KARANLIK MOD AÇILDI':'KARANLIK MOD KAPATILDI')}else if(a==='clearAll'){if(confirm('Tüm HANE verileri silinsin mi?')){localStorage.removeItem(META);localStorage.removeItem(DATA);location.reload()}}}
async function saveIncome(d,i){const amount=Number(String(d.amount||'0').replace(',','.'));if(!Number.isFinite(amount)||amount<0)throw new Error('Tutar geçersiz');const x={id:i||id(),title:upper((d.title||'GELİR').trim()||'GELİR'),amount,date:d.date||iso(),recurring:d.recurring==='true',member:d.member||'HANE'};if(i){const n=state.incomes.findIndex(z=>z.id===i);state.incomes[n]={...state.incomes[n],...x}}else state.incomes.push(x);await save();modal=null;render()}
async function saveExpense(d,i){
  ...
  if(i){const n=state.expenses.findIndex(z=>z.id===i);state.expenses[n]={...state.expenses[n],...x}}else state.expenses.push(x);await save();modal=null;render()
}
...
async function saveCardSpend(d,cardId){
  ...
  const tx={id:txId,cardId,amount,title:upper((d.title||d.category||'KART HARCAMASI').trim()),category:d.category||'Diğer',date,statementMonth:stmt,attachment:modal?.attachment||'',member:d.member||'HANE',payType:d.payType||'Tek Çekim',installmentCount:Math.max(1,+d.installmentCount||1)};
  state.cardTransactions=state.cardTransactions||[];state.cardTransactions.push(tx);if(tx.payType==='Taksitli'&&tx.installmentCount>1){state.installments=state.installments||[];state.installments.push({id:id(),txId,cardId,total:amount,count:tx.installmentCount,paid:0,startDate:date,title:tx.title})}
  const expenseAmount=(tx.payType==='Taksitli'&&tx.installmentCount>1)?amount/tx.installmentCount:amount;state.expenses.push({id:id(),cardTxId:txId,source:'card',cardId,title:tx.title,amount:expenseAmount,originalAmount:amount,category:tx.category,date,dueDate:date,recurring:false,paid:true,member:tx.member,payType:tx.payType,installmentCount:tx.installmentCount,installmentNo:1});if(tx.payType==='Taksitli'&&tx.installmentCount>1){for(let n=2;n<=tx.installmentCount;n++){const dd=new Date(date+'T12:00:00');dd.setMonth(dd.getMonth()+n-1);state.expenses.push({id:id(),cardTxId:txId,source:'card',cardId,title:tx.title+' · '+n+'/'+tx.installmentCount+' TAKSİT',amount:expenseAmount,originalAmount:amount,category:tx.category,date:iso(dd),dueDate:iso(dd),recurring:false,paid:false,member:tx.member,payType:'Taksitli',installmentCount:tx.installmentCount,installmentNo:n})}}
...
async function saveCardPayment(d,cardId){
  ...
  state.cardPayments.push({id:id(),cardId,amount,date:d.date||iso(),title:c.bank+' '+c.name+' ödeme'});let rem=amount;state.expenses.filter(x=>x.cardId===cardId&&x.payType==='Taksitli'&&!x.paid).sort((a,b)=>a.date.localeCompare(b.date)).forEach(x=>{if(rem+0.001>=(+x.amount||0)){x.paid=true;x.paidDate=d.date||iso();rem-=+x.amount||0}});(state.installments||[]).filter(x=>x.cardId===cardId).forEach(plan=>{const parts=state.expenses.filter(e=>e.cardTxId===plan.txId&&e.payType==='Taksitli');plan.paid=parts.filter(e=>e.paid).length});
...
function displayMoney(v){return state?.settings?.privacy?'••••••':money(v)}
function dueClass(date){const d=Math.ceil((new Date(date+'T12:00:00')-new Date())/86400000);return d<0?'GECİKMİŞ':d===0?'BUGÜN':d===1?'YARIN':d<=7?'BU HAFTA':'YAKLAŞIYOR'}
function alerts(){let a=[];state.expenses.filter(x=>x.recurring&&!fixedPaidForMonth(x)).forEach(x=>a.push({title:x.title,amount:x.amount,date:x.dueDate||x.date,kind:'SABİT GİDER',action:'editFixedExpense',id:x.id}));state.cards.filter(x=>(+x.balance||0)>0).forEach(c=>{const p=cardPaymentInfo(c);a.push({title:c.bank+' '+c.name,amount:c.balance,date:iso(p.dueDate),kind:'KART',action:'editCard',id:c.id})});state.flexAccounts.filter(x=>(+x.balance||0)>0).forEach(x=>a.push({title:x.bank+' '+x.name,amount:x.balance,date:x.dueDate||iso(),kind:'ESNEK HESAP',action:'editFlex',id:x.id}));a.sort((x,y)=>x.date.localeCompare(y.date));return `<div class="section"><b>YAKLAŞAN ÖDEMELER</b><span>${a.length} KAYIT</span></div><div class="list">${a.length?a.map(x=>`<div class="item dueItem" data-action="${x.action}" data-id="${x.id}"><div class="ico">◷</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x.kind}</small><em class="dueBadge">${dueClass(x.date)}</em></div><div class="right"><b>${displayMoney(x.amount)}</b></div></div>`).join(''):'<div class="notice">YAKLAŞAN ÖDEME YOK.</div>'}</div>`}
function calendar(){const y=calendarDate.getFullYear(),m=calendarDate.getMonth(),first=new Date(y,m,1),last=new Date(y,m+1,0),prefix=(first.getDay()+6)%7,ymv=`${y}-${String(m+1).padStart(2,'0')}`;let cells='';for(let i=0;i<prefix;i++)cells+='<div></div>';for(let d=1;d<=last.getDate();d++){const ds=`${ymv}-${String(d).padStart(2,'0')}`,inc=state.incomes.filter(x=>x.date===ds),exp=state.expenses.filter(x=>x.date===ds),paid=state.expenses.filter(x=>x.recurring&&fixedPaidForMonth(x,ymv)&&String(x.dueDate||x.date).slice(8,10)==String(d).padStart(2,'0'));cells+=`<button class="calDay" data-action="calendarDay" data-date="${ds}"><b>${d}</b><span>${inc.length?'<i class="ci"></i>':''}${exp.length?'<i class="ce"></i>':''}${paid.length?'<i class="cp"></i>':''}</span></button>`}return `<div class="calHead"><button data-action="calPrev">‹</button><b>${new Date(y,m,1).toLocaleDateString('tr-TR',{month:'long',year:'numeric'}).toLocaleUpperCase('tr-TR')}</b><button data-action="calNext">›</button></div><div class="calWeek"><b>PZT</b><b>SAL</b><b>ÇAR</b><b>PER</b><b>CUM</b><b>CMT</b><b>PAZ</b></div><div class="calendarGrid">${cells}</div><div class="calLegend"><span><i class="ci"></i> GELİR</span><span><i class="ce"></i> GİDER</span><span><i class="cp"></i> ÖDENDİ</span></div>`}
function calendarDayDetail(ds){const inc=state.incomes.filter(x=>x.date===ds),exp=state.expenses.filter(x=>x.date===ds),paid=exp.filter(x=>x.recurring&&fixedPaidForMonth(x,ds.slice(0,7))),ti=inc.reduce((a,x)=>a+(+x.amount||0),0),te=exp.reduce((a,x)=>a+(+x.amount||0),0);return `<div class="detailMiniGrid"><div class="detailMini"><span>GELEN</span><b>${displayMoney(ti)}</b></div><div class="detailMini"><span>GİDEN</span><b>${displayMoney(te)}</b></div><div class="detailMini"><span>ÖDENEN</span><b>${displayMoney(paid.reduce((a,x)=>a+(+x.amount||0),0))}</b></div><div class="detailMini"><span>NET</span><b>${displayMoney(ti-te)}</b></div></div><div class="list">${[...inc.map(x=>({...x,k:'i'})),...exp.map(x=>({...x,k:'e'}))].map(x=>`<div class="item" data-action="${x.k==='i'?'editIncome':'editExpense'}" data-id="${x.id}"><div class="ico">${x.k==='i'?'＋':catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.category||'GELİR'}</small></div><div class="right"><b>${displayMoney(x.amount)}</b></div></div>`).join('')||'<div class="notice">BU GÜN HAREKET YOK.</div>'}</div>`}
function members(){return `<div class="section"><b>HANE ÜYELERİ</b><button class="miniAddBtn" data-action="addMember">+ ÜYE EKLE</button></div><div class="list">${(state.members||[]).map(x=>`<div class="item"><div class="ico">${esc(x.icon||'●')}</div><div><b>${esc(x.name)}</b><small>HANE ÜYESİ</small></div><div class="right"><button class="iconEdit" data-action="editMember" data-id="${x.id}">✎</button></div></div>`).join('')||'<div class="notice">HENÜZ HANE ÜYESİ EKLENMEDİ.</div>'}</div>`}
function memberForm(x={}){return `<form class="form" id="memberForm">${input('name','Üye Adı',x.name||'')}${input('icon','İkon',x.icon||'●')}<button class="btn gold" type="submit">KAYDET</button>${x.id?`<button type="button" class="btn" data-action="delMember" data-id="${x.id}">SİL</button>`:''}</form>`}
function searchPanel(){return `<div class="searchPanel"><input id="txSearchBox" placeholder="Hareket ara..." value="${esc(txSearch)}"><button data-action="txSearch">ARA</button></div>`}
const _transactionsV20=transactions;transactions=function(){let html=_transactionsV20();return searchPanel()+html.replace(/<div class="item"/g,'<div class="item searchableTx"')}
const _flexPanelV20=flexPanel;flexPanel=function(){return _flexPanelV20().replace(/flexAccount luxuryCard/g,'flexAccount luxuryCard verticalFlexV20').replace(/<small>\$\{esc\(a.name\)\}<\/small>/g,'<small>${esc(a.name)}</small>')}


function navigateTo(next,replace=false){
  if(!next||next===current){modal=null;render();return}
  navHistory.push(current);current=next;modal=null;
  try{const st={hane:true,view:next};replace?history.replaceState(st,''):history.pushState(st,'')}catch{}
  render();
}
window.addEventListener('popstate',e=>{if(!state)return;const v=e.state?.view;if(v&&v!==current){current=v;modal=null;render();return}current=navHistory.pop()||'home';modal=null;render()});
function homeCustomizeForm(){
 const labels={summary:'ÖZET / GELİR-GİDER-KALAN',quick:'HIZLI İŞLEMLER',month:'BU AY',recommended:'ÖNERİLEN KART'};
 return `<form class="form" id="homeCustomizeForm"><div class="notice">Bölümleri göster/gizle. Sıralamayı yukarı/aşağı düğmeleriyle değiştir.</div>${state.homeLayout.map((k,i)=>`<div class="homeCfg"><label><input type="checkbox" name="show_${k}" ${state.homeHidden.includes(k)?'':'checked'}> ${labels[k]}</label><div><button type="button" data-action="homeMove" data-key="${k}" data-dir="-1">↑</button><button type="button" data-action="homeMove" data-key="${k}" data-dir="1">↓</button></div></div>`).join('')}<button class="btn gold" type="submit">KAYDET</button></form>`
}
function applyHomeLayout(){if(current!=='home'||!state)return;const c=$('.content');if(!c)return;c.style.display='flex';c.style.flexDirection='column';const groups={summary:[$('.summary'),$('.donutBox')],quick:[$('.premiumQuick')?.previousElementSibling,$('.premiumQuick')],month:[$('.monthActionGrid')?.previousElementSibling,$('.monthActionGrid')],recommended:[$('.reco')?.previousElementSibling,$('.reco')]};state.homeLayout.forEach((k,i)=>(groups[k]||[]).forEach(n=>{if(n){n.style.order=String(10+i);n.style.display=state.homeHidden.includes(k)?'none':''}}));const hero=$('.homeHero'),quote=$('.quote');if(hero)hero.style.order='0';if(quote)quote.style.order='1'}
function searchPanel(){const cats=['all',...C],members=['all','HANE',...(state.members||[]).map(x=>x.name)];return `<form class="searchPanel advancedSearch" id="txFilterForm" onsubmit="return false"><input name="q" id="txSearchBox" placeholder="Hareket ara..." value="${esc(txSearch)}"><div class="filterGrid"><select name="category">${cats.map(x=>`<option value="${esc(x)}" ${txCategory===x?'selected':''}>${x==='all'?'TÜM KATEGORİLER':esc(x)}</option>`).join('')}</select><select name="source"><option value="all">TÜM ÖDEME TÜRLERİ</option><option value="cash" ${txSource==='cash'?'selected':''}>NAKİT</option><option value="card" ${txSource==='card'?'selected':''}>KART</option></select><select name="member">${members.map(x=>`<option value="${esc(x)}" ${txMember===x?'selected':''}>${x==='all'?'TÜM ÜYELER':esc(x)}</option>`).join('')}</select><input name="start" type="date" value="${esc(txStart)}"><input name="end" type="date" value="${esc(txEnd)}"><input name="min" type="number" placeholder="Min ₺" value="${esc(txMin)}"><input name="max" type="number" placeholder="Max ₺" value="${esc(txMax)}"></div><button type="button" data-action="txSearch">FİLTRELE</button></form>`}
function transactions(){let a=[...state.incomes.map(x=>({...x,type:'income',source:x.source||'cash'})),...state.expenses.map(x=>({...x,type:'expense'})),...(state.cardPayments||[]).map(x=>({...x,type:'cardPayment',source:'card',member:'HANE'}))];if(txFilter==='income')a=a.filter(x=>x.type==='income');else if(txFilter==='expense')a=a.filter(x=>x.type==='expense'&&x.source!=='card');else if(txFilter==='card')a=a.filter(x=>x.type==='cardPayment'||x.source==='card');if(txSearch)a=a.filter(x=>`${x.title||''} ${x.category||''} ${x.member||''}`.toLocaleUpperCase('tr-TR').includes(txSearch));if(txCategory!=='all')a=a.filter(x=>x.category===txCategory);if(txSource!=='all')a=a.filter(x=>x.source===txSource);if(txMember!=='all')a=a.filter(x=>(x.member||'HANE')===txMember);if(txStart)a=a.filter(x=>x.date>=txStart);if(txEnd)a=a.filter(x=>x.date<=txEnd);if(txMin!=='')a=a.filter(x=>(+x.amount||0)>=+txMin);if(txMax!=='')a=a.filter(x=>(+x.amount||0)<=+txMax);if(!txStart&&!txEnd)a=a.filter(x=>String(x.date||'').startsWith(state.selectedMonth));a.sort((a,b)=>String(b.date).localeCompare(String(a.date)));const B=(k,l)=>`<button data-action="txFilter" data-filter="${k}" class="${txFilter===k?'active':''}">${l}</button>`;return searchPanel()+`<div class="seg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Kart')}</div><div class="section"><b>HAREKETLER</b><span>${a.length} KAYIT</span></div><div class="list">${a.length?a.map(row).join(''):'<div class="notice">Bu filtrede hareket yok.</div>'}</div>`}
...
function renderLock(){if(!meta()){renderSetup();return}pin='';const m=meta()||{};$('#app').innerHTML=`<div class="lock haneApprovedLock"><div class="approvedLockLogo"><img src="./icons/hane-v5-logo.jpg" alt="HANE"></div><div class="approvedLockMotto">EVİNİ PLANLA · HAYATINI KOLAYLAŞTIR <span>PLANLA · KONTROL ET · RAHAT YAŞA</span></div><div class="approvedLockCore"><div class="lockAvatar ava approvedProfile">${m.photo?`<img src="${m.photo}" alt="Profil">`:`${esc((m.name||'H')[0])}`}</div><div class="approvedHello">HANE’YE HOŞ GELDİN</div><h2>${esc(m.name||'HANE')}</h2><div class="approvedTagline">AİLE · DÜZEN · HUZUR</div><div class="pinDots">${[0,1,2,3].map(i=>`<i data-dot="${i}"></i>`).join('')}</div><div class="keypad approvedKeypad">${[1,2,3,4,5,6,7,8,9].map(n=>`<button type="button" data-key="${n}">${n}</button>`).join('')}<button type="button" class="blankKey" disabled>◆</button><button type="button" data-key="0">0</button><button type="button" class="del" data-key="del">⌫</button></div><button type="button" class="lockOk approvedLockOk" id="lockOk">✓ GİRİŞ</button><div class="approvedLockFoot"><b>HANE</b> · DAHA DÜZENLİ BİR YAŞAM</div></div></div>`;$$('[data-key]').forEach(b=>b.onclick=()=>pinKey(b.dataset.key));$('#lockOk').onclick=confirmPin}
...
function render(){applyTheme();$('#app').innerHTML=`<main class="phone">${buildTopBar()}<div class="content">${view()}</div>${nav()}</main>${modalWrap()}`;bind();applyHomeLayout();if(current==='reports')requestAnimationFrame(drawChart)}
...
function bootHane(){
  try{
    ...
    try{history.replaceState({hane:true,view:'home'},'')}catch{};renderLock();
  }catch(err){
