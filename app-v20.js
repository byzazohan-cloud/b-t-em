...
function normalizeV19(st){
  ...
  st.accounts=Array.isArray(st.accounts)?st.accounts:[];st.flexAccounts=Array.isArray(st.flexAccounts)?st.flexAccounts:[];st.categoryMeta=st.categoryMeta||{};st.customCategories=Array.isArray(st.customCategories)?st.customCategories:[];st.fixedPayments=Array.isArray(st.fixedPayments)?st.fixedPayments:[];
...
function buildTopBar(){
  ...
  const t={transactions:'HAREKETLER',fixed:'GİDERLER',cards:'FİNANS',reports:'RAPORLAR',profile:'PROFİL',backup:'YEDEKLEME',settings:'AYARLAR',categories:'KATEGORİLER',theme:'TEMA STÜDYOSU',alerts:'HATIRLATMALAR',about:'HAKKINDA',monthSpent:'BU AY HARCANAN',monthPaid:'BU AY ÖDENEN',calendar:'TAKVİM'};
...
function menuBody(){const a=[['home','🏠','ANA EKRAN'],['transactions','📋','HAREKETLER'],['fixed','🧾','GİDERLER'],['cards','💳','KARTLAR / ESNEK HESAP'],['reports','📊','RAPORLAR'],['calendar','▦','TAKVİM'],['alerts','🔔','HATIRLATMALAR'],['settings','⚙️','AYARLAR'],['profile','👤','PROFİL']];return `<div class="menuList">${a.map(x=>`<button data-action="menuGo" data-go="${x[0]}"><i>${x[1]}</i><b>${x[2]}</b><span>›</span></button>`).join('')}</div>`}
...
function flexPanel(){return `<div class="section"><b>ESNEK HESAPLAR</b><button class="miniAddBtn" data-action="addFlex">+ ESNEK HESAP</button></div><div class="financeCarousel">${state.flexAccounts.map(a=>`<div class="flexAccount luxuryCard verticalFlexV20"><div class="cardTop"><b>${esc(a.bank)}</b><span>ESNEK</span></div><small>${esc(a.name)}</small><div class="grid"><div><small>LİMİT</small><b>${money(a.limit)}</b></div><div><small>KULLANILAN</small><b>${money(a.balance)}</b></div><div><small>KULLANILABİLİR</small><b>${money(Math.max(0,(+a.limit||0)-(+a.balance||0)))}</b></div><div><small>HESAP KESİM</small><b>${esc(a.statementDate||'-')}</b></div><div><small>SON ÖDEME</small><b>${esc(a.dueDate||'-')}</b></div></div><div class="cardActions"><button class="btn" data-action="flexSpend" data-id="${a.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="flexPay" data-id="${a.id}">₺ ÖDEME YAPTIM</button><button class="iconEdit" data-action="editFlex" data-id="${a.id}">✎</button></div></div>`).join('')||'<div class="notice">HENÜZ ESNEK HESAP EKLENMEDİ.</div>'}</div>`}
...
function reports(){const T=periodTotals(),inside=x=>x.date>=T.range.start&&x.date<=T.range.end,B=(k,l)=>`<button data-action="reportPeriod" data-period="${k}" class="${reportPeriod===k?'active':''}">${l}</button>`,items=[...state.incomes.filter(inside).map(x=>({...x,_kind:'income'})),...state.expenses.filter(inside).map(x=>({...x,_kind:'expense'}))].sort((a,b)=>String(b.date).localeCompare(String(a.date)));return`<div class="seg">${B('day','Gün')}${B('week','Hafta')}${B('month','Ay')}${B('year','Yıl')}${B('custom','Özel')}</div>${reportPeriod==='custom'?`<form id="customRangeForm" class="customRange"><label>Başlangıç<input type="date" name="start" value="${customStart}"></label><label>Bitiş<input type="date" name="end" value="${customEnd}"></label><button class="btn gold">UYGULA</button></form>`:''}<div class="section"><b>${T.range.label}</b><span></span></div><div class="summary"><div class="sum"><label>Toplam Gelir</label><strong style="color:var(--green)">${money(T.i)}</strong></div><div class="sum"><label>Toplam Gider</label><strong style="color:var(--red)">${money(T.e)}</strong></div><div class="sum"><label>Kalan</label><strong>${money(T.r)}</strong></div></div><div class="card" style="padding:14px;margin-top:10px"><canvas id="chart" style="width:100%;height:220px"></canvas><div class="chartLegend"><span><i style="background:var(--gi)"></i>GELİR</span><span><i style="background:var(--ge)"></i>GİDER</span><span><i style="background:var(--gr)"></i>KALAN</span></div></div><div class="section"><b>GRAFİĞE AİT HAREKETLER</b><span>${items.length} KAYIT</span></div><div class="list reportMoves">${items.length?items.map(x=>`<div class="item"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x._kind==='income'?'GELİR':esc(x.category||'DİĞER')}${x._kind==='expense'?' · '+paymentLabel(x):''}</small></div><div class="reportMoveRight"><b style="color:${x._kind==='income'?'var(--green)':'var(--red)'}">${x._kind==='income'?'+':'-'}${money(x.amount)}</b><div><button type="button" data-action="${x._kind==='income'?'editIncome':'editExpense'}" data-id="${x.id}">DÜZENLE</button><button type="button" class="danger" data-action="${x._kind==='income'?'delIncome':'delExpense'}" data-id="${x.id}">SİL</button></div></div></div>`).join(''):'<div class="notice">BU GRAFİK ARALIĞINDA HAREKET YOK.</div>'}</div>`}
...
function bind(){
  ...
  const customRangeFormEl=$('#customRangeForm');if(customRangeFormEl)customRangeFormEl.onsubmit=e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries());if(d.start&&d.end&&d.start<=d.end){customStart=d.start;customEnd=d.end;reportPeriod='custom';render()}else alert('Geçerli tarih aralığı seçin.');};
...
function displayMoney(v){return money(v)}
