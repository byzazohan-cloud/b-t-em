...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.8 · G5 HAREKET AKIŞI</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
...
function bind(){
  ...
  if(fixedExpenseFormEl) fixedExpenseFormEl.onsubmit=async e=>{
    ...
    try{const wasEdit=!!modal?.id;await saveFixedExpense(Object.fromEntries(new FormData(form).entries()),modal?.id);showToast(wasEdit?'GÜNCELLENDİ':'KAYDEDİLDİ')}
...
function render(){if(state)initBrowserNav();applyTheme();$('#app').innerHTML=`<main class="phone">${buildTopBar()}<div class="content">${view()}</div>${nav()}</main>${modalWrap()}`;bind();if(returnScrollTop!=null){const y=returnScrollTop;returnScrollTop=null;requestAnimationFrame(()=>{const c=document.querySelector('.content');if(c)c.scrollTop=y})}if(current==='reports')requestAnimationFrame(drawChart)}
