...
function transactions(){
  ...
  return `<div class="movementsPage">${monthNavigator()}<div class="liveTxSearch premiumMovementSearch"><span>⌕</span><input id="txSearch" autocomplete="off" placeholder="Hareketlerde ara..." value="${esc(txSearch)}">${q?'<button type="button" data-action="clearTxSearch">×</button>':''}</div><div class="seg movementSeg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Ödeme')}</div><div class="movementMeta"><b>${q?'ARAMA SONUÇLARI':monthLabel(m)}</b><span>${a.length} KAYIT</span></div>${a.length?rows:'<div class="notice movementEmpty">Eşleşen hareket bulunamadı.</div>'}</div>`
}
