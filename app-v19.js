...
function categoryIconSvg(cat,size=30){
  ...
  if(fixed3d[k])return `<img class="cat3dAsset" src="icons/categories3d/${fixed3d[k]}.png?v=V121" width="${size}" height="${size}" alt="" aria-hidden="true">`;
...
function nav(){const items=[['home','home','ANA SAYFA'],['transactions','tasks','HAREKETLER'],['fixed','wallet','GİDERLER'],['cards','wallet','FİNANS'],['calendar','calendar','TAKVİM'],['reports','report','RAPORLAR']];return`<nav class="nav premiumNav v1947CleanNav v123Menu3d">${items.map(x=>`<button data-tab="${x[0]}" class="${current===x[0]?'active':''}"><span class="navIcon"><img class="menu3dIcon" src="icons/menu3d/${x[1]}.png?v=V123" alt=""></span><span>${x[2]}</span></button>`).join('')}</nav>`}
...
function expenseForm(x={}){
  ...
  const normal3dFile=c=>({'Market':'market','Manav':'market','Fırın':'market','Kafe':'restoran','Yemek':'restoran','Restoran':'restoran','Online Alışveriş':'alisveris','Kuyumculuk':'alisveris','Ulaşım':'ulasim','Otopark':'ulasim','Otoyol/Köprü':'ulasim','Araç Bakım':'ulasim','Akaryakıt':'akaryakit','Sağlık':'saglik','Eğitim':'egitim','Kırtasiye':'egitim','Giyim':'giyim','Kozmetik':'kisisel-bakim','Kişisel Bakım':'kisisel-bakim','Spor':'spor','Oyun':'eglence','Eğlence':'eglence','Tatil':'seyahat','Konaklama':'seyahat','Uçak':'seyahat','Hediye':'hediye','Evcil Hayvan':'evcil-hayvan','Ev Bakım':'ev-bakim','Temizlik':'ev-bakim','Ev':'ev-esyasi','Mobilya':'ev-esyasi','Çocuk':'bebek','Kitap':'kitap','Elektronik':'teknoloji','Diğer':'diger'}[c]||'');
  ...
  const catTiles=(cats,name)=>`<div class="premiumCatGrid ${name==='fixedCategory'?'fixed3dDirectGrid':''}" data-select-name="${name}">${cats.map(c=>{const ff=fixed3dFile(c),nf=normal3dFile(c),f=ff||nf;return `<button type="button" class="premiumCatBtn ${f?'fixed3dDirectBtn':''} ${c===currentCat?'active':''}" data-cat="${esc(c)}" style="--cat:${catColor(c)}">${f?`<img class="fixed3dDirectImg" src="${ff?'icons/categories3d/':'icons/normal3d/'}${f}.png?v=V123" alt="${esc(c)}">`:catPremiumIcon(c)}<b class="${f?'fixed3dDirectLabel':''}">${esc(c)}</b></button>`}).join('')}</div><select class="hiddenCatSelect" name="${name}">${cats.map(c=>`<option ${c===currentCat?'selected':''}>${esc(c)}</option>`).join('')}</select>`;
...
const HANE_SW_BUILD='19.4.8.20260921-V123-NORMAL-MENU-3D';
