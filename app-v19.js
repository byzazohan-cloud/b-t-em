...
function monthPaid(){
  ...
  const categoryGroups=[...items.reduce((m,x)=>{const k=x.kind==='card'?'Kredi Kartı Ödemesi':x.kind==='flex'?'Esnek Hesap Ödemesi':expenseCategoryGroup(x);m.set(k,(m.get(k)||0)+(+x.amount||0));return m},new Map()).entries()].sort((a,b)=>b[1]-a[1]);
  const categoryHtml=categoryGroups.length?`<div class="monthlyCategoryGrid">${categoryGroups.map(([cat,amt])=>`<div class="monthlyCategoryCard"><span>${esc(cat)}</span><b>${money(amt)}</b><small>${total?Math.round(amt/total*100):0}%</small></div>`).join('')}</div>`:'';
