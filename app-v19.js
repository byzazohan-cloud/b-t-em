...
// V114: Tek kategori motoru. Görünen ad değişse bile rapor/ekstre tarafında kararlı kimlik üretir.
const CATEGORY_ID_ALIASES={'Toplu Taşıma':'Ulaşım'};
function categoryCanonical(cat){const v=String(cat||'').trim();return CATEGORY_ID_ALIASES[v]||v||'Diğer'}
...
function category3dAsset(cat){
  ...
  if(fixed[k])return `icons/fixed3d_v124/${fixed[k]}.png?v=V133`;
  const normal={
  ...
  return `icons/normal3d_v124/${normal[k]||'diger'}.png?v=V133`;
}
function categoryIconSvg(cat,size=30){
 ...
 if(fixed3d[c])return `<img class="fixed3dDirectImg" src="icons/fixed3d_v124/${fixed3d[c]}.png?v=V133" alt="" width="${size}" height="${size}" aria-hidden="true">`;
 const a=normal3d[c]||normal3d[cat]||normal3d['Diğer'];
 return `<span class="normal3dSpriteV133" style="--ix:${a[1]};--iy:${a[2]};width:${size}px;height:${size}px" aria-hidden="true"></span>`;
}
...
function haneLogo(size=54,cls=''){
  ...
  return `<img class="haneLogoImg ${cls}" src="icons/hane-main-logo-v126.png?v=V133" width="${n}" alt="HANE">`;
}
function haneFullLogo(cls=''){
  return `<img class="haneFullLogo ${cls}" src="icons/hane-main-logo-v126.png?v=V133" alt="HANE">`;
}
function premiumIcon(name,size=26){
  ...
  return `<img class="system3dIconV127" src="icons/system3d_v127/${k}.png?v=V133" alt="" style="width:${size}px;height:${size}px" aria-hidden="true">`;
}
function normalizeV19(st){
  ...
  st.expenses.forEach(x=>{x.category=categoryCanonical(x.category)});st.fixedPayments.forEach(x=>{x.category=categoryCanonical(x.category)});st.cardTransactions.forEach(x=>{x.category=categoryCanonical(x.category)});st.installments.forEach(x=>{x.category=categoryCanonical(x.category)});Object.keys(st.statementCategoryRules||{}).forEach(k=>st.statementCategoryRules[k]=categoryCanonical(st.statementCategoryRules[k]));
...
function monthPaid(){
  ...
  return `<div class="monthlyLedgerPage">${monthNavigator()}<div class="monthlyLedgerHead"><div><small>${monthLabel(m)}</small><b>AYLIK HESAP</b></div><strong>${money(total)}</strong><span>TOPLAM ÖDENEN</span></div><div class="monthlyLedgerList">${items.length?items.map(x=>`<button type="button" class="monthlyLedgerRow" data-action="${edit(x)}" data-direct-edit="1" data-id="${x.id}"><span class="monthlyLedgerDate"><b>${day(x)}</b><small>${new Date(String(x.date||'')+'T12:00:00').toLocaleDateString('tr-TR',{month:'short'}).toLocaleUpperCase('tr-TR')}</small></span><span class="monthlyLedgerIcon">${icon(x)}</span><span class="monthlyLedgerInfo"><b>${esc(x.displayTitle||x.title||'ÖDEME')}</b><small>${esc(x.displayMeta||x.category||'ÖDEME')}</small></span><strong>${money(x.amount)}</strong></button>`).join(''):'<div class="monthlyLedgerEmpty">BU AY ÖDEME KAYDI YOK.</div>'}</div>${items.length?`<div class="monthlyLedgerTotal"><span>AY TOPLAMI<small>${items.length} ÖDEME</small></span><strong>${money(total)}</strong></div>`:''}</div>`
}
...
function home(){const T=totals();return`<button type="button" class="homeHero v126ProfileWelcome" data-tab="profile" aria-label="Kimlik Kartım">
  <div class="homeHeroAvatar ava">${state.profile.photo?`<img src="${state.profile.photo}" alt="Profil">`:esc((state.profile.name||'H')[0])}</div>
  <div class="homeHeroText"><small>MERHABA</small><h2>${esc(state.profile.name||'HANE')}</h2><div class="date">${new Date().toLocaleDateString('tr-TR',{day:'numeric',month:'long',year:'numeric',weekday:'long'})}</div></div><span class="v126ProfileChevron">›</span>
</button>
...
function fixed(){
  ...
  const tabs=`<div class="expenseTabs"><button data-action="expenseView" data-view="normal" class="${expenseView==='normal'?'active':''}">NORMAL</button><button data-action="expenseView" data-view="fixed" class="${expenseView==='fixed'?'active':''}">SABİT</button></div>`;
...
function reports(){
 ...
 const monthExpenses=actualExpenseEntriesForMonth(m),cats={};monthExpenses.forEach(x=>{const k=x.category||'Diğer';cats[k]=(cats[k]||0)+(+x.amount||0)});const catRows=Object.entries(cats).sort((a,b)=>b[1]-a[1]),catTotal=catRows.reduce((a,x)=>a+x[1],0)||1;
...
function settings(){return`<div class="card"><div class="setting" data-action="togglePrivacy"><span class="settingIco">${premiumIcon("lock",30)}</span><span>GİZLİLİK MODU</span><b>${state.settings.privacy?'Açık':'Kapalı'}</b></div><div class="setting" data-tab="homeEdit"><span class="settingIco">${premiumIcon("home",30)}</span><span>ANA SAYFAYI DÜZENLE</span><b>›</b></div><div class="setting" data-tab="members"><span class="settingIco">${premiumIcon("members",30)}</span><span>HANE ÜYELERİ</span><b>›</b></div><div class="setting" data-tab="categories"><span class="settingIco">${premiumIcon("category",30)}</span><span>KATEGORİLERİ DÜZENLE / EKLE</span><b>›</b></div><div class="setting" data-action="toggleDarkMode"><span class="settingIco">${premiumIcon("palette",30)}</span><span>KARANLIK MOD</span><b>${state.settings.darkMode!==false?'Açık':'Kapalı'}</b></div><div class="setting" data-tab="about"><span class="settingIco">${premiumIcon("info",30)}</span><span>HAKKINDA</span><b>›</b></div><div class="setting" data-action="clearAll"><span class="settingIco dangerIco">${premiumIcon("trash",30)}</span><span style="color:var(--red)">TÜM VERİLERİ SIFIRLA</span><b></b></div></div>`}
function notes(){
...
function categoryForm(old=''){const color=old?catColor(old):'#58c7ff';return `<form class="form" id="categoryForm">${input('name','Kategori Adı',old)}<div class="field"><label>HANE İkonu</label><div class="categoryIconPreview">${catPremiumIcon(old||'Diğer')}<small>İkonlar HANE'nin merkezi 3D ikon setinden otomatik atanır.</small></div></div><div class="field"><label>İkon Rengi</label><input type="color" name="color" value="${color}"></div><button class="btn gold" type="submit">KAYDET</button>${old&&state.customCategories.includes(old)?`<button class="btn" type="button" data-action="delCategory" data-cat="${esc(old)}">KATEGORİYİ SİL</button>`:''}</form>`}
function theme(){
...
function alerts(){const all=upcomingPayments(),groups=['GECİKMİŞ','BUGÜN','YARIN','BU HAFTA','YAKLAŞIYOR','ÖDENDİ'];return `<div class="section"><b>YAKLAŞAN ÖDEMELER</b><span>${all.filter(x=>!x.paid).length} BEKLEYEN</span></div>${groups.map(g=>{const a=all.filter(x=>paymentBucket(x)===g);if(!a.length)return'';return `<div class="paymentGroup"><div class="section"><b>${g}</b><span>${a.length}</span></div>${a.map(x=>`<div class="item paymentDue ${x.paid?'isPaid':''}"><div class="ico premiumIco">${x.kind==='fixed'?premiumIcon('fixed',22):x.kind==='card'?premiumIcon('cards',22):premiumIcon('finance',22)}</div><div><b>${esc(x.title)}</b><small>${prettyDate(x.date)} · ${x.kind==='fixed'?'SABİT GİDER':x.kind==='card'?'KREDİ KARTI':'ESNEK HESAP'}</small></div><div class="reportMoveRight"><b>${money(x.amount)}</b><small>${g}</small></div></div>`).join('')}</div>`}).join('')||'<div class="notice">YAKLAŞAN ÖDEME YOK.</div>'}`}

function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.8 · V129 AUDIT FIX</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
...
function members(){
  ...
  const cards=virtual.map(mem=>{const rows=memberExpenseRows(mem.id,m),total=rows.reduce((a,x)=>a+(+x.amount||0),0),cash=rows.filter(x=>x.source!=='card').reduce((a,x)=>a+(+x.amount||0),0),card=rows.filter(x=>x.source==='card').reduce((a,x)=>a+(+x.amount||0),0);return `<button class="memberSummaryCard" data-action="memberDetail" data-id="${mem.id}"><span class="memberAvatar">${mem.icon&&mem.icon!=='👤'?esc(mem.icon):premiumIcon('profile',26)}</span><span class="memberSummaryText"><b>${esc(mem.name)}</b><small>${rows.length} hareket · Nakit ${money(cash)} · Kart ${money(card)}</small></span><strong>${money(total)}</strong><em>›</em></button>`}).join('');
...
function view(){return({home,transactions,fixed,cards,calendar,reports,profile,backup,settings,categories,theme,alerts,about,monthSpent,monthPaid,members,homeEdit,notes}[current]||home)()}
...
function memberSelect(v=''){return `<div class="field"><label>Hane Üyesi <small style="opacity:.65">(isteğe bağlı)</small></label><select name="memberId"><option value="" ${!v?'selected':''}>HANE GENELİ / ATANMAMIŞ</option>${state.members.map(m=>`<option value="${m.id}" ${v===m.id?'selected':''}>${m.icon&&m.icon!=='👤'?esc(m.icon)+' ':''}${esc(m.name)}</option>`).join('')}</select></div>`}
function incomeForm(x={}){return`<form class="form" id="incomeForm">${input('title','Gelir Adı',x.title||'')}${input('amount','Tutar',x.amount||0,'number','step="0.01" min="0"')}${input('date','Tarih',x.date||iso(),'date')}${memberSelect(x.memberId||'me')}<div class="field"><label>Sabit Gelir</label><select name="recurring"><option value="false" ${!x.recurring?'selected':''}>Hayır</option><option value="true" ${x.recurring?'selected':''}>Evet</option></select></div><button class="btn gold" type="submit">Kaydet</button>${x.id?`<button type="button" class="btn" data-action="delIncome" data-id="${x.id}">Sil</button>`:''}</form>`}
...
function cardForm(c={}){
  ...
  return`<form class="form" id="cardForm">
    ...
    <div class="row2">${input('limit','Limit',c.limit||0,'number','step="0.01" min="0"')}${input('balance','Güncel Borç',c.balance||0,'number','step="0.01" min="0"')}</div>
...
function bind(){
  ...
  const financeCarousel=document.querySelector('.financeCardStage .financeCarousel');
  if(financeCarousel&&state?.cards?.length){
    let cardScrollRaf=0;
    const syncFinanceSelection=()=>{
      cardScrollRaf=0;
      const cards=[...financeCarousel.querySelectorAll('.haneVerticalCard')];if(!cards.length)return;
      const box=financeCarousel.getBoundingClientRect(),center=box.left+box.width/2;
      let best=0,bestDist=Infinity;
      cards.forEach((card,i)=>{const r=card.getBoundingClientRect(),dist=Math.abs((r.left+r.right)/2-center);if(dist<bestDist){bestDist=dist;best=i}});
      if(best!==selectedFinanceCard){selectedFinanceCard=best;cards.forEach((card,i)=>card.classList.toggle('selectedFinanceCard',i===best));document.querySelectorAll('.financeCardListRow').forEach((row,i)=>row.classList.toggle('selected',i===best));}
    };
    financeCarousel.addEventListener('scroll',()=>{if(cardScrollRaf)cancelAnimationFrame(cardScrollRaf);cardScrollRaf=requestAnimationFrame(syncFinanceSelection)},{passive:true});
  }
  ...
  const categoryFormEl=$('#categoryForm');if(categoryFormEl)categoryFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),old=modal?.oldCat||'',name=upper((d.name||'').trim());if(!name)return alert('Kategori adı gerekli');if(old&&old!==name){state.customCategories=state.customCategories.filter(x=>x!==old);if(!C.includes(name))state.customCategories.push(name);state.expenses.forEach(x=>{if(x.category===old)x.category=name});delete state.categoryMeta[old]}else if(!C.includes(name))state.customCategories.push(name);state.categoryMeta[name]={...(state.categoryMeta[name]||{}),color:d.color||'#58c7ff',categoryId:categoryId(name)};normalizeV19(state);await save();modal=null;render();showToast('KATEGORİ KAYDEDİLDİ')};
...
function stmtCategoryBase(t){
  ...
  // Merkezi kurum/işyeri eşleştirmeleri: PDF ekstre ve öğrenilen kurallar aynı kategori adlarını kullanır.
  if(/BUSK[İI]|BURSA\s*SU|İSKİ|ISKI|ASKİ|ASKI|İZSU|IZSU|SU\s*FATURA|SU\s*FATURASI/.test(t))return'Su';
  if(/BURSA\s*GAZ|BURSAGAZ|BAŞKENTGAZ|BASKENTGAZ|İGDAŞ|IGDAS|DOĞALGAZ|DOGALGAZ/.test(t))return'Doğalgaz';
  if(/ULUDAĞ\s*ELEKTRİK|ULUDAG\s*ELEKTRIK|UEDAŞ|UEDAS|ENERJİSA|ENERJISA|CK\s*ENERJİ|CK\s*ENERJI|GDZ\s*ELEKTRİK|GDZ\s*ELEKTRIK|ELEKTRİK|ELEKTRIK/.test(t))return'Elektrik';
  if(/TURKCELL|VODAFONE|TÜRK\s*TELEKOM|TURK\s*TELEKOM|TT\s*MOBİL|TT\s*MOBIL/.test(t))return'Cep Telefonu';
  if(/SUPERONLINE|TURKCELL\s*SUPERONLINE|TÜRKSAT|TURKSAT|KABLONET|MILLENICOM|TURKNET|İNTERNET|INTERNET/.test(t))return'İnternet';
...
const HANE_SW_BUILD='19.4.8.20260921-V133-CATEGORY-COMPLETE';
