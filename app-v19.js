...
function fixed(){
  ...
  const catHtml=cats.length?`<div class="fixedCategoryTotals">${cats.map(([cat,total])=>`<button type="button" class="fixedCatTotal" data-action="fixedCategoryDetail" data-cat="${esc(cat)}"><span><i>${I[cat]||'●'}</i><b>${esc(cat)}</b></span><strong>${money(total)}</strong></button>`).join('')}<div class="fixedCatGrand"><span>TOPLAM GİDER</span><strong>${money(catTotal)}</strong></div></div>`:'<div class="notice">KATEGORİ TOPLAMI İÇİN GİDER KAYDI YOK.</div>';
  const normalHtml=normalItems.length?normalItems.map(x=>{const card=x.cardId?state.cards.find(c=>c.id===x.cardId):null;return `<div class="item fixedItem normalExpenseItem" data-action="editExpense" data-id="${x.id}"><div class="ico premiumIco">${I[x.category]||'●'}</div><div class="fixedMain"><b>${esc(x.title)}</b><small>${x.date||''} · NORMAL GİDER · ${esc(x.category||'Diğer')} · ${x.source==='card'?'KART'+(card?' · '+esc(card.bank):''):'NAKİT'}</small></div><div class="fixedRight"><b style="color:var(--red)">-${money(x.amount)}</b><small class="tapDetailHint">AYRINTI ›</small></div></div>`}).join(''):'<div class="notice">NORMAL GİDER YOK.</div>';
  const fixedHtml=fixedItems.length?fixedItems.map(x=>{const paid=fixedPaidForMonth(x),pay=fixedPaymentFor(x);return`<div class="item fixedItem ${paid?'isPaid':''}" data-fixed-id="${x.id}"><div class="ico premiumIco" data-action="editFixedExpense" data-id="${x.id}">${I[x.category]||'●'}</div><div class="fixedMain" data-action="editFixedExpense" data-id="${x.id}"><b>${esc(x.title)}</b><small>${x.dueDate||x.date} · SABİT GİDER · ${esc(x.category||'Diğer')}${paid?` · ÖDENDİ ${pay?.date||''} · ${pay?.source==='card'?'KART':'NAKİT'}`:''}</small></div><div class="fixedRight"><b style="color:${paid?'var(--green)':'var(--red)'}">${paid?'✓ ':'-'}${money(x.amount)}</b><button type="button" class="paidToggle ${paid?'on':''}" data-action="toggleFixedPaid" data-id="${x.id}">${paid?'↶ GERİ AL':'ÖDEDİM'}</button>${paid?`<button type="button" class="tinyEdit" data-action="editFixedPayment" data-id="${x.id}">ÖDEMEYİ DÜZENLE</button>`:''}</div></div>`}).join(''):'<div class="notice">SABİT GİDER YOK.</div>';
