...
function hanAudit(){
 ...
 (state.cards||[]).forEach(c=>{const expected=(Number.isFinite(+c.balanceAnchorAmount)&&c.balanceAnchorDate?+c.balanceAnchorAmount:+c.openingBalance||0)+cardDerivedNet(c.id);if(Math.abs((+c.balance||0)-expected)>.01)add('warn','KARTLAR','KART BORCU TUTARSIZ',`${c.bank||''} ${c.name||''} · hesaplanan ${money(expected)}, kayıtlı ${money(+c.balance||0)}`,'editCard',c.id)});
 const exps=(state.expenses||[]).filter(x=>!x.recurring&&x.date&&Number.isFinite(+(x.actualAmount??x.amount)));
 const norm=t=>String(t||'').toLocaleUpperCase('tr-TR').replace(/[^A-ZÇĞİÖŞÜ0-9 ]/g,' ').replace(/\s+/g,' ').trim();
 // Aynı gün, tutarı en fazla 50 TL farklı olan benzer işyeri/açıklamaları ödeme kaynağından bağımsız incele.
 for(let i=0;i<exps.length;i++)for(let j=i+1;j<exps.length;j++){const a=exps[i],b=exps[j];if(a.date!==b.date)continue;const da=Math.abs(+(a.actualAmount??a.amount)-+(b.actualAmount??b.amount));if(da>50)continue;const na=norm(a.baseTitle||a.title),nb=norm(b.baseTitle||b.title);const similar=na&&nb&&(na===nb||na.includes(nb)||nb.includes(na));if(similar&&!/ULAŞIM|ULASIM|YOL/i.test(String(a.category||'')+' '+String(a.title||''))&&!/ULAŞIM|ULASIM|YOL/i.test(String(b.category||'')+' '+String(b.title||'')))add('warn','FİNANS','OLASI MÜKERRER / BENZER İŞLEM',`${a.date} · ${a.title||''} ${money(+(a.actualAmount??a.amount))} (${a.source||'?'}) ↔ ${b.title||''} ${money(+(b.actualAmount??b.amount))} (${b.source||'?'}) · fark ${money(da)}`,'editExpense',a.id,b.id)}
 // Aynı/benzer işyerinin kategori sapmalarını göster.
...
function hanRecordCard(id){const x=(state.expenses||[]).find(z=>String(z.id)===String(id));if(!x)return '';const card=x.cardId?(state.cards||[]).find(c=>c.id===x.cardId):null;const src=x.source==='card'?`KART${card?' · '+esc(card.bank||'')+' '+esc(card.name||''):''}`:(x.source==='cash'?'NAKİT':String(x.source||'—').toUpperCase());return `<div class="notice hanCompare"><b>${esc(x.title||'Gider')}</b><br><span>${esc(x.date||'—')} · ${money(+(x.actualAmount??x.amount)||0)} · ${esc(x.category||'Diğer')} · ${src}</span><div class="hanReviewActions"><button class="btn" data-action="hanEditFromInspect" data-record-action="editExpense" data-record-id="${esc(x.id)}">DÜZENLE</button><button class="btn danger" data-action="hanDeleteFromInspect" data-record-id="${esc(x.id)}">SİL</button></div></div>`}
function hanInspectBody(title,detail,action,id,id2=''){const key=hanIssueKey(title,id,id2);if(action==='roadFeeGroup'){const rows=roadFeeItemsForDate(id),total=rows.reduce((n,x)=>n+(+(x.actualAmount??x.amount)||0),0);return `<div class="notice"><b>NEDEN ŞÜPHELİ?</b><br>${esc(detail)}</div><div class="section"><b>AYNI GÜN ULAŞIM KAYITLARI</b><span>${rows.length} kayıt · ${money(total)}</span></div>${rows.map(x=>hanRecordCard(x.id)).join('')}<button class="btn gold" data-action="hanIssueOk" data-issue-key="${esc(key)}">BUNDA SIKINTI YOK</button><div class="notice taraSafety">Bu seçim finansal kayıtları değiştirmez; HAN bu uyarıyı tekrar göstermez.</div>`}return `<div class="notice"><b>NEDEN ŞÜPHELİ?</b><br>${esc(detail)}</div><div class="section"><b>KARŞILAŞTIRILAN KAYITLAR</b><span>Kararı sen verirsin</span></div>${hanRecordCard(id)}${id2?hanRecordCard(id2):''}<button class="btn gold" data-action="hanIssueOk" data-issue-key="${esc(key)}">BUNDA SIKINTI YOK</button><div class="notice taraSafety">Düzenle veya Sil yalnız seçtiğin kaydı etkiler. “Bunda Sıkıntı Yok” finansal kaydı değiştirmez ve bu HAN uyarısını kapatır.</div>`}
function han(){const a=hanAudit(),bad=a.issues.filter(x=>x.level==='bad').length,warn=a.issues.filter(x=>x.level==='warn').length;const groups=['VERİ','FİNANS','EKSTRE','KARTLAR','EKRANLAR','NAVİGASYON','CACHE-BUILD','YEDEK'];return `<div class="card"><div class="section"><b>HAN · FİNANSAL DENETİM ASİSTANI</b><span>${a.checked} kayıt kontrol edildi</span></div><div class="reportHero3"><div><small>KRİTİK</small><b class="${bad?'exp':'inc'}">${bad}</b></div><div><small>ŞÜPHELİ</small><b>${warn}</b></div><div><small>DURUM</small><b class="${a.ok?'inc':'exp'}">${a.ok?'SAĞLAM':'İNCELE'}</b></div></div><div class="notice"><b>${monthLabel(a.month.month)}</b><br>Gelir ${money(a.month.incomeTotal)} · Gider ${money(a.month.expenseTotal)} · Kalan ${money(a.month.remaining)}</div><div class="section"><b>HAN SONUÇLARI</b><span>Kararı sen verirsin</span></div><div class="hanGroups">${groups.map(g=>{const n=a.issues.filter(x=>x.group===g).length;return `<span>${g} · ${n}</span>`}).join('')}</div><div class="list">${a.issues.length?a.issues.map(x=>`<div class="item taraIssue"><div class="ico">${x.level==='bad'?'!':'?'}</div><div><b>${esc(x.title)}</b><small>${esc(x.detail)}</small><small>${esc(x.group)}</small></div>${x.recordAction&&x.recordId?`<button class="taraFixBtn" data-action="hanInspect" data-record-action="${esc(x.recordAction)}" data-record-id="${esc(x.recordId)}" data-record-id2="${esc(x.recordId2||'')}" data-issue-title="${esc(x.title)}" data-issue-detail="${esc(x.detail)}">İNCELE</button>`:'<span class="taraManual">İNCELE</span>'}</div>`).join(''):'<div class="notice">HAN ŞÜPHELİ VEYA TUTARSIZ KAYIT BULMADI.</div>'}</div><button class="btn" data-action="hanRun">KONTROLÜ TEKRARLA</button><div class="notice taraSafety"><b>HAN KURALI</b><br>HAN finansal kayıtları otomatik değiştirmez, silmez, birleştirmez veya kategori atamaz. Şüpheli kaydı ve nedenini gösterir; karar kullanıcıya aittir.</div></div>`}
...
function menuBody(){const a=[['home','home','ANA EKRAN'],['transactions','transactions','HAREKETLER'],['fixed','expense','GİDERLER'],['cards','cards','KARTLAR / ESNEK HESAP'],['calendar','fixed','TAKVİM'],['reports','report','RAPORLAR'],['alerts','bell','HATIRLATMALAR'],['notes','note','NOTLAR'],['categories','fixed','KATEGORİLER'],['members','profile','HANE ÜYELERİ'],['profile','profile','PROFİL / KİMLİK'],['tara','report','HAN · FİNANSAL DENETİM'],['settings','settings','AYARLAR']];return `<div class="menuList">${a.map(x=>`<button data-action="menuGo" data-go="${x[0]}"><i>${premiumIcon(x[1],24)}</i><b>${x[2]}</b><span>›</span></button>`).join('')}</div>`}
...
function settings(){return`<div class="card"><div class="setting" data-action="togglePrivacy"><span class="settingIco">👁</span><span>GİZLİLİK MODU</span><b>${state.settings.privacy?'Açık':'Kapalı'}</b></div><div class="setting" data-tab="homeEdit"><span class="settingIco">⌂</span><span>ANA SAYFAYI DÜZENLE</span><b>›</b></div><div class="setting" data-tab="members"><span class="settingIco">👥</span><span>HANE ÜYELERİ</span><b>›</b></div><div class="setting" data-tab="categories"><span class="settingIco">🎨</span><span>KATEGORİLERİ DÜZENLE / EKLE</span><b>›</b></div><div class="setting" data-tab="tara"><span class="settingIco">✓</span><span>HAN · FİNANSAL DENETİM</span><b>›</b></div><div class="setting" data-action="toggleDarkMode"><span class="settingIco">${premiumIcon("palette",30)}</span><span>KARANLIK MOD</span><b>${state.settings.darkMode!==false?'Açık':'Kapalı'}</b></div><div class="setting" data-tab="about"><span class="settingIco">${premiumIcon("info",30)}</span><span>HAKKINDA</span><b>›</b></div><div class="setting" data-action="clearAll"><span class="settingIco dangerIco">${premiumIcon("trash",30)}</span><span style="color:var(--red)">TÜM VERİLERİ SIFIRLA</span><b></b></div></div>`}
function notes(){
