...
const enc=new TextEncoder(),dec=new TextDecoder();let state=null,key=null,current='home',modal=null,pin='',timer=null,setupPhoto='',themeDraft=null,reportPeriod='month',txFilter='all',financeTab='cards',navHistory=[];
...
function flexPanel(){return `<div class="section"><b>ESNEK HESAPLAR</b><button class="miniAddBtn" data-action="addFlex">+ ESNEK HESAP</button></div><div class="flexVerticalList">${state.flexAccounts.map(a=>{const available=Math.max(0,(+a.limit||0)-(+a.balance||0));return`<div class="flexAccount haneFlexVertical"><div class="haneFlexBrand">${haneLogo(34,'flexLogo')}<div><strong>HANE</strong><small>ESNEK HESAP</small></div></div><div class="flexBank">${esc(a.bank)}</div><div class="flexName">${esc(a.name)}</div><div class="flexAvailable"><small>KULLANILABİLİR LİMİT</small><b>${money(available)}</b></div><div class="flexLimitBar"><i style="width:${(+a.limit||0)?Math.min(100,available/(+a.limit||1)*100):0}%"></i></div><div class="grid"><div><small>TOPLAM LİMİT</small><b>${money(a.limit)}</b></div><div><small>KULLANILAN</small><b>${money(a.balance)}</b></div><div><small>HESAP KESİM</small><b>${esc(a.statementDate||'-')}</b></div><div><small>SON ÖDEME</small><b>${esc(a.dueDate||'-')}</b></div></div><div class="cardActions"><button class="btn" data-action="flexSpend" data-id="${a.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="flexPay" data-id="${a.id}">₺ ÖDEME YAPTIM</button><button class="iconEdit" data-action="editFlex" data-id="${a.id}">✎</button></div></div>`}).join('')||'<div class="notice">HENÜZ ESNEK HESAP EKLENMEDİ.</div>'}</div>`}
...
function reports(){const T=periodTotals(),inside=x=>x.date>=T.range.start&&x.date<=T.range.end,B=(k,l)=>`<button data-action="reportPeriod" data-period="${k}" class="${reportPeriod===k?'active':''}">${l}</button>`,items=[...state.incomes.filter(inside).map(x=>({...x,_kind:'income'})),...state.expenses.filter(inside).map(x=>({...x,_kind:'expense'})),...(state.flexTransactions||[]).filter(x=>x.kind==='pay'&&inside(x)).map(x=>({...x,_kind:'flexPayment'}))].sort((a,b)=>String(b.date).localeCompare(String(a.date)));return`<div class="seg">${B('day','Gün')}${B('week','Hafta')}${B('month','Ay')}${B('year','Yıl')}</div><div class="section"><b>${T.range.label}</b><span></span></div><div class="summary"><div class="sum"><label>Toplam Gelir</label><strong style="color:var(--green)">${money(T.i)}</strong></div><div class="sum"><label>Toplam Gider</label><strong style="color:var(--red)">${money(T.e)}</strong></div><div class="sum"><label>Kalan</label><strong>${money(T.r)}</strong></div></div><div class="card" style="padding:14px;margin-top:10px"><canvas id="chart" style="width:100%;height:220px"></canvas><div class="chartLegend"><span><i style="background:var(--gi)"></i>GELİR</span><span><i style="background:var(--ge)"></i>GİDER</span><span><i style="background:var(--gr)"></i>KALAN</span></div></div><div class="section"><b>GRAFİĞE AİT HAREKETLER</b><span>${items.length} KAYIT</span></div><div class="list reportMoves">${items.length?items.map(x=>`<div class="item"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):x._kind==='flexPayment'?premiumIcon('cards',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x._kind==='income'?'GELİR':x._kind==='flexPayment'?'ESNEK HESAP ÖDEMESİ':esc(x.category||'DİĞER')}${x._kind==='expense'?' · '+(x.recurring?(fixedPaidForMonth(x,state.selectedMonth)?'ÖDENDİ':'ÖDENMEDİ'):paymentLabel(x)):''}</small></div><div class="reportMoveRight"><b style="color:${x._kind==='income'||x._kind==='flexPayment'?'var(--green)':'var(--red)'}">${x._kind==='income'||x._kind==='flexPayment'?'+':'-'}${money(x.amount)}</b><div><button type="button" data-action="${x._kind==='income'?'editIncome':x._kind==='flexPayment'?'editFlexPayment':'editExpense'}" data-id="${x.id}">DÜZENLE</button><button type="button" class="danger" data-action="${x._kind==='income'?'delIncome':x._kind==='flexPayment'?'delFlexPayment':'delExpense'}" data-id="${x.id}">SİL</button></div></div></div>`).join(''):'<div class="notice">BU GRAFİK ARALIĞINDA HAREKET YOK.</div>'}</div>`}
