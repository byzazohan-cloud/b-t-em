...
function actualExpenseEntriesForMonth(m=state?.selectedMonth){
  ...
  const fixed=(state?.fixedPayments||[]).filter(p=>(p.month||String(p.date||'').slice(0,7))===m).map(p=>{const ex=(state?.expenses||[]).find(x=>x.id===p.expenseId);return {...(ex||{}),...p,id:ex?.id||p.expenseId||p.id,paymentId:p.id,expenseId:p.expenseId,title:p.title||ex?.title||'SABİT GİDER',category:p.category||ex?.category||'Diğer',memberId:ex?.memberId||'me',recurring:true,billAmount:fixedBillAmount(ex),amount:+(p.actualAmount??p.amount??0)||0,date:p.date||ex?.dueDate||ex?.date||iso(),source:p.source||ex?.source||'cash',cardId:p.cardId||ex?.cardId||null,_actual:true}});
  return [...normal,...fixed]
}
...
function transactions(){
  ...
  // Tek arama: yazı yazıldığında tüm eski + yeni kayıtlarda anlık ara.
  ...
  return `<div class="seg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Kart')}</div><div class="liveTxSearch"><span>⌕</span><input id="txSearch" autocomplete="off" placeholder="Hareket ara..." value="${esc(txSearch)}">${q?'<button type="button" data-action="clearTxSearch">×</button>':''}</div><div class="section"><b>${q?'ARAMA SONUÇLARI':state.selectedMonth}</b><span>${a.length} KAYIT</span></div><div class="list">${a.length?a.map(rr).join(''):'<div class="notice">Eşleşen hareket bulunamadı.</div>'}</div>`
}
function fixedCategoryDetail(cat){
  // GİDERLER merkezi: kategori ayrıntısı sabit + normal bütün gerçek gider kayıtlarını gösterir.
  const items=state.expenses.filter(x=>(x.category||'Diğer')===cat).sort((a,b)=>String(b.dueDate||b.date||'').localeCompare(String(a.dueDate||a.date||'')));
  const total=items.reduce((n,x)=>n+(+x.amount||0),0);
  return `<div class="fixedCatDetail"><div class="fixedCatDetailHero"><div><small>KATEGORİ</small><h2>${esc(cat)}</h2></div><strong>${money(total)}</strong></div><div class="section"><b>TÜM GİDER KAYITLARI</b><span>${items.length} KAYIT</span></div><div class="list">${items.length?items.map(x=>{const fixed=!!x.recurring,paid=fixed?fixedPaidForMonth(x):!!x.paid,p=fixed?fixedPaymentFor(x):null,card=(p?.cardId||x.cardId)?state.cards.find(c=>c.id===(p?.cardId||x.cardId)):null,source=fixed?(p?.source||x.source):x.source;return `<div class="item fixedCatDetailItem" data-action="${fixed?'editFixedExpense':'editExpense'}" data-id="${x.id}"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div class="fixedCatDetailMain"><b>${esc(x.title)}</b><small>${x.dueDate||x.date||''} · ${fixed?'SABİT GİDER':'NORMAL GİDER'}${fixed?' · '+(paid?'ÖDENDİ':'ÖDENMEDİ'):''} · ${source==='card'?'KART'+(card?' · '+esc(card.bank):''):'NAKİT'}</small></div><div class="fixedCatDetailRight"><strong>${money(x.amount)}</strong><small class="tapDetailHint">AYRINTI ›</small></div></div>`}).join(''):'<div class="notice">BU KATEGORİDE GİDER KAYDI YOK.</div>'}</div></div>`
}
function fixed(){
  // GİDERLER ekranı artık yalnızca sabit giderleri değil, bütün normal + sabit giderleri kapsar.
  const all=[...state.expenses].sort((x,y)=>String(y.dueDate||y.date||'').localeCompare(String(x.dueDate||x.date||'')));
  ...
  const normalItems=state.expenses.filter(x=>!x.recurring).sort((x,y)=>String(y.date||'').localeCompare(String(x.date||'')));
  ...
  const normalHtml=normalItems.length?normalItems.map(x=>{const card=x.cardId?state.cards.find(c=>c.id===x.cardId):null;return `<div class="item fixedItem normalExpenseItem" data-action="editExpense" data-id="${x.id}"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div class="fixedMain"><b>${esc(x.title)}</b><small>${x.date||''} · NORMAL GİDER · ${esc(x.category||'Diğer')} · ${x.source==='card'?'KART'+(card?' · '+esc(card.bank):''):'NAKİT'}</small></div><div class="fixedRight"><b style="color:var(--red)">-${money(x.amount)}</b><small class="tapDetailHint">AYRINTI ›</small></div></div>`}).join(''):'<div class="notice">NORMAL GİDER YOK.</div>';
  ...
  return`<div class="section"><b>GİDERLER</b><button class="miniAddBtn" data-action="addExpense">+ GİDER EKLE</button></div><div class="section fixedCatHead"><b>KATEGORİ TOPLAMLARI</b><span>${cats.length} KATEGORİ</span></div>${catHtml}<div class="section"><b>NORMAL GİDERLER</b><span>${normalItems.length} KAYIT</span></div><div class="list fixedList">${normalHtml}</div><div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ SABİT GİDER</button></div><div class="notice fixedSortNote">ÖDENMEMİŞ SABİT GİDERLER ÜSTTE · ÖDENMİŞLER ALTTA</div><div class="list fixedList">${fixedHtml}</div>`
}
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.8 · FİNANS ÇEKİRDEK G1</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
