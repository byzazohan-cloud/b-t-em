...
function haneLogo(size=54,cls=''){
  ...
  return `<img class="haneLogoImg ${cls}" src="icons/hane-app-icon.png?v=1945" width="${n}" alt="HANE">`;
}
function haneFullLogo(cls=''){
  return `<img class="haneFullLogo ${cls}" src="icons/hane-app-icon.png?v=1945" alt="HANE">`;
}
...
function nav(){const items=[['home','⌂','ANA EKRAN','gold'],['transactions','≣','HAREKETLER','blue'],['fixed','▾','GİDERLER','red'],['cards','▰','FİNANS','violet'],['profile','●','PROFİL','green']];return`<nav class="nav premiumNav targetNav v1945Nav">${items.map(x=>`<button data-tab="${x[0]}" class="nav-${x[0]} ${current===x[0]?'active':''}"><span class="navIcon navGlass ${x[3]}"><b>${x[1]}</b></span><span>${x[2]}</span></button>`).join('')}</nav>`}
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.5</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
