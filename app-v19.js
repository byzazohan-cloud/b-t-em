...
const C=['Kira','Aidat','Market','Manav','Fırın','Harçlık','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Faturalar','İnternet','Elektrik','Su','Doğalgaz','Cep Telefonu','Diğer'];
const NORMAL_C=['Market','Manav','Fırın','Harçlık','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Diğer'];
...
function def(){return{version:19,selectedMonth:ym(new Date()),profile:{name:'',photo:'',motto:'Disiplin, özgürlüğün kapısını açar.'},settings:{lockMinutes:15,leadDays:3,notifications:false,darkMode:true},theme:{bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'},incomes:[],expenses:[],cards:[],cardTransactions:[],cardPayments:[]}}
...
function buildTopBar(){
  if(current==='home')return`<div class="top homeTop"><div class="topSpacer"></div><div class="brand brandLogo">${haneLogo(30,'brandMark')}<span>HANE</span></div><button class="ib premiumTopIcon" data-tab="profile">${premiumIcon("settings",24)}</button></div>`;
  const t={transactions:'HAREKETLER',fixed:'SABİT GİDERLER',cards:'KREDİ KARTLARIM',reports:'RAPORLAR',profile:'PROFİL AYARLARI',backup:'YEDEKLEME',settings:'UYGULAMA AYARLARI',theme:'TEMA STÜDYOSU',alerts:'BİLDİRİMLER',about:'HAKKINDA',monthSpent:'BU AY HARCANAN',monthPaid:'BU AY ÖDENEN'};
...
function nav(){const items=[['home','home','ANA EKRAN'],['transactions','transactions','HAREKETLER'],['fixed','fixed','GİDERLER'],['cards','cards','KARTLAR'],['profile','settings','AYARLAR']];return`<nav class="nav premiumNav">${items.map(x=>`<button data-tab="${x[0]}" class="${current===x[0]?'active':''}"><span class="navIcon">${premiumIcon(x[1],30)}</span><span>${x[2]}</span></button>`).join('')}</nav>`}
...
function settings(){return`<div class="card"><div class="setting" data-action="toggleDarkMode"><span class="settingIco">${premiumIcon("palette",24)}</span><span>KARANLIK MOD</span><b>${state.settings.darkMode!==false?'Açık':'Kapalı'}</b></div><div class="setting"><span class="settingIco">${premiumIcon("bell",24)}</span><span>BİLDİRİMLER</span><b>${state.settings.notifications?'Açık':'Kapalı'}</b></div><div class="setting"><span class="settingIco">${premiumIcon("globe",24)}</span><span>DİL</span><b>Türkçe</b></div><div class="setting" data-tab="about"><span class="settingIco">${premiumIcon("info",24)}</span><span>HAKKINDA</span><b>›</b></div><div class="setting" data-action="clearAll"><span class="settingIco dangerIco">${premiumIcon("trash",24)}</span><span style="color:var(--red)">TÜM VERİLERİ SIFIRLA</span><b></b></div></div>`}
function theme(){
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.0</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
function view(){return({home,transactions,fixed,cards,reports,profile,backup,settings,theme,alerts,about,monthSpent,monthPaid}[current]||home)()}
...
let financeTab='cards', financeIndex=0, sideMenuOpen=false;
...
buildTopBar=function(){if(current==='home')return `<div class="top homeTop v194Top"><button class="ib premiumTopIcon" data-action="toggleMenu">☰</button><div class="brand brandLogo">${haneLogo(30,'brandMark')}<span>HANE</span></div><div class="topPair"><button class="ib premiumTopIcon" data-tab="alerts">${premiumIcon('bell',27)}</button><button class="ib premiumTopIcon" data-tab="profile">${premiumIcon('settings',27)}</button></div></div>`;const titles={transactions:'HAREKETLER',fixed:'GİDERLER',cards:'KARTLAR / HESAPLAR',reports:'RAPORLAR',profile:'AYARLAR',backup:'YEDEKLEME',settings:'UYGULAMA AYARLARI',alerts:'HATIRLATMALAR',about:'HAKKINDA',monthSpent:'HARCAMALAR',monthPaid:'ÖDEMELER'};return `<div class="top"><button class="back" data-action="back">‹</button><div class="brand">${titles[current]||'HANE'}</div><div style="width:38px"></div></div>`};
const oldRender=render;
render=function(){applyTheme();$('#app').innerHTML=`<main class="phone">${buildTopBar()}<div class="content">${view()}</div>${nav()}</main>${sideMenu()}${modalWrap()}`;bind();if(current==='reports')requestAnimationFrame(drawChart)};
function summaryDetails(kind){const m=state.selectedMonth;let items=[];if(kind==='income')items=state.incomes.filter(x=>x.date.startsWith(m)).map(x=>({...x,k:'income'}));else if(kind==='expense')items=state.expenses.filter(x=>x.date.startsWith(m)).map(x=>({...x,k:'expense'}));else items=[...state.incomes.filter(x=>x.date.startsWith(m)).map(x=>({...x,k:'income'})),...state.expenses.filter(x=>x.date.startsWith(m)).map(x=>({...x,k:'expense'}))];items.sort((a,b)=>b.date.localeCompare(a.date));return `<div class="list">${items.length?items.map(x=>`<div class="item"><div class="ico">${x.k==='income'?'₺':catIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date}${x.category?' · '+esc(x.category):''}</small></div><div class="reportMoveRight"><b>${x.k==='income'?'+':'-'}${money(x.amount)}</b><div><button data-action="${x.k==='income'?'editIncome':'editExpense'}" data-id="${x.id}">DÜZENLE</button><button class="danger" data-action="${x.k==='income'?'delIncome':'delExpense'}" data-id="${x.id}">SİL</button></div></div></div>`).join(''):'<div class="notice">KAYIT YOK.</div>'}</div>`}
const oldHome=home;
home=function(){let s=oldHome();s=s.replace('<div class="summary"><div class="sum">','<div class="summary"><div class="sum clickableSum" data-action="summaryIncome">').replace('<div class="sum"><label>Gider</label>','<div class="sum clickableSum" data-action="summaryExpense"><label>Gider</label>').replace('<div class="sum"><label>Kalan</label>','<div class="sum clickableSum" data-action="summaryRemain"><label>Kalan</label>');return s};
function categoryManager(){const cats=normalCategories();return `<div class="section"><b>KATEGORİLERİ DÜZENLE</b><button class="miniAddBtn" data-action="addCategory">+ EKLE</button></div><div class="list">${cats.filter(x=>x!=='Diğer').map(n=>{const c=(state.customCategories||[]).find(x=>x.name===n);return `<div class="item"><div class="ico">${catPremiumIcon(n)}</div><div><b>${esc(n)}</b><small>${c?'ÖZEL KATEGORİ':'SİSTEM KATEGORİSİ'}</small></div><div class="right"><button class="miniAddBtn" data-action="editCategory" data-name="${esc(n)}">DÜZENLE</button></div></div>`}).join('')}</div>`}
function categoryForm(c={}){return `<form id="categoryForm" class="form">${input('name','Kategori Adı',c.name||'')}${input('icon','İkon / Emoji',c.icon||'✦')}${input('color','Renk',c.color||'#d8ad4f','color')}<button class="btn gold" type="submit">KAYDET</button>${c.custom?`<button class="btn" type="button" data-action="deleteCategory" data-name="${esc(c.name)}">SİL</button>`:''}</form>`}
const oldSettings=settings;
settings=function(){return `<div class="card"><div class="setting" data-action="categoryManager"><span class="settingIco">✦</span><span>KATEGORİLERİ DÜZENLE / EKLE</span><b>›</b></div></div><div style="height:10px"></div>${oldSettings()}`};
function accountForm(x={},flex=false){return `<form class="form" id="${flex?'flexForm':'accountForm'}">${input('bank','Banka',x.bank||'')}${input('name',flex?'Esnek Hesap Adı':'Hesap Adı',x.name||'')}${input('limit',flex?'Limit':'Bakiye',x.limit??x.balance??0,'number')}${input('balance',flex?'Kullanılan Tutar':'Kullanılabilir Bakiye',x.balance||0,'number')}${input('statementDate','Hesap Kesim Tarihi',x.statementDate||iso(),'date')}${input('dueDate','Son Ödeme Tarihi',x.dueDate||iso(new Date(Date.now()+10*86400000)),'date')}<button class="btn gold" type="submit">KAYDET</button>${x.id?`<button class="btn" type="button" data-action="${flex?'delFlex':'delAccount'}" data-id="${x.id}">SİL</button>`:''}</form>`}
function financeCard(c,type='card'){const info=type==='card'?cardPaymentInfo(c):null;const limit=+c.limit||0,bal=+c.balance||0;return `<div class="financeHero ${c.style||'blackgold'}"><div class="financeBank">${esc(c.bank||'BANKA')}</div><div class="financeChip">▣　)))</div><h2>${esc(c.name||'HESAP')}</h2>${type==='card'?`<div class="digits">•••• •••• •••• ${esc(c.last4||'0000')}</div>`:''}<div class="financeMeta"><span>${type==='card'?'LİMİT':'LİMİT / BAKİYE'}<b>${money(limit)}</b></span><span>${type==='card'?'GÜNCEL BORÇ':'KULLANILAN'}<b>${money(bal)}</b></span></div><div class="financeDates"><span>HESAP KESİM <b>${type==='card'?info.statementDate.toLocaleDateString('tr-TR'):(c.statementDate||'-')}</b></span><span>SON ÖDEME <b>${type==='card'?info.dueDate.toLocaleDateString('tr-TR'):(c.dueDate||'-')}</b></span></div><div class="cardActions"><button class="btn" data-action="${type==='card'?'cardSpend':type==='flex'?'flexSpend':'accountSpend'}" data-id="${c.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="${type==='card'?'cardPay':type==='flex'?'flexPay':'accountPay'}" data-id="${c.id}">₺ ÖDEME YAPTIM</button></div><button class="financeEdit" data-action="${type==='card'?'editCard':type==='flex'?'editFlex':'editAccount'}" data-id="${c.id}">DÜZENLE</button></div>`}
cards=function(){const list=financeTab==='cards'?state.cards:financeTab==='accounts'?state.accounts:state.flexAccounts;financeIndex=Math.max(0,Math.min(financeIndex,Math.max(0,list.length-1)));const x=list[financeIndex],type=financeTab==='cards'?'card':financeTab==='accounts'?'account':'flex';return `<div class="financeTabs"><button class="${financeTab==='cards'?'active':''}" data-action="financeTab" data-kind="cards">KARTLAR</button><button class="${financeTab==='accounts'?'active':''}" data-action="financeTab" data-kind="accounts">HESAPLAR</button><button class="${financeTab==='flex'?'active':''}" data-action="financeTab" data-kind="flex">ESNEK HESAP</button></div><div class="section"><b>${financeTab==='cards'?'KARTLAR':financeTab==='accounts'?'HESAPLAR':'ESNEK HESAPLAR'}</b><button class="miniAddBtn" data-action="${type==='card'?'addCard':type==='account'?'addAccount':'addFlex'}">+ EKLE</button></div>${x?`<div class="financeSwipe"><button data-action="financePrev">‹</button>${financeCard(x,type)}<button data-action="financeNext">›</button></div><div class="financeDots">${list.map((z,i)=>`<button class="${i===financeIndex?'active':''}" data-action="financePick" data-index="${i}">${esc(z.bank||z.name||String(i+1))}</button>`).join('')}</div>`:'<div class="notice">HENÜZ KAYIT YOK.</div>'}`};
function simpleSpendForm(x){return `<form class="form" id="simpleSpendForm">${input('title','Açıklama','')}${input('amount','Tutar',0,'number')}${input('date','Tarih',iso(),'date')}<button class="btn gold">KAYDET</button></form>`}
function simplePayForm(x){return `<form class="form" id="simplePayForm">${input('amount','Ödeme Tutarı',0,'number')}${input('date','Tarih',iso(),'date')}<button class="btn gold">KAYDET</button></form>`}
const oldMonthPaid=monthPaid;
monthPaid=function(){const m=state.selectedMonth,card=(state.cardPayments||[]).filter(x=>String(x.date||'').startsWith(m));let html=oldMonthPaid();card.forEach(x=>{html=html.replace(`<div class="item"><div class="ico premiumIco">▭</div><div><b>${esc(x.title||'KREDİ KARTI ÖDEMESİ')}</b>`,`<div class="item"><div class="ico premiumIco">▭</div><div><b>${esc(x.title||'KREDİ KARTI ÖDEMESİ')}</b>`)});html=html.replace(/<div class="right"><b style="color:var\(--green\)">([^<]+)<\/b><\/div><\/div>/g,'<div class="right"><b style="color:var(--green)">$1</b></div></div>');return html};
const oldAct=act;
act=async function(a,el){
 if(a==='toggleMenu'){sideMenuOpen=!sideMenuOpen;render();return}
 if(a==='summaryIncome'||a==='summaryExpense'||a==='summaryRemain'){open(a==='summaryIncome'?'GELİR AYRINTILARI':a==='summaryExpense'?'GİDER AYRINTILARI':'KALAN HESABI',summaryDetails(a==='summaryIncome'?'income':a==='summaryExpense'?'expense':'remain'));return}
 if(a==='categoryManager'){open('KATEGORİ YÖNETİMİ',categoryManager());return}
 if(a==='addCategory'){open('KATEGORİ EKLE',categoryForm());return}
 if(a==='editCategory'){const n=el.dataset.name,c=(state.customCategories||[]).find(x=>x.name===n);open('KATEGORİYİ DÜZENLE',categoryForm(c?{...c,custom:true}:{name:n,icon:I[n]||'✦',color:CAT_COLORS[n]||'#d8ad4f',custom:false}),{categoryOld:n,categoryCustom:!!c});return}
 if(a==='deleteCategory'){const n=el.dataset.name;state.customCategories=state.customCategories.filter(x=>x.name!==n);await save();modal=null;render();return}
 if(a==='financeTab'){financeTab=el.dataset.kind;financeIndex=0;render();return} if(a==='financePrev'){const l=financeTab==='cards'?state.cards:financeTab==='accounts'?state.accounts:state.flexAccounts;if(l.length)financeIndex=(financeIndex-1+l.length)%l.length;render();return} if(a==='financeNext'){const l=financeTab==='cards'?state.cards:financeTab==='accounts'?state.accounts:state.flexAccounts;if(l.length)financeIndex=(financeIndex+1)%l.length;render();return} if(a==='financePick'){financeIndex=+el.dataset.index||0;render();return}
 if(a==='addAccount'){open('HESAP EKLE',accountForm());return} if(a==='editAccount'){const x=state.accounts.find(z=>z.id===el.dataset.id);open('HESABI DÜZENLE',accountForm(x),{id:x.id});return} if(a==='delAccount'){state.accounts=state.accounts.filter(x=>x.id!==el.dataset.id);await save();modal=null;render();return}
 if(a==='addFlex'){open('ESNEK HESAP EKLE',accountForm({},true));return} if(a==='editFlex'){const x=state.flexAccounts.find(z=>z.id===el.dataset.id);open('ESNEK HESABI DÜZENLE',accountForm(x,true),{id:x.id});return} if(a==='delFlex'){state.flexAccounts=state.flexAccounts.filter(x=>x.id!==el.dataset.id);await save();modal=null;render();return}
 if(['accountSpend','flexSpend'].includes(a)){open('HARCAMA EKLE',simpleSpendForm(),{financeType:a.startsWith('flex')?'flex':'account',financeId:el.dataset.id});return} if(['accountPay','flexPay'].includes(a)){open('ÖDEME YAPTIM',simplePayForm(),{financeType:a.startsWith('flex')?'flex':'account',financeId:el.dataset.id});return}
 if(a==='editCardPayment'){const p=state.cardPayments.find(x=>x.id===el.dataset.id);if(p)open('ÖDEMEYİ DÜZENLE',`<form class="form" id="editPaymentForm">${input('amount','Tutar',p.amount,'number')}${input('date','Tarih',p.date,'date')}<button class="btn gold">KAYDET</button><button class="btn" type="button" data-action="delCardPayment" data-id="${p.id}">SİL</button></form>`,{paymentId:p.id});return}
 if(a==='delCardPayment'){const p=state.cardPayments.find(x=>x.id===el.dataset.id),c=p&&state.cards.find(x=>x.id===p.cardId);if(c)c.balance=(+c.balance||0)+(+p.amount||0);state.cardPayments=state.cardPayments.filter(x=>x.id!==el.dataset.id);await save();modal=null;render();return}
 return oldAct(a,el)
};
const oldBind=bind;
bind=function(){oldBind();const cf=$('#categoryForm');if(cf)cf.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),name=upper(d.name.trim());if(!name)return;const old=modal?.categoryOld,idx=state.customCategories.findIndex(x=>x.name===old);const obj={name,icon:d.icon||'✦',color:d.color||'#d8ad4f'};if(idx>=0)state.customCategories[idx]=obj;else state.customCategories.push(obj);if(old&&old!==name){state.expenses.forEach(x=>{if(x.category===old)x.category=name});}I[name]=obj.icon;CAT_COLORS[name]=obj.color;await save();modal=null;render()};
 const af=$('#accountForm');if(af)af.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),x={id:modal?.id||id(),bank:upper(d.bank),name:upper(d.name),limit:+d.limit||0,balance:+d.balance||0,statementDate:d.statementDate,dueDate:d.dueDate,transactions:[]},i=state.accounts.findIndex(z=>z.id===x.id);if(i>=0)x.transactions=state.accounts[i].transactions||[],state.accounts[i]=x;else state.accounts.push(x);await save();modal=null;render()};
 const ff=$('#flexForm');if(ff)ff.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),x={id:modal?.id||id(),bank:upper(d.bank),name:upper(d.name),limit:+d.limit||0,balance:+d.balance||0,statementDate:d.statementDate,dueDate:d.dueDate,transactions:[]},i=state.flexAccounts.findIndex(z=>z.id===x.id);if(i>=0)x.transactions=state.flexAccounts[i].transactions||[],state.flexAccounts[i]=x;else state.flexAccounts.push(x);await save();modal=null;render()};
 const sf=$('#simpleSpendForm');if(sf)sf.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),arr=modal.financeType==='flex'?state.flexAccounts:state.accounts,x=arr.find(z=>z.id===modal.financeId),amt=+d.amount||0;if(x){x.balance=(+x.balance||0)+amt;x.transactions=x.transactions||[];x.transactions.push({id:id(),type:'spend',title:upper(d.title||'HARCAMA'),amount:amt,date:d.date})}await save();modal=null;render()};
 const pf=$('#simplePayForm');if(pf)pf.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),arr=modal.financeType==='flex'?state.flexAccounts:state.accounts,x=arr.find(z=>z.id===modal.financeId),amt=+d.amount||0;if(x){x.balance=Math.max(0,(+x.balance||0)-amt);x.transactions=x.transactions||[];x.transactions.push({id:id(),type:'pay',title:'ÖDEME',amount:amt,date:d.date})}await save();modal=null;render()};
 const ep=$('#editPaymentForm');if(ep)ep.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),p=state.cardPayments.find(x=>x.id===modal.paymentId);if(p){const c=state.cards.find(x=>x.id===p.cardId),old=+p.amount||0,neu=+d.amount||0;if(c)c.balance=Math.max(0,(+c.balance||0)+old-neu);p.amount=neu;p.date=d.date}await save();modal=null;render()};
};
