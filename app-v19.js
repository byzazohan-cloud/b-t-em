...
function hanAudit(){
 ...
 const month=financeMonthSnapshot(state.selectedMonth);return {issues,month,checked:all.length,ok:issues.length===0}
}
function hanRecordCard(id){const x=(state.expenses||[]).find(z=>String(z.id)===String(id));if(!x)return '';const card=x.cardId?(state.cards||[]).find(c=>c.id===x.cardId):null;const src=x.source==='card'?`KART${card?' · '+esc(card.bank||'')+' '+esc(card.name||''):''}`:(x.source==='cash'?'NAKİT':String(x.source||'—').toUpperCase());return `<div class="notice hanCompare"><b>${esc(x.title||'Gider')}</b><br><span>${esc(x.date||'—')} · ${money(+(x.actualAmount??x.amount)||0)} · ${esc(x.category||'Diğer')} · ${src}</span><br><button class="btn" data-action="hanEditFromInspect" data-record-action="editExpense" data-record-id="${esc(x.id)}">KAYDI DÜZENLE</button></div>`}
function hanInspectBody(title,detail,action,id,id2=''){if(action==='roadFeeGroup'){const rows=roadFeeItemsForDate(id),total=rows.reduce((n,x)=>n+(+(x.actualAmount??x.amount)||0),0);return `<div class="notice"><b>NEDEN ŞÜPHELİ?</b><br>${esc(detail)}</div><div class="section"><b>AYNI GÜN ULAŞIM KAYITLARI</b><span>${rows.length} kayıt · ${money(total)}</span></div>${rows.map(x=>hanRecordCard(x.id)).join('')}<div class="notice taraSafety">HAN bu kayıtları değiştirmez. Kayıtların doğruysa işlem yapmana gerek yok.</div>`}return `<div class="notice"><b>NEDEN ŞÜPHELİ?</b><br>${esc(detail)}</div><div class="section"><b>KARŞILAŞTIRILAN KAYITLAR</b><span>Kararı sen verirsin</span></div>${hanRecordCard(id)}${id2?hanRecordCard(id2):''}<div class="notice taraSafety">HAN yalnızca şüphe nedenini gösterir. Finansal kayıt otomatik değiştirilmez.</div>`}
...
const HANE_ENGINE_CACHE='hane-engine-19.4.29-V113-S19-HAN-FIX1';
