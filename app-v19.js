...
function fixed(){
  ...
  const catHtml=cats.length?`<div class="fixedCategoryTotals">${cats.map(([cat,total])=>`<div class="fixedCatTotal"><span><i>${I[cat]||'●'}</i><b>${esc(cat)}</b></span><strong>${money(total)}</strong></div>`).join('')}<div class="fixedCatGrand"><span>TOPLAM SABİT GİDER</span><strong>${money(catTotal)}</strong></div></div>`:'<div class="notice">KATEGORİ TOPLAMI İÇİN KAYIT YOK.</div>';
