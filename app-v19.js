...
function nav(){const items=[['home','home','ANA EKRAN'],['transactions','transactions','HAREKETLER'],['fixed','fixed','GİDERLER'],['cards','cards','FİNANS'],['profile','profile','PROFİL']];return`<nav class="nav premiumNav targetNav">${items.map(x=>`<button data-tab="${x[0]}" class="nav-${x[0]} ${current===x[0]?'active':''}"><span class="navIcon">${premiumIcon(x[1],34)}</span><span>${x[2]}</span></button>`).join('')}</nav>`}
