...
async function confirmStatementImport(){if(!state.cards.some(c=>c.id===statementImportCardId))throw new Error('Seçilen kart artık bulunamadı.');const checks=[...document.querySelectorAll('[data-stmt-check]')];let added=0,skipped=0;const localCounts={};for(const el of checks){if(!el.checked)continue;const i=+el.dataset.stmtCheck,r=statementImportRows[i];if(!r)continue;const date=document.querySelector(`[data-stmt-date="${i}"]`)?.value||r.date,title=stmtCleanTitle(document.querySelector(`[data-stmt-title="${i}"]`)?.value||r.title),amount=Number(document.querySelector(`[data-stmt-amount="${i}"]`)?.value),categoryRaw=document.querySelector(`[data-stmt-category="${i}"]`)?.value||r.category,category=C.includes(categoryRaw)?categoryRaw:'Diğer';if(!stmtValidIsoDate(date)||!title||!Number.isFinite(amount)||amount===0){skipped++;continue}const base=stmtFingerprint(statementImportCardId,date,title,amount),existing=stmtExistingCount(statementImportCardId,date,title,amount);let ord;if(base===r.baseFp){ord=r.occurrence;if(existing>=ord){skipped++;continue}}else{const n=(localCounts[base]=(localCounts[base]||0)+1);ord=existing+n}const ex={id:id(),title:upper(title),amount,actualAmount:amount,date,category,source:'card',cardId:statementImportCardId,recurring:false,memberId:'',attachment:'',importBaseFingerprint:base,importFingerprint:`${base}|#${ord}`,importedFromStatement:true,importedRefund:amount<0};state.expenses.push(ex);added++}await save();modal=null;render();showToast(`${added} İŞLEM EKLENDİ${skipped?` · ${skipped} ATLANDI`:''}`)}
