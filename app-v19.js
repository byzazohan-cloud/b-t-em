...
function stmtMoney(v){let x=String(v||'').replace(/\s/g,'').replace(/TL|TRY|₺/gi,'').replace(/[^0-9,.-]/g,'');if(!x)return NaN;if(x.includes(',')&&x.includes('.'))x=x.replace(/\./g,'').replace(',','.');else if(x.includes(','))x=x.replace(',','.');return Number(x)}
...
function stmtAnchorYear(text){const ys=[...String(text||'').matchAll(/\b\d{1,2}[.\/-]\d{1,2}[.\/-](20\d{2}|\d{2})\b/g)].map(m=>{let y=+m[1];return y<100?2000+y:y}).filter(y=>y>2000&&y<2100);const named=[...String(text||'').toLocaleUpperCase('tr-TR').matchAll(/\b\d{1,2}\s+(?:OCAK|OCA|ŞUBAT|ŞUB|MART|MAR|NİSAN|NİS|MAYIS|MAY|HAZİRAN|HAZ|TEMMUZ|TEM|AĞUSTOS|AĞU|EYLÜL|EYL|EKİM|EKİ|KASIM|KAS|ARALIK|ARA)\s+(20\d{2}|\d{2})\b/g)].map(m=>{let y=+m[1];return y<100?2000+y:y});return [...ys,...named].sort((a,b)=>b-a)[0]||new Date().getFullYear()}
function stmtDateInfo(v,anchorYear){const raw=String(v||''),up=raw.toLocaleUpperCase('tr-TR');let m=raw.match(/\b(\d{1,2})[.\/-](\d{1,2})(?:[.\/-](\d{2,4}))?\b/);if(m){let y=m[3]?+m[3]:anchorYear;if(y<100)y+=2000;const d=+m[1],mo=+m[2];if(d>=1&&d<=31&&mo>=1&&mo<=12)return{date:`${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`,raw:m[0],month:mo,yearExplicit:!!m[3]}}m=up.match(/\b(\d{1,2})\s+(OCAK|OCA|ŞUBAT|ŞUB|MART|MAR|NİSAN|NİS|MAYIS|MAY|HAZİRAN|HAZ|TEMMUZ|TEM|AĞUSTOS|AĞU|EYLÜL|EYL|EKİM|EKİ|KASIM|KAS|ARALIK|ARA)(?:\s+(\d{2,4}))?\b/);if(m){let y=m[3]?+m[3]:anchorYear;if(y<100)y+=2000;const d=+m[1],mo=STMT_MONTHS[m[2]];if(d>=1&&d<=31)return{date:`${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`,raw:m[0],month:mo,yearExplicit:!!m[3]}}return null}
function stmtCategory(t){t=String(t||'').toLocaleUpperCase('tr-TR');if(/MİGROS|MIGROS|MARKET|BİM|BIM|A101|ŞOK|SOK|CARREFOUR|METRO MARKET/.test(t))return'Market';if(/BENZİN|BENZIN|PETROL|OPET|SHELL|\bBP\b|TOTAL|AYTEMİZ|AYTEMIZ/.test(t))return'Ulaşım';if(/RESTAUR|YEMEK|CAFE|KAFE|GETİR|GETIR|YEMEKSEPETİ|YEMEKSEPETI|STARBUCKS/.test(t))return'Yeme İçme';if(/ECZANE|HASTANE|MEDİKAL|MEDIKAL|SAĞLIK|SAGLIK/.test(t))return'Sağlık';if(/FIRIN|PASTANE/.test(t))return'Fırın';if(/ELEKTRİK|ELEKTRIK|DOĞALGAZ|DOGALGAZ|SU FATURA|TURKCELL|VODAFONE|TÜRK TELEKOM|TURK TELEKOM/.test(t))return'Fatura';if(/TAKSİ|TAKSI|OTOBÜS|OTOBUS|METRO|ULAŞIM|ULASIM/.test(t))return'Ulaşım';return'Diğer'}
...
const HANE_OCR_SCRIPT='https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/tesseract.min.js';
const HANE_OCR_WORKER='https://cdn.jsdelivr.net/npm/tesseract.js@5.1.1/dist/worker.min.js';
const HANE_OCR_CORE='https://cdn.jsdelivr.net/npm/tesseract.js-core@5.1.1';
const HANE_PDF_MODULE='https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.min.mjs';
const HANE_PDF_WORKER='https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/pdf.worker.min.mjs';
let statementOcrWorker=null,statementOcrLabel='OCR';
async function loadTesseract(){if(window.Tesseract)return window.Tesseract;await new Promise((res,rej)=>{const old=document.querySelector('script[data-hane-ocr="1"]');if(old){old.addEventListener('load',res,{once:true});old.addEventListener('error',()=>rej(new Error('OCR motoru açılamadı. Uygulama güncellendikten sonra bir kez internetle açıp tekrar deneyin.')),{once:true});return}const sc=document.createElement('script');sc.dataset.haneOcr='1';sc.src=HANE_OCR_SCRIPT;sc.crossOrigin='anonymous';sc.referrerPolicy='no-referrer';sc.onload=res;sc.onerror=()=>rej(new Error('OCR motoru açılamadı. Uygulama güncellendikten sonra bir kez internetle açıp tekrar deneyin.'));document.head.appendChild(sc)});return window.Tesseract}
...
async function readStatementFile(file){
  if(file.type==='application/pdf'||/\.pdf$/i.test(file.name)){
    let pdfjs;try{pdfjs=await import(HANE_PDF_MODULE)}catch{throw new Error('PDF okuyucu açılamadı. Uygulama güncellendikten sonra bir kez internetle açıp tekrar deneyin.')}
    ...
    let ocr='';try{const w=await getStatementOcrWorker('PDF OCR');for(let n=1;n<=pages.length;n++){statementOcrLabel=`PDF SAYFA ${n}/${pages.length}`;const pg=pages[n-1],vp=pg.getViewport({scale:1.6}),canvas=document.createElement('canvas');canvas.width=Math.ceil(vp.width);canvas.height=Math.ceil(vp.height);await pg.render({canvasContext:canvas.getContext('2d'),viewport:vp}).promise;const r=await w.recognize(canvas);ocr+='\n'+(r.data.text||'')}}finally{await releaseStatementOcrWorker()}
    if(ocr.replace(/\s/g,'').length>40)return ocr;throw new Error('PDF içindeki işlem satırları okunamadı.');
  }
  try{const w=await getStatementOcrWorker('FOTOĞRAF OKUNUYOR');const r=await w.recognize(file);return r.data.text||''}finally{await releaseStatementOcrWorker()}
}
async function confirmStatementImport(){const checks=[...document.querySelectorAll('[data-stmt-check]')];let added=0,skipped=0;const localCounts={};for(const el of checks){if(!el.checked)continue;const i=+el.dataset.stmtCheck,r=statementImportRows[i];if(!r)continue;const date=document.querySelector(`[data-stmt-date="${i}"]`)?.value||r.date,title=stmtCleanTitle(document.querySelector(`[data-stmt-title="${i}"]`)?.value||r.title),amount=Number(document.querySelector(`[data-stmt-amount="${i}"]`)?.value),category=document.querySelector(`[data-stmt-category="${i}"]`)?.value||r.category;if(!date||!title||!Number.isFinite(amount)||amount===0){skipped++;continue}const base=stmtFingerprint(statementImportCardId,date,title,amount),existing=stmtExistingCount(statementImportCardId,date,title,amount);let ord;if(base===r.baseFp){ord=r.occurrence;if(existing>=ord){skipped++;continue}}else{const n=(localCounts[base]=(localCounts[base]||0)+1);ord=existing+n}const ex={id:id(),title:upper(title),amount,actualAmount:amount,date,category,source:'card',cardId:statementImportCardId,recurring:false,memberId:'',attachment:'',importBaseFingerprint:base,importFingerprint:`${base}|#${ord}`,importedFromStatement:true,importedRefund:amount<0};state.expenses.push(ex);added++}await save();modal=null;render();showToast(`${added} HARCAMA EKLENDİ${skipped?` · ${skipped} ATLANDI`:''}`)}
  ...
  try{
    ...
    if(statementImportInput){
      statementImportInput.addEventListener('change',async e=>{
        ...
        open('EKSTRE OKUNUYOR',`<div class="notice"><b id="statementImportProgress">DOSYA HAZIRLANIYOR...</b><br>Dosya HANE dışına yüklenmez. Okuma cihazında yapılır; yalnızca tarih, açıklama, tutar ve kategori kaydedilir.</div>`,{cardId:statementImportCardId});
        try{const text=await readStatementFile(f),rows=parseStatementText(text,statementImportCardId);open('EKSTRE ÖNİZLEME',statementPreview(statementImportCardId,rows),{cardId:statementImportCardId})}catch(err){console.error(err);open('EKSTRE OKUNAMADI',`<div class="notice">${esc(err.message||'Dosya okunamadı.')}</div>`,{cardId:statementImportCardId})}finally{input.value=''}
      });
    }
