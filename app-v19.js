...
const CAT_COLORS={Kira:'#d8ad4f',Aidat:'#a86ef7',Market:'#19d77d',Faturalar:'#ff8c42','İnternet':'#6ed4ff',Elektrik:'#ffd84d',Su:'#3fa9ff','Doğalgaz':'#ff6b45','Cep Telefonu':'#b879ff','Ulaşım':'#4dd6c7','Sağlık':'#ff5f78','Eğlence':'#f06dad','Diğer':'#9ca3af'};
function catPremiumIcon(cat){return `<span class="catGem" style="--cat:${CAT_COLORS[cat]||'#d8ad4f'}"><span>${I[cat]||'●'}</span></span>`}
...
function def(){return{version:19,selectedMonth:ym(new Date()),profile:{name:'',photo:'',motto:'Disiplin, özgürlüğün kapısını açar.'},settings:{lockMinutes:15,leadDays:3,notifications:false},theme:{bg:'#000000',accent:'#d8ad4f',income:'#248ef5',expense:'#ff4658',remain:'#16d77d'},incomes:[],expenses:[],cards:[],cardTransactions:[],cardPayments:[]}}
...
function settings(){return`<div class="card"><div class="setting"><span class="settingIco">${premiumIcon("bell",24)}</span><span>BİLDİRİMLER</span><b>${state.settings.notifications?'Açık':'Kapalı'}</b></div><div class="setting"><span class="settingIco">${premiumIcon("globe",24)}</span><span>DİL</span><b>Türkçe</b></div><div class="setting" data-tab="about"><span class="settingIco">${premiumIcon("info",24)}</span><span>HAKKINDA</span><b>›</b></div><div class="setting" data-action="clearAll"><span class="settingIco dangerIco">${premiumIcon("trash",24)}</span><span style="color:var(--red)">TÜM VERİLERİ SIFIRLA</span><b></b></div></div>`}
function theme(){
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 1.9.0</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
...
function expenseForm(x={}){
  const isFixed=!!x.recurring,currentCat=x.category||(isFixed?'Kira':'Market'),normalCats=C.filter(c=>!FIXED_C.includes(c)||c==='Faturalar');
...
function bind(){
  ...
  $$('.premiumCatBtn').forEach(b=>b.onclick=()=>{const grid=b.closest('.premiumCatGrid'),name=grid?.dataset.selectName,sel=document.querySelector(`select[name="${name}"]`);if(sel)sel.value=b.dataset.cat;grid?.querySelectorAll('.premiumCatBtn').forEach(x=>x.classList.toggle('active',x===b))});
...
async function saveExpense(d,i){
  ...
  const x={id:i||id(),title:upper((d.title||(fixed?'SABİT GİDER':'GİDER')).trim()||(fixed?'SABİT GİDER':'GİDER')),amount,category:fixed?(d.fixedCategory||'Kira'):(d.normalCategory||'Diğer'),date:fixed?(d.dueDate||iso()):(d.date||iso()),dueDate:fixed?(d.dueDate||iso()):null,recurring:fixed,paid:fixed?(old?.paid||false):true,sortOrder:old?.sortOrder??state.expenses.length,paidMonths:[...(old?.paidMonths||[])],source,cardId:source==='card'?d.cardId:null,attachment:modal?.attachment||old?.attachment||''};
