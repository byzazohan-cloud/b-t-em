...
function actualExpenseEntriesForMonth(m=state?.selectedMonth){
  ...
  const fixed=(state?.fixedPayments||[]).filter(p=>String(p.date||'').startsWith(m)).map(p=>{const ex=(state?.expenses||[]).find(x=>x.id===p.expenseId);return {...(ex||{}),...p,id:ex?.id||p.expenseId||p.id,paymentId:p.id,expenseId:p.expenseId,title:p.title||ex?.title||'SABİT GİDER',category:p.category||ex?.category||'Diğer',memberId:ex?.memberId||'me',recurring:true,billAmount:fixedBillAmount(ex),amount:+(p.actualAmount??p.amount??0)||0,date:p.date||ex?.dueDate||ex?.date||iso(),source:p.source||ex?.source||'cash',cardId:p.cardId||ex?.cardId||null,_actual:true}});
  return [...normal,...fixed]
}
function actualExpenseEntriesInRange(start,end){
  ...
  const fixed=(state?.fixedPayments||[]).filter(p=>String(p.date||'')>=start&&String(p.date||'')<=end).map(p=>{const ex=(state?.expenses||[]).find(x=>x.id===p.expenseId);return {...(ex||{}),...p,id:ex?.id||p.expenseId||p.id,paymentId:p.id,expenseId:p.expenseId,title:p.title||ex?.title||'SABİT GİDER',category:p.category||ex?.category||'Diğer',memberId:ex?.memberId||'me',recurring:true,billAmount:fixedBillAmount(ex),amount:+(p.actualAmount??p.amount??0)||0,date:p.date||ex?.dueDate||ex?.date||iso(),source:p.source||ex?.source||'cash',cardId:p.cardId||ex?.cardId||null,_actual:true}});
  return [...normal,...fixed]
}
function cardDerivedNet(cardId){
  const normal=(state?.expenses||[]).filter(x=>!x.recurring&&x.source==='card'&&x.cardId===cardId).reduce((a,x)=>a+(+(x.actualAmount??x.amount)||0),0);
  const fixed=(state?.fixedPayments||[]).filter(p=>p.source==='card'&&p.cardId===cardId).reduce((a,p)=>a+(+(p.actualAmount??p.amount)||0),0);
  const paid=(state?.cardPayments||[]).filter(p=>p.cardId===cardId).reduce((a,p)=>a+(+p.amount||0),0);
  return normal+fixed-paid
}
function syncDerivedCardTransactions(){
  const out=[];
  (state?.expenses||[]).filter(x=>!x.recurring&&x.source==='card'&&x.cardId).forEach(x=>{if(!x.cardTxId)x.cardTxId=id();const c=state.cards.find(c=>c.id===x.cardId);out.push({id:x.cardTxId,expenseId:x.id,cardId:x.cardId,amount:+(x.actualAmount??x.amount)||0,totalAmount:x.totalAmount,title:x.title,baseTitle:x.baseTitle,category:x.category,date:x.date,purchaseDate:x.purchaseDate||x.date,statementMonth:c?statementMonthFor(c,x.date):String(x.date||'').slice(0,7),attachment:x.attachment||'',installmentGroup:x.installmentGroup,installmentNo:x.installmentNo,installmentCount:x.installmentCount,derived:true})});
...
function recalculateFinanceCore(){
  ...
  (state.cards||[]).forEach(c=>{if(!Number.isFinite(+c.openingBalance))c.openingBalance=+c.balance||0;c.balance=Math.max(0,(+c.openingBalance||0)+cardDerivedNet(c.id))})
}
...
function summaryDetailBody(kind){const m=state.selectedMonth;const inc=state.incomes.filter(x=>String(x.date||'').startsWith(m)).map(x=>({...x,_kind:'income'}));const exp=actualExpenseEntriesForMonth(m).map(x=>({...x,_kind:'expense'}));const items=(kind==='income'?inc:kind==='expense'?exp:[...inc,...exp]).sort((a,b)=>String(b.date).localeCompare(String(a.date)));const T=totals(m);const title=kind==='income'?'GELİR DETAYI':kind==='expense'?'GİDER DETAYI':'KALAN DETAYI';return `<div class="summaryDetailBox"><div class="summaryDetailHead"><small>${m}</small><h2>${kind==='income'?money(T.i):kind==='expense'?money(T.e):money(T.r)}</h2><b>${title}</b></div>${kind==='remain'?`<div class="summaryBreakdown"><span>Gelir <b style="color:var(--green)">${money(T.i)}</b></span><span>Gider <b style="color:var(--red)">${money(T.e)}</b></span><span>Kalan <b style="color:var(--gold2)">${money(T.r)}</b></span></div>`:''}<div class="list">${items.length?items.map(x=>`<div class="item"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):x._kind==='flexPayment'?premiumIcon('cards',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x._kind==='income'?'GELİR':esc(x.category||'DİĞER')}${x._kind==='expense'&&x.recurring?' · '+(fixedPaidForMonth(x,m)?'ÖDENDİ':'ÖDENMEDİ'):''}</small></div><div class="reportMoveRight"><b style="color:${x._kind==='income'||x._kind==='flexPayment'?'var(--green)':'var(--red)'}">${x._kind==='income'||x._kind==='flexPayment'?'+':'-'}${money(x.amount)}</b><div><button data-action="${x._kind==='income'?'editIncome':x._kind==='flexPayment'?'editFlexPayment':'editExpense'}" data-id="${x.id}">DETAY</button><button class="danger" data-action="${x._kind==='income'?'delIncome':x._kind==='flexPayment'?'delFlexPayment':'delExpense'}" data-id="${x.id}">SİL</button></div></div></div>`).join(''):'<div class="notice">KAYIT YOK.</div>'}</div></div>`}
...
function paymentSourceDetailBody(source){const s=paymentSourceStats(),items=(source==='card'?s.card:s.cash).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||''))),total=source==='card'?s.cardTotal:s.cashTotal;return `<div class="statementHero"><small>${state.selectedMonth} · ${source==='card'?'KREDİ KARTI':'NAKİT'}</small><h2>${money(total)}</h2><b>${items.length} HAREKET</b></div><div class="list">${items.length?items.map(x=>{const c=x.cardId?state.cards.find(z=>z.id===x.cardId):null,action=x.recurring?'editFixedPayment':'editExpense',rid=x.recurring?(x.expenseId||x.id):x.id;return `<div class="item" data-action="${action}" data-id="${rid}" data-direct-edit="1"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date||''} · ${esc(x.category||'Diğer')} · ${source==='card'?'KREDİ KARTI'+(c?' • '+esc(c.bank)+' '+esc(c.name):''):'NAKİT'}</small></div><div class="right"><b style="color:var(--red)">-${money(x.amount)}</b><small class="tapDetailHint">DÜZENLE ›</small></div></div>`}).join(''):'<div class="notice">BU AY HAREKET YOK.</div>'}</div>`}
function cardStatementItems(cardId,m){
  const expenses=(state.expenses||[]).filter(x=>!x.recurring&&x.source==='card'&&x.cardId===cardId&&String(x.date||'').startsWith(m)).map(x=>({kind:'spend',id:x.id,date:x.date,title:x.title,category:x.category,amount:+(x.actualAmount??x.amount)||0,action:'editExpense'}));
  const fixed=(state.fixedPayments||[]).filter(p=>p.source==='card'&&p.cardId===cardId&&String(p.date||'').startsWith(m)).map(p=>({kind:'fixed',id:p.id,date:p.date,title:p.title||'SABİT GİDER',category:p.category,amount:+(p.actualAmount??p.amount)||0,action:'editStatementFixedPayment'}));
  const pays=(state.cardPayments||[]).filter(p=>p.cardId===cardId&&String(p.date||'').startsWith(m)).map(p=>({kind:'payment',id:p.id,date:p.date,title:'KART ÖDEMESİ',category:'Ödeme',amount:+p.amount||0,action:'editCardPayment'}));
  return [...expenses,...fixed,...pays].sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
}
function cardStatementBody(cardId,m=cardStatementMonth||state.selectedMonth){const c=state.cards.find(x=>x.id===cardId);if(!c)return '<div class="notice">KART BULUNAMADI.</div>';const items=cardStatementItems(cardId,m),spent=items.filter(x=>x.kind!=='payment').reduce((n,x)=>n+x.amount,0),paid=items.filter(x=>x.kind==='payment').reduce((n,x)=>n+x.amount,0);return `<div class="statementHead"><button data-action="cardStatementShift" data-dir="-1" data-id="${c.id}">‹</button><div><small>AYLIK EKSTRE</small><b>${m}</b></div><button data-action="cardStatementShift" data-dir="1" data-id="${c.id}">›</button></div><div class="statementHero"><small>${esc(c.bank)} · ${esc(c.name)} · •••• ${esc(c.last4)}</small><h2>${money(c.balance)}</h2><b>KALAN GÜNCEL BORÇ</b></div><div class="statementTotals"><span>HARCAMALAR <b>${money(spent)}</b></span><span>ÖDEMELER <b>${money(paid)}</b></span><span>AYLIK FARK <b>${money(spent-paid)}</b></span></div><div class="list">${items.length?items.map(x=>`<div class="item statementItem" data-action="${x.action}" data-id="${x.id}" data-direct-edit="1"><div class="ico premiumIco">${x.kind==='payment'?premiumIcon('cards',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x.kind==='payment'?'ÖDEME':x.kind==='fixed'?'SABİT GİDER':'HARCAMA'}${x.category?' · '+esc(x.category):''}</small></div><div class="right"><b style="color:${x.kind==='payment'?'var(--green)':'var(--red)'}">${x.kind==='payment'?'-':'+'}${money(x.amount)}</b><small class="tapDetailHint">DÜZENLE / SİL ›</small></div></div>`).join(''):'<div class="notice">BU AY BU KARTTA HAREKET YOK.</div>'}</div>`}
...
function row(x){
  ...
  return`<div class="item" data-action="${x.type==='income'?'editIncome':'editExpense'}" data-id="${x.id}"><div class="ico">${x.type==='income'?premiumIcon('income',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date}${x.recurring?' · Sabit':''}${x.source==='card'?' · Kart harcaması':''}</small></div><div class="right"><b style="color:${x.type==='income'?'var(--green)':'var(--red)'}">${x.type==='income'?'+':'-'}${money(x.amount)}</b></div></div>`
}
...
function fixedCategoryDetail(cat){
  ...
  return `<div class="fixedCatDetail"><div class="fixedCatDetailHero"><div><small>${esc(range.label)} · KATEGORİ</small><h2>${esc(cat)}</h2></div><strong>${money(total)}</strong></div><div class="section"><b>DÖNEM GİDER HAREKETLERİ</b><span>${items.length} KAYIT</span></div><div class="list">${items.length?items.map(x=>{const fixed=!!x.recurring,card=x.cardId?state.cards.find(c=>c.id===x.cardId):null;return `<div class="item fixedCatDetailItem" data-action="${fixed?'editFixedPayment':'editExpense'}" data-id="${fixed?(x.expenseId||x.id):x.id}"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div class="fixedCatDetailMain"><b>${esc(x.title)}</b><small>${x.date||''} · ${fixed?'SABİT GİDER':'NORMAL GİDER'} · ${x.source==='card'?'KART'+(card?' · '+esc(card.bank):''):'NAKİT'}</small></div><div class="fixedCatDetailRight"><strong>${money(x.amount)}</strong><small class="tapDetailHint">AYRINTI ›</small></div></div>`}).join(''):'<div class="notice">BU DÖNEM BU KATEGORİDE GİDER YOK.</div>'}</div></div>`
}
function fixed(){
  ...
  const catHtml=cats.length?`<div class="fixedCategoryTotals">${cats.map(([cat,total])=>`<button type="button" class="fixedCatTotal" data-action="fixedCategoryDetail" data-cat="${esc(cat)}"><span><i>${catPremiumIcon(cat)}</i><b>${esc(cat)}</b></span><strong>${money(total)}</strong></button>`).join('')}<div class="fixedCatGrand"><span>TOPLAM GİDER</span><strong>${money(catTotal)}</strong></div></div>`:'<div class="notice">KATEGORİ TOPLAMI İÇİN GİDER KAYDI YOK.</div>';
  ...
  return`${monthNavigator()}${paymentSourceSummary()}<div class="section"><b>GİDERLER</b><button class="miniAddBtn" data-action="addExpense">+ GİDER EKLE</button></div><div class="section fixedCatHead"><b>${monthLabel(m)} · KATEGORİ TOPLAMLARI</b><span>${cats.length} KATEGORİ</span></div>${catHtml}<div class="section"><b>NORMAL GİDERLER</b><span>${normalItems.length} KAYIT</span></div><div class="list fixedList">${normalHtml}</div><div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ SABİT GİDER</button></div><div class="notice fixedSortNote">ÖDENMEMİŞ SABİT GİDERLER ÜSTTE · ÖDENMİŞLER ALTTA</div><div class="list fixedList">${fixedHtml}</div>`
}
...
function reports(){
 ...
 const row=x=>{let label='',edit='',idv=x.id,color='var(--red)',sign='-';if(x._kind==='income'){label='GELİR';edit='editIncome';color='var(--green)';sign='+'}else if(x._kind==='expense'){label=(x.source==='card'?'KART HARCAMASI'+(x.installmentCount>1?' · '+x.installmentNo+'/'+x.installmentCount+' TAKSİT':''):x.source==='flex'?'ESNEK HESAP HARCAMASI':'GİDER')+' · '+(x.category||'Diğer');edit='editExpense'}else if(x._kind==='fixedPayment'){label='SABİT GİDER · ÖDENDİ';edit='editFixedPayment';idv=x.expenseId;color='var(--gold2)';sign=''}else if(x._kind==='cardPayment'){label='KART ÖDEMESİ';edit='editCardPayment';color='#35a7ff';sign=''}else{label='ESNEK HESAP ÖDEMESİ';edit='editFlexPayment';color='#29d3c2';sign=''}return `<div class="item reportTapItem" role="button" tabindex="0" data-action="${edit}" data-id="${idv}"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):x._kind==='expense'?catPremiumIcon(x.category):premiumIcon('cards',22)}</div><div><b>${esc(x.title||label)}</b><small>${x.date} · ${label}</small></div><div class="reportMoveRight"><b style="color:${color}">${sign}${money(x.amount)}</b><small>AYRINTI ›</small></div></div>`};
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.8 · G3 KART / NAKİT / EKSTRE</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
function members(){return `<div class="section"><b>HANE ÜYELERİ</b><button class="miniAddBtn" data-action="addMember">+ ÜYE EKLE</button></div><div class="list">${state.members.map(m=>`<div class="item"><div class="ico">${esc(m.icon)}</div><div><b>${esc(m.name)}</b><small>Gelir / gider / rapor filtresinde kullanılabilir</small></div><button class="tinyEdit" data-action="editMember" data-id="${m.id}">DETAY</button></div>`).join('')}</div>`}
...
function memberSelect(v='me'){return `<div class="field"><label>Hane Üyesi</label><select name="memberId">${state.members.map(m=>`<option value="${m.id}" ${v===m.id?'selected':''}>${esc(m.icon)} ${esc(m.name)}</option>`).join('')}</select></div>`}
...
function bind(){
  ...
  const fixedPaymentFormEl=$('#fixedPaymentForm');if(fixedPaymentFormEl)fixedPaymentFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),p=(state.fixedPayments||[]).find(p=>p.id===modal?.paymentId),ex=state.expenses.find(x=>x.id===modal?.id);if(!p)return;const nextAmount=Number(String(d.amount||'0').replace(',','.')),nextSource=d.source==='card'?'card':'cash';if(!Number.isFinite(nextAmount)||nextAmount<0)return alert('Geçerli tutar girin.');if(nextSource==='card'&&!d.cardId)return alert('Kart seçin.');p.amount=nextAmount;p.actualAmount=nextAmount;p.date=d.date||p.date||iso();p.category=d.category||ex?.category||'Diğer';p.source=nextSource;p.cardId=nextSource==='card'?d.cardId:null;if(ex){ex.category=p.category;(state.fixedPayments||[]).filter(fp=>fp.expenseId===ex.id).forEach(fp=>fp.category=ex.category||'Diğer')}await save();modal=null;render();showToast('ÖDEME GÜNCELLENDİ')};
...
async function saveExpense(d,i){
  ...
  const x={id:i||id(),title:upper((d.title||(fixed?'SABİT GİDER':'GİDER')).trim()||(fixed?'SABİT GİDER':'GİDER')),amount,actualAmount:fixed?null:amount,billAmount:fixed?amount:null,category:fixed?(d.fixedCategory||'Kira'):((d.normalCategory==='Diğer'&&(d.customCategory||'').trim())?upper(d.customCategory.trim()):(d.normalCategory||'Diğer')),date:fixed?(d.dueDate||iso()):(d.date||iso()),dueDate:fixed?(d.dueDate||iso()):null,recurring:fixed,paid:fixed?(old?.paid||false):true,sortOrder:old?.sortOrder??state.expenses.length,paidMonths:[...(old?.paidMonths||[])],source,memberId:d.memberId||old?.memberId||'me',cardId:source==='card'?d.cardId:null,attachment:modal?.attachment||old?.attachment||'',cardTxId:fixed?null:(old?.cardTxId||id())};
  const isNew=!i;
...
async function saveCardSpend(d,cardId){
  ...
  for(let n=1;n<=count;n++){
    ...
    state.expenses.push({id:id(),cardTxId:txId,source:'card',cardId,title:label,baseTitle:title,amount:part,actualAmount:part,totalAmount:amount,category,date:partDate,dueDate:partDate,recurring:false,paid:true,installmentGroup:groupId,installmentNo:n,installmentCount:count});
  }
  ...
  try{
    ...
    if(receiptInput){
      receiptInput.addEventListener('change',e=>{
        ...
        readImg(f,async x=>{if(modal?.id){const ex=state?.expenses?.find(z=>z.id===modal.id);if(ex){ex.attachment=x;await save();modal=null;render();return}}modal.attachment=x;render()},1280);
      });
    }
    if(restoreInput){
