...
function credit(c){const pi=cardPaymentInfo(c);return `<div class="credit card luxuryCard clickableCredit ${c.style||'blackgold'}" data-action="cardStatement" data-id="${c.id}" data-card-id="${c.id}" role="button" tabindex="0"><div class="haneCardBrand"><img src="icons/hane-app-icon.png" alt="HANE"><div><strong>HANE</strong><small>DAHA DÜZENLİ BİR YAŞAM</small></div></div><div class="cardTop"><b>${esc(c.bank)}</b><span>${esc(c.network||'KREDİ KARTI')}</span></div><small class="cardName">${esc(c.name)}</small><div class="chip"></div><div class="digits">•••• •••• •••• ${esc(c.last4)}</div><div class="grid"><div><small>TOPLAM LİMİT</small><b>${money(c.limit)}</b></div><div><small>KULLANILABİLİR LİMİT</small><b>${money(Math.max(0,(+c.limit||0)-(+c.balance||0)))}</b></div><div><small>HESAP KESİM TARİHİ</small><b>${pi.statementDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})}</b></div><div><small>SON ÖDEME TARİHİ</small><b>${pi.dueDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})}</b></div></div><div class="cardActions"><button class="btn" data-action="cardSpend" data-id="${c.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="cardPay" data-id="${c.id}">₺ ÖDEME YAPTIM</button><button class="btn statementBtn" data-action="cardStatement" data-id="${c.id}">≡ EKSTRE</button><button class="iconEdit" data-action="editCard" data-id="${c.id}">✎</button></div></div>`}
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4.8 · G4 HANE ÜYESİ SİSTEMİ</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
...
function memberDetailBody(memberId,m=state.selectedMonth){
  ...
  return `<div class="memberDetailSummary"><small>${monthLabel(m)} TOPLAM HARCAMA</small><b>${money(total)}</b><span>${rows.length} hareket</span></div>${rows.length?`<div class="list memberMovementList">${rows.map(x=>{const card=x.cardId?state.cards.find(c=>c.id===x.cardId):null;const pay=x.source==='card'?`KREDİ KARTI${card?' · '+esc(card.name):''}`:'NAKİT';const action=x.recurring?'editFixedExpense':'editExpense';return `<div class="item memberMove"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div><b>${esc(x.title||x.category||'GİDER')}</b><small>${x.date||''} · ${esc(x.category||'Diğer')} · ${pay}</small></div><div class="right"><b>${money(x.amount)}</b><div class="memberRowActions"><button data-action="${action}" data-direct-edit="1" data-id="${x.id}">DÜZENLE</button><button data-action="delExpense" data-id="${x.id}">SİL</button></div></div></div>`}).join('')}</div>`:'<div class="notice">Bu ay bu kişiye atanmış gider yok.</div>'}`;
}
function members(){
...
async function saveExpense(d,i){
  ...
  const x={id:i||id(),title:upper((d.title||(fixed?'SABİT GİDER':'GİDER')).trim()||(fixed?'SABİT GİDER':'GİDER')),amount,actualAmount:fixed?null:amount,billAmount:fixed?amount:null,category:fixed?(d.fixedCategory||'Kira'):((d.normalCategory==='Diğer'&&(d.customCategory||'').trim())?upper(d.customCategory.trim()):(d.normalCategory||'Diğer')),date:fixed?(d.dueDate||iso()):(d.date||iso()),dueDate:fixed?(d.dueDate||iso()):null,recurring:fixed,paid:fixed?(old?.paid||false):true,sortOrder:old?.sortOrder??state.expenses.length,paidMonths:[...(old?.paidMonths||[])],source,memberId:(d.memberId??old?.memberId??''),cardId:source==='card'?d.cardId:null,attachment:modal?.attachment||old?.attachment||'',cardTxId:fixed?null:(old?.cardTxId||id())};
  const isNew=!i;
