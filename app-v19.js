...
const HANE_UI=Object.freeze({
  build:'20260923-V113-S18-MUTABAKAT',
...
function cardStatementBody(cardId,m=cardStatementMonth||state.selectedMonth){
  ...
  const bankBox=snap?`<div class="statementBankSummary simpleBankStatement"><b>BANKA EKSTRESİ</b><div class="statementEquation"><span><small>DEVREDEN</small><b>${prev==null?'-':money(prev)}</b></span><i>+</i><span><small>HARCAMALAR</small><b>${bankSpend==null?'-':money(bankSpend)}</b></span><i>+</i><span><small>FAİZ / ÜCRET</small><b>${money(fees)}</b></span><i>−</i><span><small>ÖDEMELER</small><b>${bankPays==null?'-':money(bankPays)}</b></span><i>=</i><span class="debt"><small>DÖNEM BORCU</small><b>${bankDebt==null?'-':money(bankDebt)}</b></span></div></div><div class="statementCompare"><div><small>HANE'DE BULUNAN HARCAMA / İADE</small><b>${foundCount} işlem · ${money(haneNet)}</b></div><div><small>HANE'DE BULUNAN KART ÖDEMESİ</small><b>${payCount} işlem · ${money(paid)}</b></div>${snap?`<div class="stmtCountGrid compact"><span><small>BANKA İŞLEM</small><b>${snap.bankRowCount??'-'}</b></span><span><small>EŞLEŞEN</small><b>${snap.matchedExistingCount??0}</b></span><span><small>YENİ</small><b>${snap.newSpendCount??snap.rowCount??0}</b></span><span><small>FAİZ/MASRAF</small><b>${snap.feeRowCount??0}</b></span></div>`:''}${diff!=null&&Math.abs(diff)>.01?`<p>${diff>0?'HANE’de fazla okunan':'HANE’de eksik okunan'} harcama: <b>${money(Math.abs(diff))}</b></p>`:`<p>Ekstre harcama toplamı ile HANE uyumlu.</p>`}${debtDiff!=null?`<p>HANE hesaplanan borç: <b>${money(calcDebt)}</b> · Banka borcu: <b>${money(bankDebt)}</b> · ${Math.abs(debtDiff)<=.01?'✓ UYUMLU':`Fark: <b>${money(Math.abs(debtDiff))}</b>`}</p>`:''}</div>`:'';
...
function stmtMatchSelect(row,i){
  ...
  const cs=stmtManualMatchCandidates(statementImportCardId,row).slice(0,4),opts=cs.map(z=>`<option value="${esc(z.x.id)}" ${row.matchStatus==='matched'&&row.matchId===z.x.id?'selected':''}>EŞLEŞTİR · ${esc(z.x.title)} · ${z.x.date} · ${money(Math.abs(+(z.x.actualAmount??z.x.amount)||0))}</option>`).join('');
  return `<select data-stmt-match="${i}"><option value="">YENİ İŞLEM OLARAK EKLE</option>${opts}</select><small>${row.matchStatus==='matched'?'✓ Mevcut HANE kaydı bulundu':row.matchStatus==='possible'?'⚠ Olası eşleşme — kullanıcı onayı gerekli':'Yeni banka hareketi'}${row.matchReason?' · '+esc(row.matchReason):''}</small>`
}
...
const HANE_SW_BUILD='20260923-V113-S18-MUTABAKAT';
const HANE_SW_URL='./sw.js?v='+encodeURIComponent(HANE_SW_BUILD);
const HANE_ENGINE_CACHE='hane-engine-19.4.24-V113-S18-MUTABAKAT';
