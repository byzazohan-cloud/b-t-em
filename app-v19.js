...
function fixed(){
  ...
  return`<div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ GİDER EKLE</button></div><div class="notice">ÖDENMEMİŞ GİDERLER ÜSTTE · ÖDENMİŞ GİDERLER ALTTA</div><div class="list fixedList">${a.length?a.map(x=>{const paid=fixedPaidForMonth(x),pay=fixedPaymentFor(x);return`<div class="item fixedItem ${paid?'isPaid':''}" data-fixed-id="${x.id}"><div class="ico premiumIco" data-action="editFixedExpense" data-id="${x.id}">${I[x.category]||'●'}</div><div data-action="editFixedExpense" data-id="${x.id}"><b>${esc(x.title)}</b><small>${x.dueDate||x.date} · SABİT${paid?` · ÖDENDİ ${pay?.date||''} · ${pay?.source==='card'?'KART':'NAKİT'}`:''}</small></div><div class="fixedRight"><b style="color:${paid?'var(--green)':'var(--red)'}">${paid?'✓ ':'-'}${money(x.amount)}</b><button type="button" class="paidToggle ${paid?'on':''}" data-action="toggleFixedPaid" data-id="${x.id}">${paid?'↶ GERİ AL':'ÖDEDİM'}</button>${paid?`<button type="button" class="tinyEdit" data-action="editFixedPayment" data-id="${x.id}">ÖDEMEYİ DÜZENLE</button>`:''}</div></div>`}).join(''):'<div class="notice">SABİT GİDER YOK.</div>'}</div>`
}
