...
function transactions(){
  ...
  let a=[...state.incomes.map(x=>({...x,type:'income'})),...state.expenses.filter(x=>!x.recurring).map(x=>({...x,type:'expense'})),...state.expenses.filter(x=>x.recurring).map(x=>({...x,type:'fixedExpense',date:fixedPaymentDate(x,m),_paid:fixedPaidForMonth(x,m)})),...(state.cardPayments||[]).map(x=>({...x,type:'cardPayment',source:'card'})),...(state.flexTransactions||[]).map(x=>({...x,type:x.kind==='pay'?'flexPayment':'flexSpend',source:'flex'}))].filter(x=>String(x.date||'').startsWith(m));
  ...
  const q=txSearch.trim().toLocaleLowerCase('tr-TR');if(q)a=a.filter(x=>[x.title,x.category,x.bank].some(v=>String(v||'').toLocaleLowerCase('tr-TR').includes(q)));
  ...
  const rr=x=>{const mem=state.members.find(m=>m.id===(x.memberId||'me'));if(x.type==='fixedExpense')return `<div class="item ${x._paid?'isPaid':''}"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · SABİT GİDER · ${x._paid?'ÖDENDİ':'ÖDENMEDİ'} · ${esc(mem?.name||'BEN')}</small></div><div class="right"><b style="color:${x._paid?'var(--green)':'var(--red)'}">-${money(x.amount)}</b><button class="tinyEdit" data-action="editFixedExpense" data-id="${x.id}">DÜZENLE</button></div></div>`;if(x.type==='flexPayment'||x.type==='flexSpend')return `<div class="item"><div class="ico premiumIco">${premiumIcon('cards',22)}</div><div><b>${esc(x.title||'ESNEK HESAP')}</b><small>${x.date} · ${x.type==='flexPayment'?'ESNEK HESAP ÖDEMESİ':'ESNEK HESAP HARCAMASI'}</small></div><div class="right"><b>${money(x.amount)}</b></div></div>`;return row(x)};
  return `<div class="seg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Kart')}</div><div class="advancedFilters"><input id="txSearch" placeholder="Ara..." value="${esc(txSearch)}"><input id="txDate" type="date" value="${txDate}"><select id="txCategory"><option value="">Tüm kategoriler</option>${C.map(c=>`<option ${txCategory===c?'selected':''}>${esc(c)}</option>`).join('')}</select><select id="txPay"><option value="">Tüm ödeme türleri</option><option value="cash" ${txPay==='cash'?'selected':''}>Nakit</option><option value="card" ${txPay==='card'?'selected':''}>Kart</option><option value="flex" ${txPay==='flex'?'selected':''}>Esnek Hesap</option></select><input id="txMin" type="number" placeholder="Min ₺" value="${txMin}"><input id="txMax" type="number" placeholder="Max ₺" value="${txMax}"><select id="txMember"><option value="">Tüm üyeler</option>${state.members.map(m=>`<option value="${m.id}" ${txMember===m.id?'selected':''}>${esc(m.icon)} ${esc(m.name)}</option>`).join('')}</select><button class="btn" data-action="clearTxFilters">FİLTRELERİ TEMİZLE</button></div><div class="section"><b>${state.selectedMonth}</b><span>${a.length} KAYIT</span></div><div class="list">${a.length?a.map(rr).join(''):'<div class="notice">Bu filtrede hareket yok.</div>'}</div>`
}
function fixed(){
