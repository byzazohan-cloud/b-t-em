...
let C=['Kira','Aidat','Market','Manav','Fırın','Harçlık','Kafe','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Faturalar','İnternet','Elektrik','Su','Doğalgaz','Cep Telefonu','Diğer'];
let NORMAL_C=['Market','Manav','Fırın','Harçlık','Kafe','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Diğer'];
...
function normalizeV19(st){
  ...
  C=[...new Set(['Kira','Aidat','Market','Manav','Fırın','Harçlık','Kafe','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Faturalar','İnternet','Elektrik','Su','Doğalgaz','Cep Telefonu','Diğer',...st.customCategories])];NORMAL_C=[...new Set(['Market','Manav','Fırın','Harçlık','Kafe','Yemek','Restoran','Giyim','Akaryakıt','Ulaşım','Sağlık','Eğitim','Ev','Temizlik','Eğlence','Diğer',...st.customCategories])];FIXED_C=[...new Set(['Kira','Aidat','Faturalar','İnternet','Elektrik','Su','Doğalgaz','Cep Telefonu',...st.customCategories])];
...
function summaryDetailBody(kind){
  ...
  const rows=items.map(x=>{
    ...
    return `<div class="item"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${x._kind==='income'?'GELİR':esc(x.category||'DİĞER')}${fixed?' · ÖDENDİ':''}</small></div><div class="reportMoveRight"><b style="color:${x._kind==='income'?'var(--green)':'var(--red)'}">${x._kind==='income'?'+':'-'}${money(x.amount)}</b><div><button data-action="${edit}" data-direct-edit="1" data-id="${rid}">DETAY</button><button class="danger" data-action="${del}" data-id="${rid}">SİL</button></div></div></div>`;
  }).join('');
...
function monthSpent(){
  ...
  return`<div class="monthDetailPage"><div class="detailHero card"><div><small>${monthLabel(m)}</small><h2>${money(total)}</h2><span>TOPLAM HARCAMA</span></div><div class="detailDonut" style="background:${total?`conic-gradient(${segs})`:'#151515'}"><div><b>${items.length}</b><small>İŞLEM</small></div></div></div><div class="detailMiniGrid"><div class="detailMini"><i>◷</i><span>GÜNLÜK ORTALAMA</span><b>${money(avg)}</b></div><div class="detailMini"><i>▦</i><span>KATEGORİ SAYISI</span><b>${cats.length}</b></div></div><div class="section"><b>KATEGORİLER</b><span></span></div><div class="detailCategoryList">${cats.length?cats.map((x,i)=>{const p=total?Math.round(x[1]/total*100):0;return`<div class="detailCat"><div class="detailCatIcon">${catPremiumIcon(x[0])}</div><div class="detailCatMid"><div><b>${esc(x[0])}</b><span>%${p}</span></div><div class="detailBar"><i style="width:${p}%;background:${palette[i%palette.length]}"></i></div></div><strong>${money(x[1])}</strong></div>`}).join(''):'<div class="notice">BU AY HARCAMA YOK.</div>'}</div><div class="section"><b>HARCAMALAR</b><span>${items.length} İŞLEM</span></div><div class="list detailTxList">${items.length?items.map(x=>{const fixed=x.recurring&&x.paymentId,action=fixed?'editStatementFixedPayment':'editExpense',rid=fixed?x.paymentId:x.id;return `<div class="item" data-action="${action}" data-direct-edit="1" data-id="${rid}"><div class="ico premiumIco">${catPremiumIcon(x.category)}</div><div><b>${esc(x.title)}</b><small>${x.date} · ${esc(x.category||'DİĞER')}${fixed?' · SABİT GİDER':''}</small></div><div class="right"><b style="color:var(--red)">-${money(x.amount)}</b></div></div>`}).join(''):'<div class="notice">BU AY HARCAMA YOK.</div>'}</div></div>`
}
function monthPaid(){
...
function credit(c){const pi=cardPaymentInfo(c),available=Math.max(0,(+c.limit||0)-(+c.balance||0));return `<div class="credit card luxuryCard clickableCredit haneVerticalCard ${c.style||'blackgold'}" data-action="cardStatement" data-id="${c.id}" data-card-id="${c.id}" role="button" tabindex="0"><div class="hvcGlow"></div><div class="hvcBrand"><img src="icons/hane-app-icon.png" alt="HANE"><div><strong>HANE</strong><small>DAHA DÜZENLİ BİR YAŞAM</small></div></div><div class="hvcHead"><div><b>${esc(c.bank)}</b><small>${esc(c.name||'KREDİ KARTI')}</small></div><span>${esc(c.network||'KREDİ KARTI')}</span></div><div class="hvcChip"></div><div class="hvcDigits">•••• •••• •••• <b>${esc(c.last4)}</b></div><div class="hvcGrid"><div><small>TOPLAM LİMİT</small><b>${money(c.limit)}</b></div><div><small>KULLANILABİLİR LİMİT</small><b>${money(available)}</b></div><div><small>HESAP KESİM TARİHİ</small><b>${pi.statementDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'}).toLocaleUpperCase('tr-TR')}</b></div><div><small>SON ÖDEME TARİHİ</small><b>${pi.dueDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'}).toLocaleUpperCase('tr-TR')}</b></div></div><div class="hvcFooter"><span>${esc(c.bank)} · •${esc(c.last4)}</span><small>DOKUN · EKSTRE</small></div></div>`}
...
function calendar(){
 ...
 const row=x=>{const map={income:['GELİR','var(--green)','editIncome','delIncome'],expense:['GİDER','var(--red)','editExpense','delExpense'],fixedPayment:['SABİT GİDER ÖDEMESİ','var(--gold2)','editStatementFixedPayment','delFixedPaymentExact'],cardPayment:['KART ÖDEMESİ','#35a7ff','editCardPayment','delCardPayment'],flexSpend:['ESNEK HESAP HARCAMASI','#b66cff','editExpense','delExpense'],flexPayment:['ESNEK HESAP ÖDEMESİ','#29d3c2','editFlexPayment','delFlexPayment']},z=map[x._kind],idv=x._kind==='fixedPayment'?x.id:x._kind==='flexSpend'?x.expenseId:x.id,c=x.cardId?state.cards.find(q=>q.id===x.cardId):null,member=x.memberId?memberName(x.memberId):'',pay=x._kind==='expense'||x._kind==='fixedPayment'?(x.source==='card'?'KREDİ KARTI'+(c?' • '+c.bank+' '+c.name:''):'NAKİT'):'';return `<div class="calendarMove ${x._kind}"><i></i><div><b>${esc(x.title||z[0])}</b><small>${z[0]}${x.category?' · '+esc(x.category):''}${pay?' · '+esc(pay):''}${member?' · '+esc(member):''}</small></div><strong style="color:${z[1]}">${x._kind==='income'?'+':x._kind==='expense'||x._kind==='flexSpend'?'-':''}${money(x.amount)}</strong><div class="calActions"><button data-action="${z[2]}" data-id="${idv}" data-direct-edit="1">DÜZENLE</button><button class="danger" data-action="${z[3]}" data-id="${idv}">SİL</button></div></div>`};
...
function reports(){
 ...
 const row=x=>{let label='',edit='',idv=x.id,color='var(--red)',sign='-';if(x._kind==='income'){label='GELİR';edit='editIncome';color='var(--green)';sign='+'}else if(x._kind==='expense'){label=(x.source==='card'?'KART HARCAMASI'+(x.installmentCount>1?' · '+x.installmentNo+'/'+x.installmentCount+' TAKSİT':''):x.source==='flex'?'ESNEK HESAP HARCAMASI':'GİDER')+' · '+(x.category||'Diğer');edit='editExpense'}else if(x._kind==='fixedPayment'){label='SABİT GİDER · ÖDENDİ';edit='editStatementFixedPayment';idv=x.id;color='var(--gold2)';sign=''}else if(x._kind==='cardPayment'){label='KART ÖDEMESİ';edit='editCardPayment';color='#35a7ff';sign=''}else{label='ESNEK HESAP ÖDEMESİ';edit='editFlexPayment';color='#29d3c2';sign=''}return `<div class="item reportTapItem" role="button" tabindex="0" data-action="${edit}" data-id="${idv}"><div class="ico premiumIco">${x._kind==='income'?premiumIcon('income',22):x._kind==='expense'?catPremiumIcon(x.category):premiumIcon('cards',22)}</div><div><b>${esc(x.title||label)}</b><small>${x.date} · ${label}</small></div><div class="reportMoveRight"><b style="color:${color}">${sign}${money(x.amount)}</b><small>AYRINTI ›</small></div></div>`};
  ...
  try{
    ...
    if(receiptInput){
      receiptInput.addEventListener('change',e=>{
        ...
        readImg(f,x=>{if(!modal)return;rememberModalForm();modal.attachment=x;const st=document.getElementById('expenseReceiptStatus');if(st)st.style.display='block';const btn=document.getElementById('expenseReceiptBtn');if(btn)btn.textContent='✓ FİŞ / FOTOĞRAF HAZIR';input.value=''},1280);
      });
    }
    if(restoreInput){
