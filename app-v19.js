...
let navHistory=['home'],calendarDate=new Date(),customStart=iso(new Date(new Date().getFullYear(),new Date().getMonth(),1)),customEnd=iso(),txSearch='';
...
function bind(){
  bindFixedSorting();
  $$('[data-tab]').forEach(x=>x.onclick=()=>{const next=x.dataset.tab;if(next==='theme')themeDraft={...(state.theme||{})};else if(current==='theme')themeDraft=null;if(next!==current)navHistory.push(current);current=next;modal=null;render()});
