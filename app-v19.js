...
function transactions(){
  ...
  // Arama varsa tüm geçmişte ara; arama yoksa seçili ayı göster.
  ...
  if(q)a=a.filter(x=>{const card=state.cards.find(c=>c.id===x.cardId),flex=state.flexAccounts.find(f=>f.id===x.flexId),mem=state.members.find(mm=>mm.id===(x.memberId||'me'));return [x.title,x.category,x.bank,x.source,x.type,card?.bank,card?.name,flex?.bank,flex?.name,mem?.name].some(v=>String(v||'').toLocaleLowerCase('tr-TR').includes(q))});
  if(txDate)a=a.filter(x=>x.date===txDate);if(txCategory)a=a.filter(x=>x.category===txCategory);if(txPay)a=a.filter(x=>(txPay==='cash'?(x.source||'cash')==='cash':txPay==='card'?x.source==='card'||x.type==='cardPayment':x.source==='flex'||x.type.startsWith('flex')));if(txMin)a=a.filter(x=>(+x.amount||0)>=+txMin);if(txMax)a=a.filter(x=>(+x.amount||0)<=+txMax);if(txMember)a=a.filter(x=>(x.memberId||'me')===txMember);
  ...
  const rr=x=>{const mem=state.members.find(m=>m.id===(x.memberId||'me')),card=state.cards.find(c=>c.id===x.cardId),flex=state.flexAccounts.find(f=>f.id===x.flexId);let kind=x.type==='income'?'GELİR':x.type==='fixedExpense'?'SABİT GİDER':x.type==='cardPayment'?'KART ÖDEMESİ':x.type==='flexPayment'?'ESNEK HESAP ÖDEMESİ':x.type==='flexSpend'?'ESNEK HESAP HARCAMASI':'GİDER';let pay=x.source==='card'?`KART${card?' · '+esc(card.bank)+' '+esc(card.name):''}`:x.source==='flex'?`ESNEK HESAP${flex?' · '+esc(flex.bank):''}`:'NAKİT';let edit=x.type==='income'?'editIncome':x.type==='fixedExpense'?'editFixedExpense':x.type==='cardPayment'?'editCardPayment':x.type==='flexPayment'?'editFlexPayment':'editExpense',del=x.type==='income'?'delIncome':x.type==='cardPayment'?'delCardPayment':x.type==='flexPayment'?'delFlexPayment':'delExpense';return `<div class="item searchResultItem ${x._paid?'isPaid':''}"><div class="ico premiumIco">${x.type==='income'?premiumIcon('income',22):x.type==='cardPayment'?premiumIcon('cards',22):catPremiumIcon(x.category)}</div><div><b>${esc(x.title||kind)}</b><small>${x.date||''} · ${kind}${x.category?' · '+esc(x.category):''} · ${pay}${mem?' · '+esc(mem.name):''}${x.type==='fixedExpense'?' · '+(x._paid?'ÖDENDİ':'ÖDENMEDİ'):''}</small></div><div class="right"><b style="color:${x.type==='income'?'var(--green)':'var(--red)'}">${x.type==='income'?'+':'-'}${money(x.amount)}</b><div><button class="tinyEdit" data-action="${edit}" data-id="${x.id}">DETAY</button>${x.type!=='fixedExpense'&&x.type!=='flexSpend'?`<button class="tinyEdit danger" data-action="${del}" data-id="${x.id}">SİL</button>`:''}</div></div></div>`};
  return `<div class="seg">${B('all','Tümü')}${B('income','Gelir')}${B('expense','Gider')}${B('card','Kart')}</div><div class="advancedFilters"><div class="txSearchBar"><input id="txSearch" placeholder="Hareket ara..." value="${esc(txSearch)}"><button class="btn gold" data-action="runTxSearch">ARA</button></div><input id="txDate" type="date" value="${txDate}"><select id="txCategory"><option value="">Tüm kategoriler</option>${C.map(c=>`<option ${txCategory===c?'selected':''}>${esc(c)}</option>`).join('')}</select><select id="txPay"><option value="">Tüm ödeme türleri</option><option value="cash" ${txPay==='cash'?'selected':''}>Nakit</option><option value="card" ${txPay==='card'?'selected':''}>Kart</option><option value="flex" ${txPay==='flex'?'selected':''}>Esnek Hesap</option></select><input id="txMin" type="number" placeholder="Min ₺" value="${txMin}"><input id="txMax" type="number" placeholder="Max ₺" value="${txMax}"><select id="txMember"><option value="">Tüm üyeler</option>${state.members.map(m=>`<option value="${m.id}" ${txMember===m.id?'selected':''}>${esc(m.icon)} ${esc(m.name)}</option>`).join('')}</select><button class="btn" data-action="clearTxFilters">FİLTRELERİ TEMİZLE</button></div><div class="section"><b>${q?'ARAMA SONUÇLARI':state.selectedMonth}</b><span>${a.length} KAYIT</span></div><div class="list">${a.length?a.map(rr).join(''):'<div class="notice">Eşleşen hareket bulunamadı.</div>'}</div>`
}
function fixedCategoryDetail(cat){
  const items=state.expenses.filter(x=>x.recurring&&(x.category||'Diğer')===cat).sort((a,b)=>String(b.dueDate||b.date||'').localeCompare(String(a.dueDate||a.date||'')));
  const total=items.reduce((n,x)=>n+(+x.amount||0),0);
  return `<div class="fixedCatDetail"><div class="fixedCatDetailHero"><div><small>KATEGORİ</small><h2>${esc(cat)}</h2></div><strong>${money(total)}</strong></div><div class="section"><b>KAYITLAR</b><span>${items.length} KAYIT</span></div><div class="list">${items.length?items.map(x=>{const paid=fixedPaidForMonth(x),p=fixedPaymentFor(x),card=p?.cardId?state.cards.find(c=>c.id===p.cardId):null;return `<div class="item fixedCatDetailItem"><div class="ico premiumIco">${I[x.category]||'●'}</div><div class="fixedCatDetailMain"><b>${esc(x.title)}</b><small>${x.dueDate||x.date||''} · ${paid?'ÖDENDİ':'ÖDENMEDİ'}${paid?' · '+(p?.source==='card'?'KART'+(card?' · '+esc(card.bank):''):'NAKİT'):''}</small></div><div class="fixedCatDetailRight"><strong>${money(x.amount)}</strong><div><button class="tinyEdit" data-action="editFixedExpense" data-id="${x.id}">DETAY</button><button class="tinyEdit danger" data-action="delExpense" data-id="${x.id}">SİL</button></div></div></div>`}).join(''):'<div class="notice">BU KATEGORİDE KAYIT YOK.</div>'}</div></div>`
}
function fixed(){
  const a=state.expenses.filter(x=>x.recurring).sort((x,y)=>{const xp=fixedPaidForMonth(x)?1:0,yp=fixedPaidForMonth(y)?1:0;if(xp!==yp)return xp-yp;return String(x.dueDate||x.date||'').localeCompare(String(y.dueDate||y.date||''))||String(x.title||'').localeCompare(String(y.title||''),'tr')});
  const cats=categoryStats(a),catTotal=cats.reduce((n,x)=>n+(+x[1]||0),0);
  const catHtml=cats.length?`<div class="fixedCategoryTotals">${cats.map(([cat,total])=>`<button type="button" class="fixedCatTotal" data-action="fixedCategoryDetail" data-cat="${esc(cat)}"><span><i>${I[cat]||'●'}</i><b>${esc(cat)}</b></span><strong>${money(total)}</strong></button>`).join('')}<div class="fixedCatGrand"><span>TOPLAM SABİT GİDER</span><strong>${money(catTotal)}</strong></div></div>`:'<div class="notice">KATEGORİ TOPLAMI İÇİN KAYIT YOK.</div>';
  return`<div class="section"><b>SABİT GİDERLER</b><button class="miniAddBtn" data-action="addFixedExpense">+ GİDER EKLE</button></div><div class="section fixedCatHead"><b>KATEGORİ TOPLAMLARI</b><span>${cats.length} KATEGORİ</span></div>${catHtml}<div class="notice fixedSortNote">ÖDENMEMİŞ GİDERLER ÜSTTE · ÖDENMİŞ GİDERLER ALTTA</div><div class="list fixedList">${a.length?a.map(x=>{const paid=fixedPaidForMonth(x),pay=fixedPaymentFor(x);return`<div class="item fixedItem ${paid?'isPaid':''}" data-fixed-id="${x.id}"><div class="ico premiumIco" data-action="editFixedExpense" data-id="${x.id}">${I[x.category]||'●'}</div><div class="fixedMain" data-action="editFixedExpense" data-id="${x.id}"><b>${esc(x.title)}</b><small>${x.dueDate||x.date} · SABİT${paid?` · ÖDENDİ ${pay?.date||''} · ${pay?.source==='card'?'KART':'NAKİT'}`:''}</small></div><div class="fixedRight"><b style="color:${paid?'var(--green)':'var(--red)'}">${paid?'✓ ':'-'}${money(x.amount)}</b><button type="button" class="paidToggle ${paid?'on':''}" data-action="toggleFixedPaid" data-id="${x.id}">${paid?'↶ GERİ AL':'ÖDEDİM'}</button>${paid?`<button type="button" class="tinyEdit" data-action="editFixedPayment" data-id="${x.id}">ÖDEMEYİ DÜZENLE</button>`:''}</div></div>`}).join(''):'<div class="notice">SABİT GİDER YOK.</div>'}</div>`
}
...
function bind(){
  ...
  const fixedPaymentFormEl=$('#fixedPaymentForm');if(fixedPaymentFormEl)fixedPaymentFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),p=(state.fixedPayments||[]).find(p=>p.id===modal?.paymentId),ex=state.expenses.find(x=>x.id===modal?.id);if(!p)return;const oldAmount=+p.amount||0,oldCard=p.source==='card'?state.cards.find(c=>c.id===p.cardId):null;if(oldCard)oldCard.balance=Math.max(0,(+oldCard.balance||0)-oldAmount);const nextAmount=+d.amount||0,nextSource=d.source==='card'?'card':'cash';if(nextSource==='card'&&!d.cardId)return alert('Kart seçin.');p.amount=nextAmount;p.date=d.date||iso();p.category=d.category||ex?.category||'Diğer';p.source=nextSource;p.cardId=nextSource==='card'?d.cardId:null;if(ex){ex.category=p.category;(state.fixedPayments||[]).filter(fp=>fp.expenseId===ex.id).forEach(fp=>fp.category=ex.category||'Diğer')}const nextCard=nextSource==='card'?state.cards.find(c=>c.id===p.cardId):null;if(nextCard)nextCard.balance=(+nextCard.balance||0)+nextAmount;await save();modal=null;render();showToast('ÖDEME GÜNCELLENDİ')};
...
async function saveExpense(d,i){
  ...
  const x={id:i||id(),title:upper((d.title||(fixed?'SABİT GİDER':'GİDER')).trim()||(fixed?'SABİT GİDER':'GİDER')),amount,category:fixed?(d.fixedCategory||'Kira'):((d.normalCategory==='Diğer'&&(d.customCategory||'').trim())?upper(d.customCategory.trim()):(d.normalCategory||'Diğer')),date:fixed?(d.dueDate||iso()):(d.date||iso()),dueDate:fixed?(d.dueDate||iso()):null,recurring:fixed,paid:fixed?(old?.paid||false):true,sortOrder:old?.sortOrder??state.expenses.length,paidMonths:[...(old?.paidMonths||[])],source,memberId:d.memberId||old?.memberId||'me',cardId:source==='card'?d.cardId:null,attachment:modal?.attachment||old?.attachment||''};
  ...
  if(i){const n=state.expenses.findIndex(z=>z.id===i);state.expenses[n]={...state.expenses[n],...x};(state.fixedPayments||[]).filter(p=>p.expenseId===i).forEach(p=>{p.amount=amount;p.title=x.title;p.category=x.category||'Diğer'})}else state.expenses.push(x);await save();modal=null;render()
}
...
async function saveCardSpend(d,cardId){
  ...
  for(let n=1;n<=count;n++){
    ...
    state.expenses.push({id:id(),cardTxId:txId,source:'card',cardId,title:label,baseTitle:title,amount:part,totalAmount:amount,category,date:partDate,dueDate:partDate,recurring:false,paid:true,installmentGroup:groupId,installmentNo:n,installmentCount:count});
  }
  state.installments.push({id:groupId,cardId,title,category,totalAmount:amount,count,startDate:date,createdAt:iso()});c.balance=(+c.balance||0)+amount;
  await save();modal=null;render();
}
