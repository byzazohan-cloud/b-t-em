...
function cardSpendForm(c){return`<form class="form" id="cardSpendForm">${input('amount','Toplam Tutar',0,'number','step="0.01" min="0"')}${input('title','Açıklama','')}${select('category','Kategori',C,'Market')}${input('date','Tarih',iso(),'date')}${select('paymentType','Ödeme Şekli',['Tek Çekim','Taksitli'],'Tek Çekim')}<div class="field"><label>Taksit Sayısı (Taksitli ise)</label><input name="installmentCount" type="number" min="2" max="36" value="2"></div><div class="notice">Taksitli seçimde toplam tutar aylara otomatik dağıtılır; her taksit ayrı aya tek gider olarak yansır.</div><button type="button" class="btn" data-action="receipt">📷 Fiş / Kamera</button><button class="btn gold" type="submit">Harcamayı Kaydet</button></form>`}
