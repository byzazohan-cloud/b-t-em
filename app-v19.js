...
function cards(){
  const B=(k,l)=>`<button data-action="financeTab" data-finance="${k}" class="${financeTab===k?'active':''}">${l}</button>`;
  ...
  return `<div class="financeHub"><div class="financeTabs financeHubTabs">${B('cards','KARTLAR')}${B('accounts','HESAPLAR')}${B('debts','BORÇLAR')}</div>${panel}</div>`
}
function cardsPanel(){return `<div class="section financeSectionHead"><b>DİKEY KARTLARIM</b><button class="miniAddBtn" data-action="addCard">+ KART EKLE</button></div><div class="financeCarousel">${state.cards.map(credit).join('')||'<div class="notice">HENÜZ KREDİ KARTI EKLENMEDİ.</div>'}</div><div class="financeDots">${state.cards.map((c,i)=>`<button data-action="scrollCard" data-index="${i}">${esc(c.bank)} · •${esc(c.last4)}</button>`).join('')}</div>`}
