...
function normalizeV19(st){
  ...
  // V113+ S1: ileri sürümlerin ortak bağlantı/denetim verileri. Eski kayıtları değiştirmez; yalnız eksik kapsayıcıları oluşturur.
  st.transactionLinks=st.transactionLinks&&typeof st.transactionLinks==='object'?st.transactionLinks:{};
  st.sharedLimitGroups=Array.isArray(st.sharedLimitGroups)?st.sharedLimitGroups:[];
  st.billIdentityRules=st.billIdentityRules&&typeof st.billIdentityRules==='object'?st.billIdentityRules:{};
  st.auditState=st.auditState&&typeof st.auditState==='object'?st.auditState:{acknowledged:{},lastRun:null};
  if(!st.auditState.acknowledged||typeof st.auditState.acknowledged!=='object')st.auditState.acknowledged={};
  st.schemaMeta=st.schemaMeta&&typeof st.schemaMeta==='object'?st.schemaMeta:{};
  st.schemaMeta.core='V113_PLUS_S1';
...
// V113+ S1 merkezi finansal hesap kaynağı. Ekranlar aynı gerçekleşmiş kayıt kümesini tüketir.
function financeMonthCore(m=state.selectedMonth){
  const incomes=(state.incomes||[]).filter(x=>String(x.date||'').startsWith(m));
  const expenses=actualExpenseEntriesForMonth(m);
  const cashExpenses=expenses.filter(x=>x.source!=='card'),cardExpenses=expenses.filter(x=>x.source==='card');
  const incomeTotal=incomes.reduce((a,x)=>a+(+x.amount||0),0),expenseTotal=expenses.reduce((a,x)=>a+(+x.amount||0),0);
  const cashTotal=cashExpenses.reduce((a,x)=>a+(+x.amount||0),0),cardTotal=cardExpenses.reduce((a,x)=>a+(+x.amount||0),0);
  const cardPaid=(state.cardPayments||[]).filter(x=>String(x.date||'').startsWith(m)).reduce((a,x)=>a+(+x.amount||0),0);
  return{month:m,incomes,expenses,cashExpenses,cardExpenses,incomeTotal,expenseTotal,remain:incomeTotal-expenseTotal,cashTotal,cardTotal,cardPaid,paidTotal:cashTotal+cardPaid};
}
function totals(m=state.selectedMonth){const c=financeMonthCore(m);return{i:c.incomeTotal,e:c.expenseTotal,r:c.remain}}
function cashFlow(m=state.selectedMonth){const c=financeMonthCore(m);return{spent:c.expenseTotal,paid:c.paidTotal,directPaid:c.cashTotal,cardPaid:c.cardPaid}}
function paymentSourceStats(m=state.selectedMonth){const c=financeMonthCore(m);return{cash:c.cashExpenses,card:c.cardExpenses,cashTotal:c.cashTotal,cardTotal:c.cardTotal}}
...
const HANE_SW_BUILD='19.4.8.20260921-V113-PROFILE-FUNCTION-RESTORE';
const HANE_SW_URL='./sw.js?v='+encodeURIComponent(HANE_SW_BUILD);
const HANE_ENGINE_CACHE='hane-v19-4-8-V113-PROFILE-FUNCTION-RESTORE';
