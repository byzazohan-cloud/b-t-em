...
function fixed(){
  ...
  const fixedRows=fixedItems.map(x=>{const paid=fixedPaidForMonth(x,m),pay=fixedPaymentFor(x,m);return `<div class="item expenseCenterRow fixedExpenseCenterRow ${paid?'isPaid':''}" data-fixed-id="${x.id}"><div class="ico premiumIco" data-action="editFixedExpense" data-id="${x.id}">${catPremiumIcon(x.category)}</div><div class="expenseCenterMain" data-action="editFixedExpense" data-id="${x.id}"><b>${esc(x.title)}</b><small>${prettyDate(dateForSelectedMonth(x.dueDate||x.date,m))} · ${esc(x.category||'Diğer')}${paid?' · ÖDENDİ'+(pay?.date?' '+prettyDate(pay.date):''):' · BEKLİYOR'}</small></div><div class="expenseCenterRight"><b style="color:${paid?'var(--green)':'var(--red)'}">${money(paid?(+(pay?.actualAmount??pay?.amount)||fixedBillAmount(x)):fixedBillAmount(x))}</b><button type="button" class="paidToggle ${paid?'on':''}" data-action="toggleFixedPaid" data-id="${x.id}">${paid?'↶ GERİ AL':'ÖDEDİM'}</button></div></div>`}).join('');
