...
function expenseForm(x={}){
  ...
  const catTiles=(cats,name)=>`<div class="premiumCatGrid ${name==='fixedCategory'?'fixed3dDirectGrid':''}" data-select-name="${name}">${cats.map(c=>{const f=name==='fixedCategory'?fixed3dFile(c):'';return `<button type="button" class="premiumCatBtn ${f?'fixed3dDirectBtn':''} ${c===currentCat?'active':''}" data-cat="${esc(c)}" style="--cat:${catColor(c)}">${f?`<img class="fixed3dDirectImg" src="icons/categories3d/${f}.png?v=V121" alt="${esc(c)}">`:catPremiumIcon(c)}<b class="${f?'fixed3dDirectLabel':''}">${esc(c)}</b></button>`}).join('')}</div><select class="hiddenCatSelect" name="${name}">${cats.map(c=>`<option ${c===currentCat?'selected':''}>${esc(c)}</option>`).join('')}</select>`;
...
const HANE_SW_BUILD='19.4.8.20260921-V121-SW-DIRECT-3D';
