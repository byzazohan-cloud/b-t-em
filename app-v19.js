...
function bind(){
  ...
  const categoryFormEl=$('#categoryForm');if(categoryFormEl)categoryFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries()),old=modal?.oldCat||'',name=upper((d.name||'').trim());if(!name)return alert('Kategori adı gerekli');if(old&&old!==name){state.customCategories=state.customCategories.filter(x=>x!==old);if(!C.includes(name))state.customCategories.push(name);state.expenses.forEach(x=>{if(x.category===old)x.category=name});(state.fixedPayments||[]).forEach(x=>{if(x.category===old)x.category=name});(state.cardTransactions||[]).forEach(x=>{if(x.category===old)x.category=name});Object.keys(state.statementCategoryRules||{}).forEach(k=>{if(state.statementCategoryRules[k]===old)state.statementCategoryRules[k]=name});delete state.categoryMeta[old]}else if(!C.includes(name))state.customCategories.push(name);state.categoryMeta[name]={...(state.categoryMeta[name]||{}),iconKey:d.iconKey||'Diğer',color:d.color||'#d8ad4f'};normalizeV19(state);await save();modal=null;render();showToast('KATEGORİ KAYDEDİLDİ')};
  document.querySelectorAll('.categoryIconChoice').forEach(b=>b.onclick=()=>{const f=b.closest('form'),inp=f?.querySelector('[name="iconKey"]');if(inp)inp.value=b.dataset.iconKey||'Diğer';f?.querySelectorAll('.categoryIconChoice').forEach(x=>x.classList.toggle('active',x===b))});
  const categoryMergeFormEl=$('#categoryMergeForm');if(categoryMergeFormEl)categoryMergeFormEl.onsubmit=e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries());if(!d.source||!d.target||d.source===d.target)return alert('Farklı iki kategori seçin.');const n=categoryUsageCount(d.source);open('BİRLEŞTİRME ONAYI',`<div class="notice"><b>${esc(d.source)}</b> → <b>${esc(d.target)}</b><br><br><b>${n}</b> kayıt/kural etkilenecek. Hiçbir finansal kayıt silinmeyecek.</div><button class="btn gold" data-action="categoryMergeConfirm" data-source="${esc(d.source)}" data-target="${esc(d.target)}">ONAYLA VE BİRLEŞTİR</button>`)};
  const categoryDeleteTransferFormEl=$('#categoryDeleteTransferForm');if(categoryDeleteTransferFormEl)categoryDeleteTransferFormEl.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(e.currentTarget).entries());if(!d.target)return alert('Hedef kategori seçin.');if(!confirm(d.source+' kategorisindeki kayıtlar '+d.target+' kategorisine aktarılsın mı?'))return;const n=categoryTransfer(d.source,d.target);normalizeV19(state);await save();modal=null;render();showToast(n+' KAYIT/KURAL AKTARILDI')};
