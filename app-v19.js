...
function normalizeV19(st){
  ...
  st.expenses.forEach((x,i)=>{if(typeof x.sortOrder!=='number')x.sortOrder=i;if(!Array.isArray(x.paidMonths))x.paidMonths=[]});
  return st;
}
...
async function saveExpense(d,i){
  ...
  const old=i?state.expenses.find(z=>z.id===i):null,fixed=d.expenseType==='fixed';
  const x={id:i||id(),title:upper((d.title||(fixed?'SABİT GİDER':'GİDER')).trim()||(fixed?'SABİT GİDER':'GİDER')),amount,category:fixed?(d.fixedCategory||'Kira'):(d.normalCategory||'Diğer'),date:fixed?(d.dueDate||iso()):(d.date||iso()),dueDate:fixed?(d.dueDate||iso()):null,recurring:fixed,paid:fixed?(old?.paid||false):true,sortOrder:old?.sortOrder??state.expenses.length,paidMonths:[...(old?.paidMonths||[])]};
