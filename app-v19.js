...
function hanAudit(){
 ...
 const add=(level,group,title,detail,recordAction='',recordId='')=>issues.push({level,group,title,detail,recordAction,recordId});
 ...
 for(let i=0;i<exps.length;i++)for(let j=i+1;j<exps.length;j++){const a=exps[i],b=exps[j];if(a.date!==b.date)continue;const da=Math.abs(+(a.actualAmount??a.amount)-+(b.actualAmount??b.amount));if(da>50)continue;const na=norm(a.baseTitle||a.title),nb=norm(b.baseTitle||b.title);const similar=na&&nb&&(na===nb||na.includes(nb)||nb.includes(na));if(similar)add('warn','FİNANS','OLASI MÜKERRER / BENZER İŞLEM',`${a.date} · ${a.title||''} ${money(+(a.actualAmount??a.amount))} (${a.source||'?'}) ↔ ${b.title||''} ${money(+(b.actualAmount??b.amount))} (${b.source||'?'}) · fark ${money(da)}`,'editExpense',a.id)}
 ...
 const road=new Map();exps.filter(x=>/ULAŞIM|ULASIM|YOL/i.test(String(x.category||'')+' '+String(x.title||''))).forEach(x=>{if(!road.has(x.date))road.set(x.date,[]);road.get(x.date).push(x)});road.forEach((rows,date)=>{if(rows.length>=5)add('warn','FİNANS','AYNI GÜN ÇOKLU ULAŞIM KAYDI',`${date} tarihinde ${rows.length} ulaşım/yol kaydı bulundu. Toplam ${money(rows.reduce((a,x)=>a+(+(x.actualAmount??x.amount)||0),0))}.`,'editExpense',rows[0].id)});
...
const HANE_SW_BUILD='20260923-V113-S19-HAN';
const HANE_SW_URL='./sw.js?v='+encodeURIComponent(HANE_SW_BUILD);
const HANE_ENGINE_CACHE='hane-engine-19.4.28-V113-S19-HAN';
