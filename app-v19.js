...
function menuBody(){const a=[['home','🏠','ANA EKRAN'],['transactions','📋','HAREKETLER'],['fixed','🧾','GİDERLER'],['cards','💳','KARTLAR / HESAPLAR'],['reports','📊','RAPORLAR'],['alerts','🔔','HATIRLATMALAR'],['settings','⚙️','AYARLAR'],['profile','👤','PROFİL']];return `<div class="menuList">${a.map(x=>`<button data-action="menuGo" data-go="${x[0]}"><i>${x[1]}</i><b>${x[2]}</b><span>›</span></button>`).join('')}</div>`}
...
function cards(){const B=(k,l)=>`<button data-action="financeTab" data-finance="${k}" class="${financeTab===k?'active':''}">${l}</button>`;return `<div class="financeTabs">${B('cards','KARTLAR')}${B('accounts','HESAPLAR')}${B('flex','ESNEK HESAP')}</div>${financeTab==='cards'?cardsPanel():financeTab==='accounts'?accountsPanel():flexPanel()}`}
...
function credit(c){return `<div class="credit card luxuryCard ${c.style||'blackgold'}" data-card-id="${c.id}"><div class="cardTop"><b>${esc(c.bank)}</b><span>${esc(c.network||'VISA')}</span></div><small>${esc(c.name)}</small><div class="chip">▤</div><div class="digits">•••• •••• •••• ${esc(c.last4)}</div><div class="grid"><div><small>LİMİT</small><b>${money(c.limit)}</b></div><div><small>GÜNCEL BORÇ</small><b>${money(c.balance)}</b></div><div><small>HESAP KESİM</small><b>${cardPaymentInfo(c).statementDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})}</b></div><div><small>SON ÖDEME</small><b>${cardPaymentInfo(c).dueDate.toLocaleDateString('tr-TR',{day:'numeric',month:'short'})}</b></div></div><div class="cardActions"><button class="btn" data-action="cardSpend" data-id="${c.id}">＋ HARCAMA EKLE</button><button class="btn gold" data-action="cardPay" data-id="${c.id}">₺ ÖDEME YAPTIM</button><button class="iconEdit" data-action="editCard" data-id="${c.id}">✎</button></div></div>`}
...
function about(){return`<div class="profile"><div class="aboutV5Logo">${haneFullLogo("aboutV5")}</div><p>SÜRÜM 19.4</p><p>PREMİUM EV BÜTÇEN.<br>VERİLERİN CİHAZINDA ŞİFRELİ SAKLANIR.</p></div>`}
function view(){return({home,transactions,fixed,cards,reports,profile,backup,settings,categories,theme,alerts,about,monthSpent,monthPaid}[current]||home)()}
