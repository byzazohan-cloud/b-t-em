...
function top(title,plus){const t=el('div','topbar');const b=el('button','back','‹');b.onclick=back;t.append(b);t.append(el('div','title',title));const r=el('button','icon-btn',plus?'＋':'');if(plus)r.onclick=plus;t.append(r);return t}
...
function formSecurity(sh,force=false){const f=el('form');f.innerHTML=`${force?'<div class="security-note">Varsayılan parola 1234 güvenli değildir. Şimdi en az 8 karakterli yeni bir parola belirleyin.</div>':''}${field('Mevcut Parola','oldPin','','password','autocomplete="current-password" required')}${field('Yeni Parola','pin','','password','autocomplete="new-password" minlength="8" maxlength="64" required')}${field('Yeni Parola Tekrar','pin2','','password','autocomplete="new-password" minlength="8" maxlength="64" required')}${field('Kilitleme Süresi (dakika)','lockMinutes',state.profile.lockMinutes||15,'number','min="1" max="120" required')}<button class="primary">Güvenliği Güncelle</button>`;f.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(f));if(String(d.pin).length<8){notice('Yeni parola en az 8 karakterli olmalı.');return}if(d.pin!==d.pin2){notice('Yeni parolalar aynı değil.');return}try{const oldKey=await deriveKey(d.oldPin,unb64(secureMeta.salt),secureMeta.iterations||PBKDF2_ITERATIONS);await decryptState(readJSON(DATA_KEY),oldKey)}catch{notice('Mevcut parola yanlış.');return}state.profile.lockMinutes=Math.min(120,Math.max(1,+d.lockMinutes||15));await createSecureStore(state,d.pin,{forcePinChange:false});secureMeta.forcePinChange=false;localStorage.setItem(META_KEY,JSON.stringify(secureMeta));modal=null;render();notice('Parola değiştirildi ve veriler yeni anahtarla tekrar şifrelendi.')};sh.append(f)}
